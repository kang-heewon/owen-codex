...
function buildMaintainerCloseComment({ prNumber }) {
  return [
    ...
    '',
    'A hot-fix build is available now. Try it with `owx update --dev` and let us know whether it resolves the issue.',
  ].join('\n');
}
...
function buildMaintainerPrComment({ issueNumbers }) {
  return [
    ...
    '',
    'A hot-fix build is available now. Issue creators can try it with `owx update --dev` and let us know whether it resolves the issue.',
  ].join('\n');
}
