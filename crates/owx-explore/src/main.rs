...
const CODEX_BIN_ENV: &str = "OWX_EXPLORE_CODEX_BIN";
const HARNESS_ROOT_ENV: &str = "OWX_EXPLORE_ROOT";
const CODEX_TIMEOUT_MS_ENV: &str = "OWX_EXPLORE_CODEX_TIMEOUT_MS";
const PROCESS_LIMIT_ENV: &str = "OWX_EXPLORE_PROCESS_LIMIT";
const CODEX_OUTPUT_LIMIT_BYTES_ENV: &str = "OWX_EXPLORE_CODEX_OUTPUT_LIMIT_BYTES";
...
const TEMP_ALLOWLIST_DIR_PREFIX: &str = "owx-explore-allowlist-";
...
const WINDOWS_UNSUPPORTED_ALLOWLIST_MESSAGE: &str =
    "owx explore built-in harness is not ready on Windows because its allowlist runtime relies on POSIX sh/bash wrappers. Set OWX_EXPLORE_BIN to a compatible custom harness, prefer `owx sparkshell` for shell-native read-only lookups, or run `owx doctor` for readiness details.";
...
fn emit_model_fallback_event(event: &FallbackEvent) {
    ...
    eprintln!(
        "[owx explore] spark model `{}` unavailable or failed (exit {}). Falling back to `{}`.",
        event.from_model, event.exit_code, event.to_model
    );
    if !event.stderr.trim().is_empty() {
        eprintln!("[owx explore] spark stderr: {}", event.stderr.trim());
    }
}

fn fallback_attempt_event_message(event: &FallbackEvent) -> String {
    format!(
        "[owx explore] fallback-attempt=model from=`{}` to=`{}` reason=spark_attempt_failed exit={}. Cost/behavior boundary changed if fallback succeeds; stdout fallback notice is emitted only after successful fallback output.",
...
fn usage() -> &'static str {
    "Usage: owx-explore --cwd <dir> --prompt <text> --prompt-file <explore-prompt.md> --instructions-file <AGENTS.md> --model-spark <model> --model-fallback <model>"
}
...
fn invoke_codex(args: &Args, model: &str, prompt_contract: &str) -> io::Result<AttemptResult> {
    ...
    match output {
        ...
        TimedCommandOutput::TimedOut { stderr } => Ok(AttemptResult {
            status_code: 124,
            stderr: format!(
                "[owx explore] codex exec timed out after {}ms; terminated process tree{}{}",
                timeout.as_millis(),
        ...
        TimedCommandOutput::ProcessLimitExceeded {
            ...
            status_code: 125,
            stderr: format!(
                "[owx explore] codex exec exceeded per-run process limit ({process_count}>{process_limit}); terminated process tree to avoid runaway shell storms{}{}",
                if stderr.trim().is_empty() {
                    ""
                } else {
        ...
        TimedCommandOutput::OutputLimitExceeded {
            ...
            status_code: 126,
            stderr: format!(
                "[owx explore] codex exec exceeded subprocess {stream} output limit ({output_limit} bytes); terminated process tree to avoid unbounded memory growth{}{}",
                if stderr.trim().is_empty() {
                    ""
                } else {
                    ". stderr before termination: "
...
fn temp_output_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    env::temp_dir().join(format!("owx-explore-{}-{}.md", std::process::id(), nanos))
}

fn compose_exec_prompt(user_prompt: &str, prompt_contract: &str) -> String {
    format!(
        concat!(
            "You are OWX Explore, a low-cost read-only repository exploration harness.\\n",
...
fn build_direct_wrapper(self_exe: &Path, command: &str) -> Result<String, String> {
    ...
    Ok(format!(
        "#!/bin/sh\nprintf '%s\\n' {} >&2\nexit 127\n",
        shell_quote(&format!(
            "owx explore allowlisted host command `{command}` is unavailable on this host"
        )),
    ))
}
...
fn resolve_host_command(command: &str) -> Option<PathBuf> {
    ...
    if candidate.is_absolute()
        ...
        && !is_owx_explore_allowlist_path(candidate)
    {
    ...
    for entry in env::split_paths(&path) {
        ...
        if is_usable_host_command(&resolved) && !is_owx_explore_allowlist_path(&resolved) {
...
fn is_owx_explore_allowlist_path(path: &Path) -> bool {
...
fn validate_direct_command(command_name: &str, args: &[String]) -> Result<(), String> {
    if !ALLOWED_DIRECT_COMMANDS.contains(&command_name) {
        return Err(format!(
            "command `{command_name}` is not on the owx explore allowlist"
        ));
    }
    ...
    match command_name {
        "rg" => {
            ...
            {
                return Err("ripgrep `--pre` is not allowed in owx explore".to_string());
            }
            if args.iter().any(|arg| arg == "-") {
                return Err("ripgrep stdin (`-`) is not allowed in owx explore".to_string());
            }
        }
        "grep" => {
            if args.iter().any(|arg| arg == "-") {
                return Err("grep stdin (`-`) is not allowed in owx explore".to_string());
            }
            if non_option_operands(args).len() < 2 {
                return Err(
                    "grep requires a pattern and at least one file/path in owx explore".to_string(),
                );
            }
        }
        "find"
        ...
        {
            return Err(
                "find actions that execute, delete, or write files are not allowed in owx explore"
                    .to_string(),
            );
        }
        "find" => {}
        "cat" => {
            ...
            if operands.is_empty() {
                return Err("cat requires at least one file/path in owx explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("cat stdin (`-`) is not allowed in owx explore".to_string());
            }
        }
        "head" | "wc" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err(format!(
                    "{command_name} requires at least one file/path in owx explore"
                ));
            }
            if operands.contains(&"-") {
                return Err(format!(
                    "{command_name} stdin (`-`) is not allowed in owx explore"
                ));
            }
        }
        "tail" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err("tail requires at least one file/path in owx explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("tail stdin (`-`) is not allowed in owx explore".to_string());
            }
            if args.iter().any(|arg| {
                matches!(arg.as_str(), "-f" | "-F" | "--retry") || arg.starts_with("--follow")
            }) {
                return Err("tail follow/retry modes are not allowed in owx explore".to_string());
            }
        }
        _ => {}
    }
...
fn validate_repo_paths(command_name: &str, args: &[String]) -> Result<(), String> {
    ...
    for operand in candidate_paths {
        ...
        if !is_textually_inside && !is_canonically_inside {
            return Err(format!(
                "path `{operand}` escapes the owx explore repository root {}",
                repo_root.display()
            ));
        }
            ...
            if let Some(canonical_candidate) = canonical_candidate {
                if let Some(canonical_repo_root) = &canonical_repo_root {
                    if !canonical_candidate.starts_with(canonical_repo_root) {
                        return Err(format!(
                            "path `{operand}` resolves outside the owx explore repository root {}",
    ...
    fn resolve_codex_binary_resolves_bare_env_override_from_path() {
        ...
        let root = TempDirGuard {
            path: env::temp_dir().join(format!(
                "owx-explore-resolve-codex-binary-{}-{}",
                std::process::id(),
                SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .unwrap_or_default()
...
"#,
    ...
    fn build_codex_path_keeps_allowlist_first_and_only_adds_sandbox_bin() {
        let allowlist_bin = Path::new("/tmp/owx-explore-allowlist/bin");
        let sandbox_bin = Path::new("/tmp/owx-explore-sandbox-bin");
    ...
    #[cfg(unix)]
    #[test]
    fn resolve_host_command_skips_owx_explore_allowlist_wrappers() {
        let _guard = env_lock();
        let allowlist_root = temp_allowlist_dir().expect("allowlist root");
        let real_root = TempDirGuard {
            path: env::temp_dir().join(format!(
                "owx-explore-host-resolution-{}-{}",
                std::process::id(),
                SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .unwrap_or_default()
    ...
    #[test]
    fn discover_codex_support_dirs_includes_home_owx_and_codex_when_present() {
        let _guard = env_lock();
        let root = temp_allowlist_dir().expect("temp root");
        let home_dir = root.path.join("home");
        create_dir_all(home_dir.join(".owx")).expect("create .owx");
        ...
        assert_eq!(dirs, vec![home_dir.join(".owx"), home_dir.join(".codex")]);
    }
    ...
    fn allowlist_platform_diagnostic_blocks_windows_with_actionable_guidance() {
        ...
        assert!(diagnostic.contains("OWX_EXPLORE_BIN"));
...
"#,
    ...
    fn fallback_attempt_event_distinguishes_attempt_from_output_notice() {
        ...
        assert!(!message.contains("## OWX Explore fallback"));
    }
    ...
    fn fallback_output_notice_records_model_boundary() {
        ...
        assert!(notice.contains("## OWX Explore fallback"));
