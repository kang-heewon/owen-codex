...
pub fn default_state_file() -> PathBuf {
    env::temp_dir().join("owx-api-daemon.json")
}
...
pub fn route_request(
    ...
    if let Some(expected) = expected_bearer.filter(|token| !token.is_empty()) {
        ...
        return Response::json(
            ...
            json!({
                "error": {
                    "message": "local bearer token required by OWX_API_REQUIRE_LOCAL_BEARER",
                    "type": "unauthorized"
                }
            }),
        );
    ...
    match (request.method.as_str(), request.path.as_str()) {
        ...
        ("GET", "/v1/models") => Response::json(
            ...
            json!({
                "object": "list",
                "data": [
                    {"id": "owx-mock", "object": "model", "owned_by": "owx"},
                    {"id": "owx-private", "object": "model", "owned_by": "local"}
                ]
            }),
        ),
...
fn local_bearer_required() -> bool {
    env::var("OWX_API_REQUIRE_LOCAL_BEARER")
        .ok()
...
fn responses_response(request: &Request, backend: &BackendMode) -> Response {
    ...
    if body.get("stream").and_then(Value::as_bool).unwrap_or(false) {
        ...
        let payload = json!({"type": "message", "delta": "owx mock response"});
    ...
    Response::json(
        200,
        json!({
            "id": format!("owx-{}", now_unix()),
            "object": "response",
            "model": body.get("model").and_then(Value::as_str).unwrap_or("owx-mock"),
            "backend": backend.as_str(),
            "output_text": format!("owx mock response to: {}", redact_secrets(&input)),
            "choices": [{
                "index": 0,
                "message": {"role": "assistant", "content": "owx mock response"},
                "finish_reason": "stop"
            }]
        }),
    )
...
fn chat_response(request: &Request, backend: &BackendMode) -> Response {
    ...
    if body.get("stream").and_then(Value::as_bool).unwrap_or(false) {
        let text = if *backend == BackendMode::RealPrivate {
            ...
            "owx mock response".to_string()
        };
        let chunk = json!({
            "id": format!("chatcmpl-owx-{}", now_unix()),
...
fn real_private_text(body: &Value) -> std::result::Result<String, (u16, &'static str, String)> {
    if let Some(fixture) = env::var("OWX_API_REAL_PRIVATE_RESPONSE_TEXT")
        .ok()
    ...
    let Some(auth) = discover_codex_oauth() else {
        return Err((
            503,
            "missing_auth",
            "missing Codex OAuth token; run Codex login or set OWX_API_REAL_PRIVATE_RESPONSE_TEXT for fixture smoke tests".to_string(),
        ));
    };

    let Some(upstream) = env::var("OWX_API_PRIVATE_BACKEND_URL")
        .ok()
        .filter(|value| !value.trim().is_empty())
    else {
        return Err((
            ...
            "real-private OAuth was found, but OWX_API_PRIVATE_BACKEND_URL is not configured for this private backend build".to_string(),
        ));
    };
...
fn text_response_json(
    ...
    Response::json(
        ...
        json!({
            "id": format!("owx-{}", now_unix()),
            "object": object,
            "model": body.get("model").and_then(Value::as_str).unwrap_or("owx-private"),
            "backend": backend,
            "output_text": text,
            "choices": [{
                "index": 0,
...
fn image_response(request: &Request, backend: &BackendMode) -> Response {
    ...
    if body.get("stream").and_then(Value::as_bool).unwrap_or(false) {
        let mut stream = sse_event(
            ...
            &json!({"type": "image_generation.partial_image", "b64_json": "owx-mock-image-fragment"}),
        );
    ...
    Response::json(
        ...
        json!({
            ...
            "backend": backend.as_str(),
            "data": [{
                "url": "https://localhost.owx.invalid/mock-image.png",
...
fn real_private_image(
    ...
    if let Some(b64_json) = env::var("OWX_API_REAL_PRIVATE_IMAGE_B64_JSON")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
    ...
    if let Some(url) = env::var("OWX_API_REAL_PRIVATE_IMAGE_URL")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
        return Ok(GeneratedImage {
    ...
    let Some(auth) = discover_codex_oauth() else {
        return Err((
            503,
            "missing_auth",
            "missing Codex OAuth token; run Codex login or set OWX_API_REAL_PRIVATE_IMAGE_B64_JSON/OWX_API_REAL_PRIVATE_IMAGE_URL for fixture smoke tests".to_string(),
        ));
    };

    if let Some(upstream) = env::var("OWX_API_PRIVATE_IMAGE_BACKEND_URL")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
    ...
    if let Some(upstream) = env::var("OWX_API_PRIVATE_BACKEND_URL")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
    ...
    Err((
        ...
        "real-private OAuth was found, but neither OWX_API_PRIVATE_IMAGE_BACKEND_URL nor OWX_API_PRIVATE_BACKEND_URL is configured for image generation".to_string(),
    ))
}
...
fn image_response_json(body: &Value, backend: &str, image: GeneratedImage) -> Response {
    ...
    if item.is_empty() {
        item.insert(
            "url".to_string(),
            Value::String("https://localhost.owx.invalid/private-image.png".to_string()),
        );
    }
...
fn discover_codex_oauth() -> Option<CodexOAuth> {
    if let Some(token) = env::var("OWX_API_CODEX_OAUTH_TOKEN")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
        return Some(CodexOAuth {
            token,
            account_id: env::var("OWX_API_CODEX_ACCOUNT_ID")
                .ok()
                .filter(|value| !value.trim().is_empty()),
        });
    }
...
fn build_codex_native_request(
    ...
    let session_id = env::var("OWX_API_CODEX_SESSION_ID")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "owx-api-local-session".to_string());
    let thread_id = env::var("OWX_API_CODEX_THREAD_ID")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "owx-api-local-thread".to_string());
    let installation_id = env::var("OWX_API_CODEX_INSTALLATION_ID")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "owx-api-local-installation".to_string());
    let window_id = env::var("OWX_API_CODEX_WINDOW_ID")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "owx-api-local-window".to_string());
    let model = body
        .get("model")
        .and_then(Value::as_str)
        .filter(|value| !value.trim().is_empty())
        .map(str::to_string)
        .or_else(|| env::var("OWX_API_GENERATE_MODEL").ok())
        .unwrap_or_else(|| "owx-private".to_string());
...
fn codex_user_agent() -> String {
    env::var("OWX_API_CODEX_USER_AGENT")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| "codex_cli_rs/0.130.0 (owx-api)".to_string())
}
...
fn build_codex_image_request(
    ...
    let model = body
        ...
        .filter(|value| !value.trim().is_empty())
        .map(str::to_string)
        .or_else(|| env::var("OWX_API_IMAGE_MODEL").ok())
        .unwrap_or_else(|| "owx-private-image".to_string());
    let prompt = body
        .get("prompt")
        .and_then(Value::as_str)
        .unwrap_or_default();
...
fn parse_http_backend_url(url: &str) -> Result<BackendUrl> {
    let rest = url.strip_prefix("http://").ok_or_else(|| {
        OmxApiError::Message(
            "OWX_API_PRIVATE_BACKEND_URL must use http:// in V1A; use a localhost TLS terminator for HTTPS backends".to_string(),
        )
    })?;
    ...
    if !is_loopback_host(&host) && env::var_os("OWX_API_ALLOW_UNSAFE_PRIVATE_BACKEND").is_none() {
        return Err(OmxApiError::Message(format!(
            "private backend host `{host}` is not loopback; set OWX_API_ALLOW_UNSAFE_PRIVATE_BACKEND=1 only for trusted development"
        )));
    }
...
fn resolve_client_target(state_file: PathBuf) -> Result<(String, u16, Option<String>)> {
    if let Some(url) = env::var("OWX_API_BASE_URL")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
        ...
        return Ok((
            parsed.host,
            parsed.port,
            env::var("OWX_API_LOCAL_BEARER").ok(),
        ));
    }
...
fn spawn_daemon(config: &ServerConfig) -> Result<DaemonState> {
    ...
    let mut child = Command::new(exe)
        .args(args)
        .env("OWX_API_LOCAL_BEARER", &token)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()?;
    ...
    match args.first().map(String::as_str) {
        Some("dry-run") => writeln!(
            ...
            serde_json::to_string_pretty(&json!({
                ...
                "would_start": "owx-api serve --backend mock"
            }))?
        )?,
...
fn run_generate<W: Write>(args: &[String], mut stdout: W) -> Result<()> {
    ...
    let (path, body) = match kind {
        "text" => (
            "/v1/responses",
            json!({"model": env::var("OWX_API_GENERATE_MODEL").unwrap_or_else(|_| "owx-private".to_string()), "input": prompt}),
        ),
...
fn run_smoke<W: Write>(args: &[String], mut stdout: W) -> Result<()> {
    match args.first().map(String::as_str) {
        Some("text") => {
            if env::var_os("OWX_API_LIVE_SMOKE").is_none() {
                return Err(OmxApiError::Message(
                    "set OWX_API_LIVE_SMOKE=1 to run the real-private text smoke".to_string(),
                ));
            }
            let body = json!({"model": "owx-private", "input": "Say OWX API smoke OK."});
        ...
        _ => Err(OmxApiError::Message(
            "smoke requires text (example: OWX_API_LIVE_SMOKE=1 owx-api smoke text)".to_string(),
        )),
    }
}
...
fn parse_server_config(args: &[String]) -> Result<ServerConfig> {
    let mut config = ServerConfig {
        local_bearer_token: env::var("OWX_API_LOCAL_BEARER")
            .ok()
            .filter(|value| !value.trim().is_empty()),
        ..ServerConfig::default()
    };
    if let Some(backend) = env::var("OWX_API_BACKEND")
        .ok()
        .filter(|value| !value.trim().is_empty())
    {
...
fn validate_loopback_host(host: &str) -> Result<()> {
    if is_loopback_host(host) {
        ...
        Err(OmxApiError::Message(format!(
            "owx-api is localhost-only; refusing non-loopback host `{host}`"
        )))
    }
}
...
fn help_text() -> &'static str {
    "owx-api serve [--daemon] [--system --dry-run]|status|stop|generate text|generate image|smoke text\nNote: real-private backend mode is experimental and requires local bearer auth."
}
...
mod tests {
    ...
    fn routes_health_and_models() {
        ...
        assert!(String::from_utf8(models.body).unwrap().contains("owx-mock"));
    ...
    fn codex_native_request_matches_installed_codex_responses_wire_shape() {
        let _guard = env_lock();
        env::set_var("OWX_API_CODEX_SESSION_ID", "session-1");
        env::set_var("OWX_API_CODEX_THREAD_ID", "thread-1");
        env::set_var("OWX_API_CODEX_INSTALLATION_ID", "install-1");
        env::set_var("OWX_API_CODEX_WINDOW_ID", "window-1");
        env::set_var("OWX_API_CODEX_USER_AGENT", "codex_cli_rs/test");
        ...
        env::remove_var("OWX_API_CODEX_SESSION_ID");
        env::remove_var("OWX_API_CODEX_THREAD_ID");
        env::remove_var("OWX_API_CODEX_INSTALLATION_ID");
        env::remove_var("OWX_API_CODEX_WINDOW_ID");
        env::remove_var("OWX_API_CODEX_USER_AGENT");
    }

    #[test]
    fn real_private_posts_codex_native_request_and_parses_sse_text() {
        let _guard = env_lock();
        env::remove_var("OWX_API_REAL_PRIVATE_RESPONSE_TEXT");
        env::set_var("OWX_API_CODEX_OAUTH_TOKEN", "oauth-token");
        env::set_var("OWX_API_CODEX_ACCOUNT_ID", "account-1");
        env::set_var("OWX_API_CODEX_THREAD_ID", "thread-1");
        env::set_var("OWX_API_CODEX_SESSION_ID", "session-1");
        ...
        env::remove_var("OWX_API_CODEX_OAUTH_TOKEN");
        env::remove_var("OWX_API_CODEX_ACCOUNT_ID");
        env::remove_var("OWX_API_CODEX_THREAD_ID");
        env::remove_var("OWX_API_CODEX_SESSION_ID");
        env::remove_var("OWX_API_PRIVATE_BACKEND_URL");
    }
    ...
    fn real_private_image_falls_back_to_responses_image_tool() {
        let _guard = env_lock();
        env::remove_var("OWX_API_REAL_PRIVATE_IMAGE_B64_JSON");
        env::remove_var("OWX_API_REAL_PRIVATE_IMAGE_URL");
        env::remove_var("OWX_API_PRIVATE_IMAGE_BACKEND_URL");
        env::set_var("OWX_API_CODEX_OAUTH_TOKEN", "oauth-token");
    ...
    fn chat_stream_response_uses_chat_chunk_shape() {
        ...
        assert!(body.contains("\"delta\":{\"content\":\"owx mock response\"}"));
        assert!(body.contains("data: [DONE]"));
    }
    ...
    fn real_private_chat_stream_preserves_backend_errors() {
        ...
        env::set_var(
            "CODEX_HOME",
            env::temp_dir().join("owx-api-test-missing-auth"),
        );
        let telemetry = Telemetry::default();
        let response = route_request(
            &request(
    ...
    fn live_text_smoke_fails_when_private_backend_returns_error() {
        let _guard = env_lock();
        env::set_var("OWX_API_LIVE_SMOKE", "1");
        env::remove_var("OWX_API_REAL_PRIVATE_RESPONSE_TEXT");
        env::set_var("OWX_API_CODEX_OAUTH_TOKEN", "oauth-token");
        env::remove_var("OWX_API_PRIVATE_BACKEND_URL");
        ...
        env::remove_var("OWX_API_LIVE_SMOKE");
        env::remove_var("OWX_API_CODEX_OAUTH_TOKEN");
    }
    ...
    fn live_text_smoke_succeeds_when_response_fixture_set() {
        ...
        let mut stdout = Vec::new();
        env::set_var("OWX_API_LIVE_SMOKE", "1");
        env::set_var("OWX_API_REAL_PRIVATE_RESPONSE_TEXT", "ok-fixture-smoke");
    ...
    fn daemon_state_round_trips() {
        let path = env::temp_dir().join(format!("owx-api-test-{}.json", now_unix()));
        let state = DaemonState {
            pid: 42,
            host: "127.0.0.1".to_string(),
            port: 9999,
    ...
    fn daemon_startup_timeout_kills_child_process_to_avoid_process_leak() {
        let path = env::temp_dir().join(format!("owx-api-timeout-test-{}.json", now_unix()));
