use owx_api::{http_request, http_request_with_bearer, read_daemon_state};
...
fn temp_state_file(name: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "owx-api-{name}-{}-{nanos}-{counter}.json",
        std::process::id()
    ))
}
...
fn real_private_once_request(
    ...
    let bin = env!("CARGO_BIN_EXE_owx-api");
    ...
    let mut child = Command::new(bin)
        ...
        .env("OWX_API_CODEX_OAUTH_TOKEN", "oauth-token-for-e2e")
        .env("OWX_API_CODEX_ACCOUNT_ID", "account-e2e")
        .env("OWX_API_CODEX_SESSION_ID", "session-e2e")
        .env("OWX_API_CODEX_THREAD_ID", "thread-e2e")
        .env(
            "OWX_API_PRIVATE_BACKEND_URL",
            format!("http://127.0.0.1:{backend_port}/backend-api/codex"),
        )
        .env(
            "OWX_API_PRIVATE_IMAGE_BACKEND_URL",
            format!("http://127.0.0.1:{backend_port}/backend-api/codex"),
        )
        .env("OWX_API_IMAGE_MODEL", "owx-private-image-test")
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn real-private owx-api serve");
...
fn binary_system_dry_run_and_generate_emit_json() {
    ...
    for action in ["dry-run", "generate"] {
        let output = Command::new(bin)
            .args(["system", action])
            .output()
            .expect("run owx-api system action");
        assert!(
...
fn binary_real_private_image_json_uses_fake_upstream_e2e() {
    ...
    assert_eq!(forwarded_json["model"], "owx-private-image-test");
...
fn binary_local_bearer_gate_rejects_missing_authorization() {
    ...
    let mut child = Command::new(bin)
        ...
        .env("OWX_API_REQUIRE_LOCAL_BEARER", "1")
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn owx-api serve");
...
fn binary_real_private_serve_generates_bearer_and_rejects_missing_authorization() {
    ...
    let mut child = Command::new(bin)
        ...
        .env("OWX_API_REAL_PRIVATE_RESPONSE_TEXT", "fixture")
        .stdout(Stdio::null())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn owx-api real-private serve");
...
fn binary_daemon_token_is_not_printed_but_controls_generate_and_stop() {
    ...
    assert!(String::from_utf8_lossy(&generated.stdout).contains("owx mock response"));
