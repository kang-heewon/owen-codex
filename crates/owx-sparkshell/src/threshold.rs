...
mod tests {
    ...
    fn threshold_defaults_for_zero_invalid_and_blank_values() {
        let _guard = env_lock();
        unsafe { env::set_var("OWX_SPARKSHELL_LINES", "0") };
        assert_eq!(read_line_threshold(), DEFAULT_MAX_VISIBLE_LINES);

        unsafe { env::set_var("OWX_SPARKSHELL_LINES", "not-a-number") };
        assert_eq!(read_line_threshold(), DEFAULT_MAX_VISIBLE_LINES);

        unsafe { env::set_var("OWX_SPARKSHELL_LINES", "   ") };
        assert_eq!(read_line_threshold(), DEFAULT_MAX_VISIBLE_LINES);

        unsafe { env::remove_var("OWX_SPARKSHELL_LINES") };
    }

    #[test]
    fn threshold_accepts_trimmed_positive_values() {
        let _guard = env_lock();
        unsafe { env::set_var("OWX_SPARKSHELL_LINES", " 7 ") };
