...
pub fn resolve_model() -> String {
    env::var("OWX_SPARKSHELL_MODEL")
        ...
        .or_else(|| {
            env::var("OWX_DEFAULT_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
        .or_else(|| {
            env::var("OWX_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn resolve_fallback_model() -> String {
    env::var("OWX_SPARKSHELL_FALLBACK_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("OWX_DEFAULT_STANDARD_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn resolve_instructions_file() -> Option<String> {
    env::var("OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE")
        .ok()
...
pub fn read_summary_timeout_ms() -> u64 {
    env::var("OWX_SPARKSHELL_SUMMARY_TIMEOUT_MS")
        .ok()
...
fn resolve_api_base_url() -> String {
    env::var("OWX_API_BASE_URL")
        .ok()
        .map(|value| value.trim().trim_end_matches('/').to_string())
        .filter(|value| !value.is_empty())
        .or_else(|| {
            env::var("OWX_API_PORT")
                .ok()
                .map(|value| value.trim().to_string())
                .filter(|value| !value.is_empty())
...
fn resolve_api_bearer() -> Option<String> {
    env::var("OWX_API_LOCAL_BEARER")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        .or_else(|| {
            let path = env::var("OWX_API_STATE_FILE")
                .ok()
                .filter(|value| !value.trim().is_empty())
                .unwrap_or_else(|| {
                    env::temp_dir()
                        .join("owx-api-daemon.json")
                        .display()
                        .to_string()
                });
...
fn parse_http_url(url: &str) -> Result<HttpUrl, SparkshellError> {
    ...
    if !is_loopback_host(&host) && std::env::var_os("OWX_API_ALLOW_UNSAFE_BASE_URL").is_none() {
        return Err(SparkshellError::SummaryBridge(format!(
            "local API URL host `{host}` is not loopback; set OWX_API_ALLOW_UNSAFE_BASE_URL=1 only for trusted development"
        )));
    }
    Ok(HttpUrl {
        host,
...
mod tests {
    ...
    fn model_resolution_prefers_sparkshell_override() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OWX_SPARKSHELL_MODEL", "spark-a");
            env::set_var("OWX_DEFAULT_SPARK_MODEL", "spark-b");
        }
        assert_eq!(resolve_model(), "spark-a");
        unsafe {
            env::remove_var("OWX_SPARKSHELL_MODEL");
            env::remove_var("OWX_DEFAULT_SPARK_MODEL");
            env::remove_var("OWX_SPARK_MODEL");
        }
    }
    ...
    fn fallback_model_resolution_prefers_override_then_default_standard() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OWX_SPARKSHELL_FALLBACK_MODEL");
            env::remove_var("OWX_DEFAULT_STANDARD_MODEL");
        }
        assert_eq!(resolve_fallback_model(), DEFAULT_STANDARD_MODEL);

        unsafe {
            env::set_var("OWX_DEFAULT_STANDARD_MODEL", "standard-a");
        }
        assert_eq!(resolve_fallback_model(), "standard-a");

        unsafe {
            env::set_var("OWX_SPARKSHELL_FALLBACK_MODEL", "standard-b");
        }
    ...
    fn timeout_defaults_when_unset() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OWX_SPARKSHELL_SUMMARY_TIMEOUT_MS");
        }
    ...
    fn model_resolution_ignores_blank_override_and_uses_secondary_env() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OWX_SPARKSHELL_MODEL", "   ");
            env::set_var("OWX_DEFAULT_SPARK_MODEL", "spark-b");
        }
    ...
    fn instructions_file_resolution_prefers_override_and_ignores_blank() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE");
        }
        assert_eq!(resolve_instructions_file(), None);

        unsafe {
            env::set_var(
                "OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE",
        ...
        unsafe {
            env::set_var("OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE", "   ");
        }
        assert_eq!(resolve_instructions_file(), None);
    ...
    fn timeout_defaults_for_zero_and_invalid_values() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OWX_SPARKSHELL_SUMMARY_TIMEOUT_MS", "0");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);

        unsafe {
            env::set_var("OWX_SPARKSHELL_SUMMARY_TIMEOUT_MS", "bogus");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);
...
notes: nope"
    ...
    fn api_base_url_parser_rejects_non_loopback_hosts_by_default() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OWX_API_ALLOW_UNSAFE_BASE_URL");
        }
