...
fn truncate_for_prompt(text: &str, label: &str) -> String {
    let max_lines = read_positive_usize_env(
        "OWX_SPARKSHELL_SUMMARY_MAX_LINES",
        DEFAULT_SUMMARY_MAX_LINES,
    );
    let max_bytes = read_positive_usize_env(
        "OWX_SPARKSHELL_SUMMARY_MAX_BYTES",
...
mod tests {
    ...
    fn prompt_truncates_large_streams_before_embedding_them() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OWX_SPARKSHELL_SUMMARY_MAX_LINES", "4");
            env::set_var("OWX_SPARKSHELL_SUMMARY_MAX_BYTES", "120");
        }
        ...
        unsafe {
            env::remove_var("OWX_SPARKSHELL_SUMMARY_MAX_LINES");
            env::remove_var("OWX_SPARKSHELL_SUMMARY_MAX_BYTES");
        }
