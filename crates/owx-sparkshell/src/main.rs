...
use owx_mux::build_capture_pane_args;
...
const CACHE_BODY_VERSION: &str = "owx-sparkshell-cache-v2";
...
fn usage_text() -> String {
    format!(
        concat!(
            ...
            "   or: owx-sparkshell --tmux-pane <pane-id> [--tail-lines <{min}-{max}>]\n",
            "\n",
...
fn cache_dir() -> PathBuf {
    std::env::var("OWX_SPARKSHELL_CACHE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            std::env::var("OWX_TEAM_STATE_ROOT")
                .map(|root| PathBuf::from(root).join("../cache/sparkshell"))
                .unwrap_or_else(|_| PathBuf::from(".owx/cache/sparkshell"))
        })
}
...
fn classify_team(team: &str, worker: &str) -> Option<Diagnostics> {
    let state_root = std::env::var("OWX_TEAM_STATE_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(".owx/state"));
    let base = state_root
        .join("team")
        .join(team)
        .join("workers")
        ...
        if let Some(timestamp) = extract_heartbeat_ms(&heartbeat) {
            if now_ms().saturating_sub(timestamp) > STALE_HEARTBEAT_MS {
                return Some(Diagnostics {
                    ...
                    next_action: "run owx team status".to_string(),
                    confidence: 0.78,
                    errors: Vec::new(),
