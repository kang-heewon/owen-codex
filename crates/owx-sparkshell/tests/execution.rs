...
fn sparkshell_bin() -> &'static str {
    env!("CARGO_BIN_EXE_owx-sparkshell")
}
...
fn summary_mode_uses_local_api_and_model_override() {
    ...
    let output = Command::new(sparkshell_bin())
        .env("OWX_API_BASE_URL", base_url)
        .env("OWX_SPARKSHELL_LINES", "1")
        .env("OWX_SPARKSHELL_MODEL", "spark-test-model")
        .arg("sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'")
        .output()
...
fn summary_mode_retries_with_fallback_model_when_spark_is_unavailable() {
    ...
    let output = Command::new(sparkshell_bin())
        ...
        .env("OWX_SPARKSHELL_LINES", "1")
        .env("OWX_SPARKSHELL_MODEL", "spark-test-model")
        .env("OWX_SPARKSHELL_FALLBACK_MODEL", "frontier-test-model")
        .arg("sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'")
        .output()
...
fn summary_failure_when_api_is_missing_falls_back_to_raw_output() {
    ...
    let output = Command::new(sparkshell_bin())
        .env("OWX_API_BASE_URL", base_url)
        .env("OWX_SPARKSHELL_SUMMARY_TIMEOUT_MS", "500")
        .env("OWX_SPARKSHELL_LINES", "1")
        .arg("/bin/sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'; printf 'child-err\n' >&2")
...
fn team_diagnostics_reads_last_turn_at_heartbeat() {
    ...
    let output = Command::new(sparkshell_bin())
        .env("OWX_TEAM_STATE_ROOT", temp.display().to_string())
        .arg("--json")
        .arg("--team")
        .arg("demo")
        .arg("--worker")
...
fn pane_json_cache_reports_hits_and_since_last_changes() {
    ...
    let first = Command::new(sparkshell_bin())
        .env("PATH", &path)
        .env("OWX_SPARKSHELL_CACHE_DIR", cache.display().to_string())
        .arg("--json")
        .arg("--tmux-pane")
        .arg("%31")
        .output()
...
",
    ...
    assert!(cached.contains("owx-sparkshell-cache-v2"));
