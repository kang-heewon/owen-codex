use owx_mux::{canonical_contract_summary, MuxAdapter, MuxOperation, MuxTarget, TmuxAdapter};
use owx_runtime_core::{runtime_contract_summary, RuntimeCommand, RuntimeEngine};
use std::env;
use std::process;

fn main() {
    if let Err(error) = run() {
        eprintln!("owx-runtime: {error}");
        process::exit(1);
    }
}
        ...
        Some("schema") => {
            if second == Some("--json") {
                let summary = serde_json::json!({
                    "schema_version": owx_runtime_core::RUNTIME_SCHEMA_VERSION,
                    "commands": owx_runtime_core::RUNTIME_COMMAND_NAMES,
                    "events": owx_runtime_core::RUNTIME_EVENT_NAMES,
                    "transport": "tmux",
                });
                println!(
                    "{}",
...
fn print_usage() {
    println!(concat!(
        "usage: owx-runtime <command> [options]\n",
        "\n",
        "commands:\n",
