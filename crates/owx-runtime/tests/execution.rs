...
fn schema_subcommand_prints_contract_summary() {
    let output = Command::new(env!("CARGO_BIN_EXE_owx-runtime"))
        .arg("schema")
        .output()
        .expect("ran owx-runtime");
...
fn init_creates_state_directory() {
    let dir = std::env::temp_dir().join("owx-runtime-test-init");
...
fn exec_with_state_dir_persists() {
    let dir = std::env::temp_dir().join("owx-runtime-test-exec-persist");
    let _ = std::fs::remove_dir_all(&dir);
...
fn snapshot_from_state_dir_reads_persisted_state() {
    let dir = std::env::temp_dir().join("owx-runtime-test-snapshot-statedir");
    let _ = std::fs::remove_dir_all(&dir);

    // First: init and exec to create state
    Command::new(env!("CARGO_BIN_EXE_owx-runtime"))
        .args(["init", dir.to_str().unwrap()])
        .output()
        .expect("init");
