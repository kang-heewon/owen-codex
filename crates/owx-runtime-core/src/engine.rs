...
mod tests {
    ...
    fn persist_and_load_round_trip() {
        let dir = std::env::temp_dir().join("owx-runtime-test-persist");
    ...
    fn persist_and_load_round_trip_preserves_mailbox_body() {
        let dir = std::env::temp_dir().join("owx-runtime-test-mailbox-body");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn load_backfills_legacy_mailbox_event_body_from_mailbox_json() {
        let dir = std::env::temp_dir().join("owx-runtime-test-mailbox-backfill");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn compatibility_view_writes_section_files() {
        let dir = std::env::temp_dir().join("owx-runtime-test-compat");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn queue_dispatch_with_metadata_persists_and_round_trips() {
        let dir = std::env::temp_dir().join("owx-runtime-test-metadata");
        let _ = std::fs::remove_dir_all(&dir);
