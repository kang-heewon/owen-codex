...
- **build-from-scratch**: new feature, new module, or new service with no prior implementation. → Question family axes: **exit criteria**, **test strategy**, **scope boundary**, **dependency choice**, and **handoff target** (`$ultragoal` / direct execution with optional native Codex subagents). **STRONGLY PREFERS `<research_fan_out>`** before the first round.
...
- **collaboration**: multi-owner work touching shared surfaces or native Codex subagent lanes. → Question family axes: **ownership split**, **shared-file conflict resolution**, **handoff criteria**, **communication cadence**.
...
<ask_gate>
- **Batch all independent high-leverage questions for the current round into one native structured-input request** when available. Reserve one-at-a-time only for dependent question chains.
- If a safe assumption is available, state it and continue instead of blocking.
- Use native structured input when available; otherwise list a numbered prose block (`Q1: ... Q2: ...`) and wait for one complete reply.
...
- **Run multiple interview rounds** until the 6-item checklist is satisfied. After both gap-fill passes, ask a focused native structured-input batch for only the CRITICAL unresolved items, or use the prose fallback.
...
11. Batch the surviving independent questions through native structured input, or a numbered prose block fallback; wait for all answers.
