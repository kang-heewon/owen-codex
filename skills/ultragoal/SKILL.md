...
`ultragoal` turns a brief into repo-native artifacts and then drives a Codex goal safely through goal tools. New plans default to a stable pointer-style aggregate Codex goal for the whole durable plan in `.owx/ultragoal/goals.json`, including later accepted/appended stories under the original brief constraints, while OWX tracks G001/G002 story progress in the ledger. Ultragoal does not call Codex `/goal clear`; before multiple sequential ultragoal runs in one Codex session/thread, manually run `/goal clear` in the Codex UI so the previous completed aggregate goal does not block or confuse the next `create_goal`.

- `.owx/ultragoal/brief.md`
- `.owx/ultragoal/goals.json`
- `.owx/ultragoal/ledger.jsonl` (checkpoint and structured steering audit events)
...
1. Run one of:
   - `owx ultragoal create-goals --brief "<brief>"`
   - `owx ultragoal create-goals --brief-file <path>`
   - `cat <brief> | owx ultragoal create-goals --from-stdin`
   - `owx ultragoal create-goals --codex-goal-mode per-story --brief "<brief>"` only when one Codex goal context per story is explicitly preferred
2. Inspect `.owx/ultragoal/goals.json` and refine if needed.

## Complete goals

Loop until `owx ultragoal status` reports all goals complete:

1. Run `owx ultragoal complete-goals`.
2. Read the printed handoff.
3. Call `get_goal`.
4. If no active Codex goal exists, call `create_goal` with the printed payload. In aggregate mode, if the same aggregate Codex objective is already active, continue the current OWX story without creating a new Codex goal.
5. Complete the current OWX story only.
6. Run a completion audit against the story objective and real artifacts/tests. For the final story, defer this audit to Step 6 of the mandatory final cleanup and review gate so it evaluates the approved post-clean diff.
...
8. Checkpoint the durable ledger with that snapshot. Intermediate aggregate checkpoints use only `--codex-goal-json`; final clean checkpoints also require `--quality-gate-json`:
   `owx ultragoal checkpoint --goal-id <id> --status complete --evidence "<evidence>" --codex-goal-json <get_goal-json-or-path> [--quality-gate-json <quality-gate-json-or-path>]`
9. If blocked or failed, checkpoint failure:
   `owx ultragoal checkpoint --goal-id <id> --status failed --evidence "<blocker/evidence>"`
10. For legacy per-story completed-goal blockers, preserve the non-terminal blocker with:
   `owx ultragoal checkpoint --goal-id <id> --status blocked --evidence "<completed legacy Codex goal blocks create_goal in this thread>" --codex-goal-json <get_goal-json-or-path>`
11. Resume failed goals with `owx ultragoal complete-goals --retry-failed`.

## Dynamic steering

Use `owx ultragoal steer` when real findings or blockers prove the current story decomposition should change while the aggregate objective and constraints stay fixed. Steering is explicit-only and evidence-backed; broad natural-language requests are rejected instead of guessed.
...
```sh
owx ultragoal steer --kind add_subgoal --title "Investigate blocker" --objective "Validate the blocker and report evidence." --evidence "log/test output" --rationale "The blocker changes the safe execution order." --json
owx ultragoal steer --directive-json ./steering.json --json
```

Steering invariants:

- Do not edit the aggregate Codex objective, original brief constraints, quality gates, or completion status. The aggregate objective is a stable pointer to `.owx/ultragoal/goals.json` and `.owx/ultragoal/ledger.jsonl`, not an enumeration of initial goal ids.
- Do not hard-delete goals, auto-complete work, weaken verification, or silently mutate `.owx/ultragoal`.
- Accepted and rejected attempts append structured audit entries to `.owx/ultragoal/ledger.jsonl`.
...
The final ultragoal story is not complete until the active agent has run the final quality gate for one stable final candidate:

1. Run fresh targeted verification for the story's stable final candidate.
2. Run `ai-slop-cleaner` exactly once for that candidate, on changed files only; if there are no relevant edits, the cleaner still runs and records a passed/no-op report.
3. Rerun fresh verification after the cleaner pass. If the cleaner causes a regression, repair or revert the cleaner change and rerun verification; do not blindly loop the cleaner solely because its changes failed verification.
4. Run `$code-review` through the independent review path on the exact post-clean diff proven by Step 3. Clean means `codeReview.recommendation: "APPROVE"`, `codeReview.architectStatus: "CLEAR"`, and `codeReview.independentReview` contains distinct completed `code-reviewer` and `architect` subagent evidence. `COMMENT`, `WATCH`, `REQUEST CHANGES`, `BLOCK`, missing subagent evidence, unavailable delegation, and same-lane/self-review are non-clean. Any source change after approval makes the review stale.
