...
<Purpose>
Deep Interview is an intent-first Socratic clarification loop before planning or implementation. It turns vague ideas into execution-ready specifications by asking targeted questions about why the user wants a change, how far it should go, what should stay out of scope, and what OWX may decide without confirmation.
</Purpose>
...
- You need a requirements artifact before handing off to `ralplan`, `autopilot`, `ralph`, or native Codex subagents
</Use_When>
...
- **Autoresearch (`--autoresearch`)**: same interview rigor as Standard, but specialized for `$autoresearch` mission readiness and `.owx/specs/` artifact handoff
...
<Mode_Flags>
- **`--autoresearch`**: switch the interview into autoresearch-intake mode for `$autoresearch` handoff. In this mode, the interview should converge on a validator-ready research mission, write canonical artifacts under `.owx/specs/`, and preserve the explicit `refine further` vs `launch` boundary for downstream skill intake.
</Mode_Flags>
</Depth_Profiles>
...
- `owx explore` is deprecated. Use normal repository inspection tools/subagents for simple read-only brownfield fact gathering; use `owx sparkshell` only for explicit shell-native read-only evidence, and keep ambiguous or non-shell-only investigation on the richer normal path.
- Always run a preflight context intake before the first interview question
- For brownfield work, preflight must include context grounding before user-facing questions: inspect applicable `AGENTS.md` files, existing `.owx/context/` snapshots, `.owx/specs/` artifacts, and any project-local glossary/context files such as `CONTEXT.md` or `CONTEXT-MAP.md` when present.
...
- Cross-check user claims about current behavior against code and local context artifacts when discoverable. If sources disagree, ask a confirmation question that names both sources instead of silently choosing one.
...
- Durable glossary, ADR, or memory updates are opt-in and public-safe only. Deep-interview may recommend such updates in the handoff summary, but must not automatically create or dump public artifacts from interview transcripts unless the user explicitly chooses that as in-scope.
...
- Use these transcript/spec labels only; do not expose them as user-facing question metadata:
...
- Treat `[from-code][auto-confirmed]` and other non-user fact discoveries as context/transcript updates, not interview rounds: do not create a pending question obligation or increment the user-facing round number for facts the agent can safely establish.
...
- Use the native structured user-input tool for each interview round when available.
- Otherwise ask exactly one concise plain-text question and wait for the answer.
- After every answer, update the transcript before scoring ambiguity, asking another round, or handing off.
...
- Persist mode state for resume safety with CLI-first state commands (`owx state write/read --input '<json>' --json`); use `state_write` / `state_read` only when explicit MCP compatibility is enabled
</Execution_Policy>
...
2. Attempt to load the latest relevant context snapshot from `.owx/context/{slug}-*.md`.
...
4. If no snapshot exists, create a minimum context snapshot with:
   ...
   - Relevant repo rules/context inspected
...
5. For brownfield tasks, inspect the applicable documentation/rule surface before the first user-facing round. Prefer exact, nearby sources over broad scans:
   ...
   - existing `.owx/context/` snapshots, `.owx/specs/`, and planning artifacts relevant to the slug
   - project-local glossary/context files such as `CONTEXT.md`, `CONTEXT-MAP.md`, or context-specific artifacts when they exist
6. Save snapshot to `.owx/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) and reference it in mode state.
...
3. Initialize state via `owx state write --input '{"mode":"deep-interview","active":true}' --json`:
...
```json
{
  ...
  "state": {
    ...
    "context_snapshot_path": ".owx/context/<slug>-<timestamp>.md"
  }
}
```
...
If the initial context is oversized and no prompt-safe summary has been recorded yet, the next question must be only a summary request. Do not score ambiguity, run readiness gates, or hand off downstream until that summary answer is captured.
...
Maintain a **Terminology Ledger** for brownfield interviews:
- repo rules/context sources inspected, with path references
- canonical terms already used by the repo and terms to avoid or disambiguate
- user terms that conflict with current code behavior or local context
- source mismatches that require a human decision before implementation
- optional durable follow-ups that are safe to propose but not auto-apply
...
### 2b) Ask the question
Use native structured user input for every interview round when available; otherwise ask exactly one concise plain-text question and wait for the answer. Present:
...
- Deep-interview is Socratic: ask one focused round at a time. Do not combine multiple interview rounds into a batch.
...
### 2e) Persist state
Append round result and updated scores via `owx state write --input '<json>' --json`; use `state_write` only when explicit MCP compatibility is enabled.
...
- **Terminologist** (brownfield, whenever a key term is fuzzy, overloaded, or conflicts with repo context/code): force a canonical meaning against existing project language before implementation
...
1. Write interview transcript summary to:
   - `.owx/interviews/{slug}-{timestamp}.md`  
     (kept for ralph PRD compatibility)
2. Write execution-ready spec to:
   - `.owx/specs/deep-interview-{slug}.md`

Spec should include:
- Metadata (profile, rounds, final ambiguity, threshold, context type)
- Context snapshot reference/path (for planning and native-subagent reuse)
...
- Decision Boundaries (what OWX may decide without confirmation)
- Constraints
...
- Terminology Ledger with inspected repo rules/context, term conflicts, and any source mismatch decisions
...
- **Canonical artifact path:** `.owx/specs/deep-interview-autoresearch-{slug}.md`
- **Launch artifact bundle:** `.owx/specs/autoresearch-{slug}/mission.md`, `.owx/specs/autoresearch-{slug}/sandbox.md`, and `.owx/specs/autoresearch-{slug}/result.json`
- **Launch artifact directory:** `.owx/specs/autoresearch-{slug}/`
- **Required artifact sections:**
  ...
  - `Seed Inputs`
  - `Confirmation Bridge`
- **Required launch artifacts under `.owx/specs/autoresearch-{slug}/`:**
  - `mission.md`
  - `sandbox.md`
  - `result.json`
...
- **Confirmation bridge:** after artifact generation, offer at least `refine further` and `launch`; do not launch another workflow without explicit confirmation
...
Present execution options after artifact generation using explicit handoff contracts. Treat the deep-interview spec as the current requirements source of truth and preserve intent, non-goals, decision boundaries, acceptance criteria, terminology grounding, and any residual-risk warnings across the handoff.
...
Include these product-facing suggestions when they fit the clarified spec, alongside `$ultragoal`, `$ralplan`, `$autopilot`, and explicit `$ralph` fallback options:

- **`$ultragoal`** — default goal-mode follow-up for implementation or general goal-oriented follow-up specs that should be converted into durable Codex/OWX goals with sequential completion tracking.
...
Recommend `$ultragoal` as the default durable goal-mode follow-up. Use native Codex subagents with explicit `agent_type` and bounded ownership for independent parallel implementation, and keep `$ralph` only as an explicit fallback.

### 1. **`$ultragoal` (Default durable execution follow-up)**
- **Input Artifact:** `.owx/specs/deep-interview-{slug}.md` (optionally accompanied by the transcript/context snapshot for traceability)
- **Invocation:** `$ultragoal create-goals --brief-file <spec-path>` followed by `$ultragoal complete-goals` in the active execution lane
- **Consumer Behavior:** Convert the clarified spec into durable goal-mode work. Preserve intent, non-goals, decision boundaries, acceptance criteria, terminology grounding, scenario-pressure findings, and residual-risk warnings as binding story constraints.
...
- **Expected Output:** `.owx/ultragoal/brief.md`, `.owx/ultragoal/goals.json`, `.owx/ultragoal/ledger.jsonl`, implementation evidence, verification evidence, and final cleanup/review-gate evidence
- **Best When:** The clarified spec is execution-ready or the user explicitly wants durable goal tracking as the next step
- **Next Recommended Step:** Run the Ultragoal completion loop; assign independent lanes to native Codex subagents when warranted, and use `$ralph` only as an explicit fallback
...
- **Expected Output:** Canonical planning artifacts under `.owx/plans/`, especially `prd-*.md` and `test-spec-*.md`
...
- **Next Recommended Step:** Use the approved planning artifacts with `$ultragoal` as the default durable follow-up and native Codex subagents for independent lanes; choose a specialized goal workflow only when its evaluator fits

### 3. **`$autopilot`**
- **Input Artifact:** `.owx/specs/deep-interview-{slug}.md`
...
- **Next Recommended Step:** Continue through autopilot's execution/QA/validation flow; use native Codex subagents under leader-owned Ultragoal checkpointing for independent lanes
...
- **Next Recommended Step:** If this explicit fallback is selected, continue Ralph's persistence loop; if work expands into independent lanes, use native Codex subagents under `$ultragoal` checkpointing
...
- Save transcript/spec artifacts under `.owx/interviews/` and `.owx/specs/`
</Tool_Usage>
...
<Final_Checklist>
- [ ] Preflight context snapshot exists under `.owx/context/{slug}-{timestamp}.md`
...
- [ ] Transcript written to `.owx/interviews/{slug}-{timestamp}.md`
- [ ] Spec written to `.owx/specs/deep-interview-{slug}.md`
- [ ] Brownfield questions use evidence-backed confirmation when applicable
- [ ] Brownfield preflight inspected applicable repo rules/context before user-facing questions
...
- [ ] Durable ADR/memory updates, if any, were explicitly opted into and public-safe
- [ ] Handoff options provided (`$ultragoal`, `$ralplan`, `$autopilot`, explicit `$ralph` fallback) plus context-sensitive goal-mode suggestions when applicable
...
1. Repository-local `.owx/config.toml`
2. Repository-root `owx.toml`
3. User-global `~/.owx/config.toml`

This section is currently a deep-interview-specific runtime override surface, not a general replacement for Codex `config.toml` or `.owx-config.json` model/env routing.
...
If interrupted, rerun `$deep-interview`. Resume from persisted mode state via `owx state read --input '{"mode":"deep-interview"}' --json`.
