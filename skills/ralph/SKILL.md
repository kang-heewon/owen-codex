...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.owx/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. `owx explore` is deprecated; use normal repository inspection tools/subagents for simple read-only repository lookups and `owx sparkshell` only for explicit shell-native read-only evidence. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5.5 **Frontend evidence gate (when implementation affects rendered UI, even without a reference image)**:
   ...
   - Treat changes to interaction sequence, navigation, state transitions, validation or recovery, loading feedback, focus or keyboard behavior, gestures, drag/drop, or animation as UX-visible. For every UX-visible change, capture a short screen recording of the starting state, changed interaction, and outcome for every materially changed success, failure, or recovery path. Screenshots do not replace the required recording.
   - Embed or link the final screenshots and required recordings in the completion response with absolute paths and route/state/viewport labels; label each recording with its starting state, action, and outcome. Build, lint, typecheck, and unit tests do not replace browser evidence.
   - When creating or updating a pull request, add a `## Visual evidence` section to the PR body. Label every item with its route, state, and viewport, and replace local absolute paths with GitHub-hosted attachment URLs that render in the PR. Upload the attachments through the GitHub UI in a real browser, inspect them first for secrets, personal data, and other sensitive information, and confirm they render after the PR body is saved. Final screenshots are the default evidence. Every UX-visible change requires its screen recording of the starting state, changed interaction, and outcome; a recording is optional only for a purely visual change with no interaction, motion, or temporal behavior change. Prefer H.264 MP4. If upload or rendering fails, the PR handoff and merge-ready gates remain incomplete. When there is no PR, keep labeled absolute local paths for screenshots and required recordings in the completion response.
   - If browser inspection, screenshot capture, or a required recording cannot run, report the proof gap and do not declare the frontend or UX work complete.
...
- Use `owx state write/read --input '<json>' --json` for ralph mode state persistence between iterations
...
- Prefer CLI state commands. If an explicit MCP compatibility `owx_state` call reports that its stdio transport is unavailable/closed, do **not** retry the same MCP call. Retry once through the supported CLI parity surface with the same payload, preserving `workingDirectory` and `session_id`: `owx state write --input '<json>' --json`, `owx state read --input '<json>' --json`, or `owx state clear --input '<json>' --json`. If the CLI path also fails, continue with `.owx/context` / `.owx/plans` file-backed artifacts and report the state persistence blocker.
</Tool_Usage>
...
Use the CLI-first state surface for Ralph lifecycle state (`owx state write/read/clear --input '<json>' --json`). Explicit MCP compatibility tools (`state_write`, `state_read`, `state_clear`) remain acceptable only when already enabled.

- **On start**:
  `owx state write --input '{"mode":"ralph","active":true,"iteration":1,"max_iterations":10,"current_phase":"executing","started_at":"<now>","state":{"context_snapshot_path":"<snapshot-path>"}}' --json`
- **On each iteration**:
...
- **Before the final answer**:
  ...
  5. Read the state back with `owx state read --input '{"mode":"ralph"}' --json` and verify `completion_audit.passed === true`, a non-empty checklist, and non-empty verification evidence before producing the final answer.
  6. If Codex goal mode is active, call `update_goal({status:"complete"})` only after this Ralph audit read-back succeeds.
- **On cancellation/cleanup**:
  run `$cancel` (which should call `owx state clear --input '{"mode":"ralph"}' --json`)
...
Prompt-side `$ralph` workflow activation is lighter-weight than `owx ralph --prd ...`.
It seeds Ralph workflow state and guidance, but it does not implicitly launch the
CLI entrypoint or apply the PRD startup gate. Treat `owx ralph --prd ...` as the
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.owx/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.owx/plans/prd-{slug}.md`
   - Progress ledger: `.owx/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.owx/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.owx/plans/prd-{slug}.md`, create `.owx/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- During the compatibility window, Ralph `--prd` startup still validates machine-readable story state from `.owx/prd.json`.
- `.owx/plans/prd-{slug}.md` remains the canonical storage/documentation artifact, but it is not yet the startup validation source.
- If `.owx/prd.json` exists and canonical PRD is absent, migrate one-way into `.owx/plans/prd-{slug}.md`.
- If `.owx/progress.txt` exists and canonical progress ledger is absent, import one-way into `.owx/state/{scope}/ralph-progress.json`.
