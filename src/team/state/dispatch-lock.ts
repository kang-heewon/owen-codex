...
const OWX_DISPATCH_LOCK_TIMEOUT_ENV = 'OWX_DISPATCH_LOCK_TIMEOUT_MS';
...
export function resolveDispatchLockTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env[OWX_DISPATCH_LOCK_TIMEOUT_ENV];
    ...
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          ...
          `Set ${OWX_DISPATCH_LOCK_TIMEOUT_ENV} to increase (current: ${timeoutMs}ms, max: ${MAX_DISPATCH_LOCK_TIMEOUT_MS}ms).`
        );
      }
