...
async function writePhase(cwd: string, teamName: string, currentPhase: string, updatedAt: string): Promise<void> {
  await writeFile(join(cwd, '.owx', 'state', 'team', teamName, 'phase.json'), JSON.stringify({
...
describe('team identity', () => {
  ...
  it('resolves display names from OWX_TEAM_STATE_ROOT when cwd has no local team state', async () => {
    const leaderCwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-leader-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-worker-'));
    try {
      await initTeamState('shared-demo-aaaaaaaa', 'task', 'executor', 1, leaderCwd, undefined, { OWX_SESSION_ID: 'session-shared' }, {
  ...
  it('prefers active display-name candidates over retained terminal states', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-active-'));
    try {
      await initTeamState('demo-active', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-active' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-terminal', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-terminal' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
  ...
  it('prefers active display-name candidates over an exact retained terminal directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-exact-terminal-'));
    try {
      await initTeamState('demo', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-terminal' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-aaaaaaaa', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-active' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
  ...
  it('uses current leader identity to break active display-name ties', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-current-'));
    try {
      await initTeamState('demo-aaaaaaaa', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-a' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-bbbbbbbb', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-b' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });

      assert.equal(resolveTeamNameForCurrentContext('demo', cwd, { OWX_SESSION_ID: 'session-b' }), 'demo-bbbbbbbb');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('uses current leader identity before latest retained terminal state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-terminal-current-'));
    try {
      await initTeamState('demo-old-current', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-current' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-new-other', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-other' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      ...
      assert.equal(resolveTeamNameForCurrentContext('demo', cwd, { OWX_SESSION_ID: 'session-current' }), 'demo-old-current');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('resolves the latest retained terminal display-name state only when unambiguous', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-latest-terminal-'));
    try {
      await initTeamState('demo-old', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-old' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      await initTeamState('demo-new', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-new' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
  ...
  it('sanitizes unsafe lookup input instead of returning raw path-like names', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-unsafe-'));
    try {
  ...
  it('resolves display names to the current session candidate and fails closed on ambiguity', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-identity-'));
    try {
      await initTeamState('demo-aaaaaaaa', 'task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-a' }, {
        display_name: 'demo', requested_name: 'demo', identity_source: 'env-session',
      });
      ...
      assert.equal(resolveTeamNameForCurrentContext('demo', cwd, { OWX_SESSION_ID: 'session-a' }), 'demo-aaaaaaaa');
