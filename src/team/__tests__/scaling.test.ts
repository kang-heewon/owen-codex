...
async function initCommittedGitRepo(cwd: string): Promise<void> {
  ...
  execFileSync('git', ['config', 'user.name', 'OWX Test'], { cwd, stdio: 'pipe' });
  execFileSync('git', ['config', 'user.email', 'owx@example.com'], { cwd, stdio: 'pipe' });
  execFileSync('git', ['add', '.'], { cwd, stdio: 'pipe' });
  execFileSync('git', ['commit', '-m', 'init'], { cwd, stdio: 'pipe' });
}

async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-worktree-repo-'));
...
function workerStartupScriptPath(cwd: string, teamName: string, workerName: string): string {
  return join(cwd, '.owx', 'state', 'team', teamName, 'runtime', `${workerName}-startup.sh`);
}
...
async function configureScaleUpTeamForDirectDispatch(teamName: string, cwd: string): Promise<void> {
  ...
  config.tmux_session = `owx-team-${teamName}`;
  config.leader_pane_id = '%11';
  config.workers[0]!.pane_id = '%21';
  await saveTeamConfig(config, cwd);

  const manifestPath = join(cwd, '.owx', 'state', 'team', teamName, 'manifest.v2.json');
  if (!existsSync(manifestPath)) {
    await mkdir(join(cwd, '.owx', 'state', 'team', teamName), { recursive: true });
...
async function readScaleUpTaskPayloads(teamName: string, cwd: string): Promise<string[]> {
  const tasksDir = join(cwd, '.owx', 'state', 'team', teamName, 'tasks');
...
async function prepareScaleUpApprovedBindingState(
  ...
  if (state === 'malformed') {
    await writeFile(
      join(cwd, '.owx', 'state', 'team', teamName, 'approved-execution.json'),
      '{"prd_path":42}\n',
    );
    return;
  }
  ...
  if (state === 'ambiguous') {
    await writeFile(
      ...
      [
        ...
        `Launch via owx team 2:writer "${approvedTask}"`,
      ].join('\n'),
    );
    await writePersistedApprovedTeamExecutionBinding(teamName, cwd, {
      prd_path: prdPath,
  ...
  prdLines.push(`Launch via owx team 1:executor "${approvedTask}"`);
...
describe('isScalingEnabled', () => {
  ...
  it('returns false when env var is empty string', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: '' }), false);
  });

  it('returns false when env var is "0"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: '0' }), false);
  });

  it('returns false when env var is "false"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: 'false' }), false);
  });

  it('returns false when env var is "no"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: 'no' }), false);
  });

  it('returns true when env var is "1"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: '1' }), true);
  });

  it('returns true when env var is "true"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: 'true' }), true);
  });

  it('returns true when env var is "yes"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: 'yes' }), true);
  });

  it('returns true when env var is "on"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: 'on' }), true);
  });

  it('returns true when env var is "enabled"', () => {
    assert.equal(isScalingEnabled({ OWX_TEAM_SCALING_ENABLED: 'enabled' }), true);
  });
...
describe('WorkerStatus draining state', () => {
  it('writeWorkerStatus writes draining status and readWorkerStatus reads it back', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-drain-'));
    try {
  ...
  it('readWorkerStatus returns unknown for non-existent worker', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-nw-'));
    try {
...
describe('Monotonic worker index counter', () => {
  it('initTeamState sets next_worker_index to workerCount + 1', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-idx-'));
    try {
      ...
      const diskCfg = JSON.parse(
        readFileSync(join(cwd, '.owx', 'state', 'team', 'idx-test', 'config.json'), 'utf8'),
  ...
  it('next_worker_index is present in manifest.v2.json', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-manif-'));
    try {
      await initTeamState('manif-test', 'task', 'executor', 2, cwd);
      const manifest = JSON.parse(
        readFileSync(join(cwd, '.owx', 'state', 'team', 'manif-test', 'manifest.v2.json'), 'utf8'),
      ) as { next_worker_index?: number };
  ...
  it('readTeamConfig preserves next_worker_index', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-read-'));
    try {
...
describe('withScalingLock', () => {
  it('acquires and releases lock for successful operations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-lock-'));
    try {
      await initTeamState('lock-test', 'task', 'executor', 1, cwd);
      const lockDir = join(cwd, '.owx', 'state', 'team', 'lock-test', '.lock.scaling');
  ...
  it('releases lock even when function throws', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-lock-err-'));
    try {
      await initTeamState('lock-err', 'task', 'executor', 1, cwd);
      const lockDir = join(cwd, '.owx', 'state', 'team', 'lock-err', '.lock.scaling');
  ...
  it('serializes concurrent operations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-lock-con-'));
    try {
...
describe('scaleUp', () => {
  ...
  it('persists scaled-up task roles in canonical task state and inbox ids', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-role-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-role-bin-'));
    ...
    try {
      ...
      await mkdir(join(cwd, '.owx', 'state', 'team', 'scale-up-role'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'state', 'team', 'scale-up-role', 'worker-agents.md'), '# Base worker instructions\n');
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-scale-up-role';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'scale-up-role', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const workerIdentity = JSON.parse(await readFile(join(cwd, '.owx', 'state', 'team', 'scale-up-role', 'workers', 'worker-2', 'identity.json'), 'utf-8')) as { role?: string };
      assert.equal(workerIdentity.role, 'writer');

      const inbox = await readFile(join(cwd, '.owx', 'state', 'team', 'scale-up-role', 'workers', 'worker-2', 'inbox.md'), 'utf-8');
  ...
  it('injects persisted leader-owned Ultragoal context into scaled worker inboxes', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-ultragoal-context-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-ultragoal-context-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const previousPath = process.env.PATH;
    ...
    try {
      ...
      assert.match(inbox, /owx ultragoal checkpoint --goal-id G001-team-runtime-bridge/);
  ...
  it('keeps scale-up on the generic path when no approved binding is persisted', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-no-approved-binding-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-no-approved-binding-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const previousPath = process.env.PATH;
    ...
    try {
      ...
      const inbox = await readFile(
        join(cwd, '.owx', 'state', 'team', teamName, 'workers', 'worker-2', 'inbox.md'),
        'utf-8',
      );
      const tmuxCommands = await readScaleUpTmuxLogCommands(tmuxLogPath);
  ...
  it('injects approved handoff context on scale-up when the persisted binding is baseline-ready without context-pack metadata', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-plan-only-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-plan-only-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const previousPath = process.env.PATH;
  ...
  it('injects approved handoff context into scaled worker inboxes when the persisted binding stays ready', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-approved-context-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-approved-context-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const previousPath = process.env.PATH;
  ...
  it('proves the approved-binding scale-up model across generated state/count scenarios, including forbidden counterfactuals', async () => {
    for (const state of SCALE_UP_APPROVED_BINDING_STATES) {
      for (const count of SCALE_UP_COUNTS) {
        ...
        const cwd = await mkdtemp(join(tmpdir(), `owx-scale-up-model-${state}-${count}-`));
        const fakeBinDir = await mkdtemp(join(tmpdir(), `owx-scale-up-model-${state}-${count}-bin-`));
        const tmuxLogPath = join(fakeBinDir, 'tmux.log');
        const previousPath = process.env.PATH;
  ...
  for (const state of BLOCKED_SCALE_UP_APPROVED_BINDING_STATES) {
    it(`fails closed before worker launch when the persisted approved binding is ${state}`, async () => {
      ...
      const cwd = await mkdtemp(join(tmpdir(), `owx-scale-up-approved-${state}-`));
      const fakeBinDir = await mkdtemp(join(tmpdir(), `owx-scale-up-approved-${state}-bin-`));
      const tmuxLogPath = join(fakeBinDir, 'tmux.log');
      const previousPath = process.env.PATH;
      ...
      try {
        ...
        assert.equal(
          existsSync(join(cwd, '.owx', 'state', 'team', teamName, 'workers', 'worker-2', 'identity.json')),
          false,
        );
        assert.equal(
          existsSync(join(cwd, '.owx', 'state', 'team', teamName, 'workers', 'worker-2', 'inbox.md')),
          false,
        );
  ...
  it('uses project-scoped CODEX_HOME for scaled worker reasoning and model defaults', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-project-reasoning-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-project-reasoning-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    const previousStandardModel = process.env.OWX_DEFAULT_STANDARD_MODEL;
    const previousFrontierModel = process.env.OWX_DEFAULT_FRONTIER_MODEL;
    ...
    try {
      ...
      await writeFile(join(cwd, '.owx', 'setup-scope.json'), JSON.stringify({ scope: 'project' }));
      await mkdir(join(cwd, '.codex'), { recursive: true });
      await writeFile(join(cwd, '.codex', '.owx-config.json'), JSON.stringify({
        env: {
          OWX_DEFAULT_STANDARD_MODEL: 'project-standard-model',
        },
        agentReasoning: {
          writer: 'xhigh',
        },
      ...
      await mkdir(join(cwd, '.codex', 'prompts'), { recursive: true });
      await writeFile(join(cwd, '.codex', 'prompts', 'writer.md'), '<identity>You are Writer.</identity>');
      await mkdir(join(cwd, '.owx', 'state', 'team', 'scale-up-project-reasoning'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'state', 'team', 'scale-up-project-reasoning', 'worker-agents.md'), '# Base worker instructions\n');
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-scale-up-project-reasoning';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'scale-up-project-reasoning', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const workerAgents = await readFile(join(cwd, '.owx', 'state', 'team', 'scale-up-project-reasoning', 'workers', 'worker-2', 'AGENTS.md'), 'utf-8');
      ...
      if (typeof previousStandardModel === 'string') process.env.OWX_DEFAULT_STANDARD_MODEL = previousStandardModel;
      else delete process.env.OWX_DEFAULT_STANDARD_MODEL;
      if (typeof previousFrontierModel === 'string') process.env.OWX_DEFAULT_FRONTIER_MODEL = previousFrontierModel;
      else delete process.env.OWX_DEFAULT_FRONTIER_MODEL;
  ...
  it('removes generated worktree-root AGENTS when scale-up rolls back', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-rollback-worktree-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-rollback-worktree-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-rollback-worktree';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'rollback-worktree', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const workerRootAgents = join(cwd, '.owx', 'team', 'rollback-worktree', 'worktrees', 'worker-2', 'AGENTS.md');
      assert.equal(await readFile(workerRootAgents, 'utf-8'), '# Root project instructions\n');
      const backupPath = join(cwd, '.git', 'worktrees', 'worker-2', 'owx', 'root-agents-backup.json');
  ...
  it('uses canonical root AGENTS bootstrap for scaled worktree workers', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-canonical-root-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-canonical-root-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-canonical-root';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'canonical-root', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const result = await scaleUp(
        ...
        cwd,
        {
          ...
          OWX_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5.4-mini',
        },
      );
      assert.equal(result.ok, true);
      if (!result.ok) return;

      const inbox = await readFile(join(cwd, '.owx', 'state', 'team', 'canonical-root', 'workers', 'worker-2', 'inbox.md'), 'utf-8');
      assert.doesNotMatch(inbox, /## Your Specialization/);
      assert.match(inbox, /\*\*Role:\*\* writer/);

      const rootAgents = await readFile(join(cwd, '.owx', 'team', 'canonical-root', 'worktrees', 'worker-2', 'AGENTS.md'), 'utf-8');
  ...
  it('does not apply mini guidance during scale-up when the final worker model is gpt-5.5', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-frontier-role-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-frontier-role-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      await mkdir(join(cwd, '.owx', 'state', 'team', 'frontier-role'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'state', 'team', 'frontier-role', 'worker-agents.md'), '# Base worker instructions\n');
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-frontier-role';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'frontier-role', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const workerAgents = await readFile(join(cwd, '.owx', 'state', 'team', 'frontier-role', 'workers', 'worker-2', 'AGENTS.md'), 'utf-8');
  ...
  it('does not apply mini guidance during scale-up for gpt-5.4-mini-tuned overrides', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-mini-tuned-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-mini-tuned-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-mini-tuned-root';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'mini-tuned-root', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
      ...
      const result = await scaleUp(
        ...
        cwd,
        {
          OWX_TEAM_SCALING_ENABLED: '1',
          OWX_TEAM_SKIP_READY_WAIT: '1',
          OWX_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5.4-mini-tuned',
        },
      );
      assert.equal(result.ok, true);
      if (!result.ok) return;

      const rootAgents = await readFile(join(cwd, '.owx', 'team', 'mini-tuned-root', 'worktrees', 'worker-2', 'AGENTS.md'), 'utf-8');
      assert.match(rootAgents, /You are operating as the \*\*writer\*\* role/);
      assert.match(rootAgents, /<identity>You are Writer\.<\/identity>/);
  ...
  it('preserves leader/HUD layout by avoiding tiled relayout during scale-up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-up-layout-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-layout-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'owx-team-scale-up-layout';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);

      const manifestPath = join(cwd, '.owx', 'state', 'team', 'scale-up-layout', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
  ...
  it('provisions detached worktrees for scaled-up workers from persisted team worktree mode', async () => {
    ...
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-detached-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    ...
    try {
      ...
      await mkdir(join(repo, '.owx', 'state', 'team', teamName), { recursive: true });
      await writeFile(join(repo, '.owx', 'state', 'team', teamName, 'worker-agents.md'), '# Base worker instructions\n');
      await initTeamState(
        teamName,
        'task',
        'executor',
        ...
        {
          leader_cwd: repo,
          team_state_root: join(repo, '.owx', 'state'),
          workspace_mode: 'worktree',
      ...
      const manifestPath = join(repo, '.owx', 'state', 'team', teamName, 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as { policy?: Record<string, unknown> };
      manifest.policy = {
        ...(manifest.policy ?? {}),
        dispatch_mode: 'transport_direct',
  ...
  it('provisions named worktrees for scaled-up workers from persisted team worktree mode', async () => {
    const repo = await initRepo();
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-up-named-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
...
describe('scaleDown', () => {
  ...
  it('returns error when team not found', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-nf-'));
    try {
  ...
  it('returns error when trying to remove all workers', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-all-'));
    try {
  ...
  it('returns error for worker not in team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-miss-'));
    try {
  ...
  it('returns error when not enough idle workers and force=false', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-busy-'));
    try {
...
describe('scaleDown worktree AGENTS cleanup', () => {
  it('removes generated worktree-root AGENTS during scale-down', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-worktree-agents-'));
    try {
      ...
      const worktree = join(cwd, '.owx', 'team', 'scale-down-worktree', 'worktrees', 'worker-2');
      await mkdir(worktree, { recursive: true });
      await writeFile(join(worktree, 'AGENTS.md'), '# Tracked root instructions\n', 'utf8');
      await mkdir(join(cwd, '.owx', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2'), { recursive: true });
      await writeFile(
        join(cwd, '.owx', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2', 'root-agents-backup.json'),
      ...
      assert.equal(existsSync(join(cwd, '.owx', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2', 'root-agents-backup.json')), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
...
describe('scaleDown teardown hardening', () => {
  it('scaleDown removes workers when pane is already dead or missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-dead-'));
    try {
  ...
  it('scaleDown never targets leader or hud panes during teardown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-scale-down-exclusions-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'owx-scale-down-fake-tmux-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    try {
