...
async function setupTeam(name: string): Promise<{ cwd: string; cleanup: () => Promise<void> }> {
  const cwd = await mkdtemp(join(tmpdir(), `owx-interop-${name}-`));
...
async function setupDisplayTeam(
  ...
  const cwd = await mkdtemp(join(tmpdir(), `owx-interop-${internalName}-`));
  const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
...
async function withMailboxCompatHoleRuntime<T>(cwd: string, fn: () => Promise<T>): Promise<T> {
  ...
  const runtimePath = join(fakeBin, 'owx-runtime');
  const previousPath = process.env.PATH;
  const previousBinary = process.env.OWX_RUNTIME_BINARY;
  const previousBridge = process.env.OWX_RUNTIME_BRIDGE;
...
`,
  ...
  try {
    ...
    if (typeof previousBinary === 'string') process.env.OWX_RUNTIME_BINARY = previousBinary;
    else delete process.env.OWX_RUNTIME_BINARY;
    if (typeof previousBridge === 'string') process.env.OWX_RUNTIME_BRIDGE = previousBridge;
...
describe('buildLegacyTeamDeprecationHint', () => {
  it('produces CLI hint with resolved operation name', () => {
    ...
    assert.match(hint, /owx team api send-message/);
  ...
  it('falls back to generic hint for unresolvable legacy name', () => {
    ...
    assert.match(hint, /owx team api <operation>/);
  });
...
describe('validateCommonFields', () => {
  ...
  it('resolves a long display team_name before applying internal key validation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-api-long-display-'));
    try {
      await initTeamState('long-display-aaaaaaaa', 'test task', 'executor', 1, cwd, undefined, { OWX_SESSION_ID: 'session-long' }, {
      ...
      const previousSessionId = process.env.OWX_SESSION_ID;
      try {
        process.env.OWX_SESSION_ID = 'session-long';
        ...
        if (previousSessionId === undefined) delete process.env.OWX_SESSION_ID;
        else process.env.OWX_SESSION_ID = previousSessionId;
      }
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
...
describe('executeTeamApiOperation: send-message', () => {
  ...
  it('returns the persisted leader mailbox message when hook-targeted sends are deduped from a worker worktree', async () => {
    ...
    const repoCwd = await mkdtemp(join(tmpdir(), 'owx-interop-send-root-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'owx-interop-send-worktree-'));
    ...
    try {
      ...
      const configPath = join(repoCwd, '.owx', 'state', 'team', teamName, 'config.json');
      ...
      const manifestPath = join(repoCwd, '.owx', 'state', 'team', teamName, 'manifest.v2.json');
      ...
      process.env.OWX_TEAM_STATE_ROOT = join(repoCwd, '.owx', 'state');
      ...
      const mailbox = JSON.parse(await readFile(
        join(repoCwd, '.owx', 'state', 'team', teamName, 'mailbox', 'leader-fixed.json'),
        'utf-8',
  ...
  it('returns the persisted mailbox message when bridge compat omits the mailbox row under an explicit shared state root', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'owx-interop-shared-root-'));
...
describe('executeTeamApiOperation: mailbox-mark-delivered', () => {
  it('marks a message delivered after sending and promotes matching dispatch receipt', async () => {
    ...
    try {
      ...
      await mkdir(join(cwd, '.owx', 'state', 'team', 'mark-dlv', 'mailbox', 'worker-2'), { recursive: true });
  ...
  it('marks leader-fixed mailbox delivery from a worker worktree and resolves the matching dispatch receipt', async () => {
    ...
    const repoCwd = await mkdtemp(join(tmpdir(), 'owx-interop-mark-dlv-root-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'owx-interop-mark-dlv-worktree-'));
    const prevTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    delete process.env.OWX_TEAM_STATE_ROOT;
...
describe('executeTeamApiOperation: mailbox-mark-notified', () => {
  it('marks a message notified after sending', async () => {
    ...
    try {
      // Ensure the worker-2 mailbox directory exists so sendDirectMessage can write
      await mkdir(join(cwd, '.owx', 'state', 'team', 'mark-ntf', 'mailbox', 'worker-2'), { recursive: true });
      const sendResult = await executeTeamApiOperation('send-message', {
...
describe('executeTeamApiOperation: list-tasks', () => {
  ...
  it('resolves team working directory from manifest metadata over worker identity/config fallbacks', async () => {
    ...
    const cwdA = await mkdtemp(join(tmpdir(), 'owx-interop-meta-a-'));
    const cwdB = await mkdtemp(join(tmpdir(), 'owx-interop-meta-b-'));
    ...
    try {
      ...
      const teamRootA = join(cwdA, '.owx', 'state', 'team', teamName);
      ...
      config.team_state_root = join(cwdA, '.owx', 'state');
      manifest.team_state_root = join(cwdB, '.owx', 'state');
  ...
  it('resolves display team names from OWX_TEAM_STATE_ROOT when API calls run outside the leader cwd', async () => {
    const { cwd: leaderCwd, cleanup } = await setupDisplayTeam('api-root-display-11111111', 'api-root-display', 'session-api-root-display');
    const workerCwd = await mkdtemp(join(tmpdir(), 'owx-interop-api-root-worker-'));
    try {
      process.env.OWX_TEAM_STATE_ROOT = join(leaderCwd, '.owx', 'state');
  ...
  it('prefers OWX_TEAM_STATE_ROOT over manifest metadata when resolving the team working directory', async () => {
    const teamName = 'list-tsk-env-root';
    const cwdA = await mkdtemp(join(tmpdir(), 'owx-interop-env-a-'));
    const cwdB = await mkdtemp(join(tmpdir(), 'owx-interop-env-b-'));
    const prevTeamWorker = process.env.OWX_TEAM_WORKER;
    const prevTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    delete process.env.OWX_TEAM_STATE_ROOT;
    process.env.OWX_TEAM_WORKER = `${teamName}/worker-1`;
    ...
    try {
      ...
      process.env.OWX_TEAM_STATE_ROOT = join(cwdA, '.owx', 'state');
...
describe('executeTeamApiOperation: read-config', () => {
  ...
  it('returns team_not_found for nonexistent team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-interop-cfg-nf-'));
    try {
      const result = await executeTeamApiOperation('read-config', {
...
describe('executeTeamApiOperation: read-stall-state', () => {
  ...
  it('reads leader runtime activity from boxed OWX_ROOT state consistently with writer path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-interop-boxed-cwd-'));
    const box = await mkdtemp(join(tmpdir(), 'owx-interop-boxed-root-'));
    ...
    try {
      ...
      await mkdir(join(box, '.owx', 'state'), { recursive: true });
      await writeFile(join(box, '.owx', 'state', 'leader-runtime-activity.json'), JSON.stringify({
  ...
  it('suppresses stale native-stop attention after newer leader activity', async () => {
    ...
    try {
      ...
      await writeFile(join(cwd, '.owx', 'state', 'leader-runtime-activity.json'), JSON.stringify({
        last_activity_at: '2026-03-10T10:06:00.000Z',
        last_source: 'team_status',
  ...
  it('suppresses stale native-stop attention after newer detached worktree progress', async () => {
    ...
    try {
      ...
      await writeFile(join(workerWorktree, '.owx', 'state', 'current-task-baseline.json'), JSON.stringify({
        version: 1,
        tasks: [],
      }, null, 2));
...
describe('executeTeamApiOperation: get-summary', () => {
  ...
  it('returns team_not_found for nonexistent team', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-interop-sum-nf-'));
    try {
      const result = await executeTeamApiOperation('get-summary', {
...
describe('executeTeamApiOperation: error handling', () => {
  it('wraps thrown errors in an error envelope', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-interop-err-'));
    try {
      await mkdir(join(cwd, '.owx', 'state', 'team', 'err-team'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'state', 'team', 'err-team', 'config.json'), '{}', 'utf8');
