...
function uniqueAmbientSessionName(): string {
  return `owx-ambient-test-${process.pid}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}
...
describe('withTempTmuxSession', () => {
  it('provides isolated tmux env and cleans up on success', async (t) => {
    ...
    await withTempTmuxSession(async (fixture) => {
      ...
      assert.match(fixture.sessionName, /^owx-test-/);
      assert.equal(fixture.serverKind, 'synthetic');
      assert.match(fixture.serverName, /^owx-fixture-/);
