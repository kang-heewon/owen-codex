...
async function loadRuntimeCliModule() {
  process.env.OWX_RUNTIME_CLI_DISABLE_AUTO_START = '1';
...
describe('runtime-cli helpers', () => {
  ...
  it('refreshes pane targets from live team config after scale changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-live-'));
    try {
  ...
  it('reads task results from explicit OWX_TEAM_STATE_ROOT during shutdown collection', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-env-root-cwd-'));
    const explicitStateRoot = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-env-root-state-'));
  ...
  it('gracefully shuts down only when the leader explicitly requests shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-shutdown-'));
    const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    ...
    try {
      ...
      const teamRoot = join(cwd, '.owx', 'state', 'team', 'shutdown-fallback');
  ...
  it('does not auto-force failed-task shutdown without explicit issue confirmation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-confirm-issues-'));
    const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    delete process.env.OWX_TEAM_STATE_ROOT;
    try {
      ...
      const teamRoot = join(cwd, '.owx', 'state', 'team', 'shutdown-confirm-required');
      assert.equal(existsSync(teamRoot), true);
  ...
  it('does not auto-shutdown merely because monitorTeam reaches complete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-complete-'));
    const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    delete process.env.OWX_TEAM_STATE_ROOT;
    try {
      ...
      const teamRoot = join(cwd, '.owx', 'state', 'team', 'runtime-cli-complete');
      assert.equal(existsSync(teamRoot), true);
  ...
  it('preserves team state when building terminal output for completed teams', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-preserve-complete-'));
    const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    try {
      delete process.env.OWX_TEAM_STATE_ROOT;
      ...
      const teamRoot = join(cwd, '.owx', 'state', 'team', 'runtime-cli-preserve-complete');
      assert.equal(existsSync(teamRoot), true);
      ...
      assert.match(result.notice, /owx team shutdown runtime-cli-preserve-complete/);
      assert.match(result.notice, /owx team api read-stall-state/);
    } finally {
      if (typeof previousTeamStateRoot === 'string') process.env.OWX_TEAM_STATE_ROOT = previousTeamStateRoot;
      else delete process.env.OWX_TEAM_STATE_ROOT;
      await rm(cwd, { recursive: true, force: true });
  ...
  it('preserves team state when building terminal output for failed teams', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-preserve-failed-'));
    const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    try {
      delete process.env.OWX_TEAM_STATE_ROOT;
      ...
      const teamRoot = join(cwd, '.owx', 'state', 'team', 'runtime-cli-preserve-failed');
      assert.equal(existsSync(teamRoot), true);
      ...
      assert.match(result.notice, /preserving team state/i);
      assert.match(result.notice, /owx team api read-stall-state/);
      assert.match(result.notice, /owx team shutdown runtime-cli-preserve-failed/);
    } finally {
      if (typeof previousTeamStateRoot === 'string') process.env.OWX_TEAM_STATE_ROOT = previousTeamStateRoot;
      else delete process.env.OWX_TEAM_STATE_ROOT;
      await rm(cwd, { recursive: true, force: true });
  ...
  it('reports cancelled terminal phases without deleting team state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-runtime-cli-preserve-cancelled-'));
    const previousTeamStateRoot = process.env.OWX_TEAM_STATE_ROOT;
    try {
      delete process.env.OWX_TEAM_STATE_ROOT;
      ...
      const teamRoot = join(cwd, '.owx', 'state', 'team', 'runtime-cli-preserve-cancelled');
      assert.equal(existsSync(teamRoot), true);
      ...
      assert.match(result.notice, /owx team shutdown runtime-cli-preserve-cancelled/);
    } finally {
      if (typeof previousTeamStateRoot === 'string') process.env.OWX_TEAM_STATE_ROOT = previousTeamStateRoot;
      else delete process.env.OWX_TEAM_STATE_ROOT;
      await rm(cwd, { recursive: true, force: true });
