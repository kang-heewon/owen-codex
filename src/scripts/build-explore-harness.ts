...
const binaryName = process.platform === 'win32' ? 'owx-explore-harness.exe' : 'owx-explore-harness';
const sourcePath = join(root, 'target', 'release', binaryName);
const outputPath = join(binDir, binaryName);
const metadataPath = join(binDir, 'owx-explore-harness.meta.json');

const build = spawnSync('cargo', ['build', '--release', '-p', 'owx-explore-harness'], {
  cwd: root,
  encoding: 'utf-8',
