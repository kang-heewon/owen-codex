...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'owx-team-bench-repo-'));
...
async function benchReclaim(iterations = 10): Promise<number[]> {
  ...
  for (let i = 0; i < iterations; i += 1) {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-team-bench-reclaim-'));
    try {
      ...
      const taskPath = join(cwd, '.owx', 'state', 'team', 'team-bench', 'tasks', `task-${task.id}.json`);
