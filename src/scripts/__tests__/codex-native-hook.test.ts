...
async function withLoreGuardConfig<T>(
  ...
  const originalGuard = process.env.OWX_LORE_COMMIT_GUARD;
  const originalCodexHome = process.env.CODEX_HOME;
  const originalHome = process.env.HOME;
  try {
    delete process.env.OWX_LORE_COMMIT_GUARD;
    process.env.CODEX_HOME = codexHome;
    process.env.HOME = defaultHome;
    await writeFile(
      join(codexHome, "config.toml"),
      `[shell_environment_policy.set]\nOWX_LORE_COMMIT_GUARD = "${value}"\n`,
      "utf-8",
    );
    return await run(cwd);
  } finally {
    if (originalGuard === undefined) delete process.env.OWX_LORE_COMMIT_GUARD;
    else process.env.OWX_LORE_COMMIT_GUARD = originalGuard;
...
async function writeActiveAutopilotSession(cwd: string, sessionId: string): Promise<void> {
  ...
  await writeJson(join(cwd, ".owx", "state", "sessions", sessionId, "autopilot-state.json"), {
    active: true,
...
async function writeHookCounterPlugin(cwd: string): Promise<string> {
  const markerPath = join(cwd, ".owx", "stop-hook-counter.json");
  await mkdir(join(cwd, ".owx", "hooks"), { recursive: true });
  await writeFile(
    join(cwd, ".owx", "hooks", "count-stop-hook.mjs"),
    `import { mkdir, readFile, writeFile } from "node:fs/promises";
import { dirname, join } from "node:path";

export async function onHookEvent(event) {
  if (event.event !== "stop") return;
  const outPath = join(process.cwd(), ".owx", "stop-hook-counter.json");
...
describe("codex native hook config", () => {
  it("builds the expected managed hooks.json shape", () => {
    const config = buildManagedCodexHooksConfig("/tmp/owx");
...
describe("codex native hook dispatch", () => {
  ...
  it("does not treat a different module url as the main module", () => {
    assert.equal(
      isCodexNativeHookMainModule(
        pathToFileURL("/tmp/owx native/other-script.js").href,
        "/tmp/owx native/codex-native-hook.js",
      ),
      false,
    );
  });
  ...
  it("emits Stop-schema-safe block JSON when unidentifiable malformed stdin has native Stop runtime surface", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-malformed-stop-surface-"));
    try {
  ...
  it("preserves non-Stop fail-closed JSON when malformed stdin identifies a non-Stop hook", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-malformed-nonstop-"));
    try {
      await mkdir(join(cwd, ".owx"), { recursive: true });
      const result = spawnSync(process.execPath, [nativeHookScriptPath()], {
        cwd,
  ...
  it("redacts unterminated prompt-like malformed stdin fields", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-malformed-unterminated-"));
    try {
  ...
  it("logs a bounded redacted raw stdin prefix when CLI stdin is malformed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-malformed-log-prefix-"));
    try {
  ...
  it("returns empty JSON for oversized Stop stdin without parsing or creating inactive state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-oversized-"));
    try {
  ...
  it("blocks oversized Stop stdin when current session autopilot is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-oversized-active-"));
    try {
      ...
      assert.equal(existsSync(join(cwd, ".owx", "logs")), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("does not block oversized Stop stdin for unrelated root autopilot state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-oversized-stale-root-"));
    try {
      await writeJson(join(cwd, ".owx", "state", "session.json"), {
        session_id: "sess-current-without-active-autopilot",
        cwd,
      });
      await writeJson(join(cwd, ".owx", "state", "autopilot-state.json"), {
        active: true,
        current_phase: "execution",
      });
      const oversizedStop = JSON.stringify({
  ...
  it("does not block oversized Stop stdin when terminal run-state shadows stale autopilot state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-oversized-terminal-run-"));
    try {
      const sessionId = "sess-cli-stop-oversized-terminal-run";
      await writeActiveAutopilotSession(cwd, sessionId);
      await writeJson(join(cwd, ".owx", "state", "sessions", sessionId, "run-state.json"), {
        version: 1,
        active: false,
        mode: "autopilot",
        outcome: "finish",
  ...
  it("fails closed for oversized non-Stop stdin before parsing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-nonstop-oversized-"));
    try {
  ...
  it("does not crash Stop hook dispatch when the exec follow-up queue is malformed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-stop-exec-followup-corrupt-"));
    try {
      const session = await writeSessionStart(cwd, "sess-exec-followup-corrupt");
      const queuePath = join(cwd, ".owx", "state", "sessions", session.session_id, "exec-followups.json");
      ...
      const auditPath = join(cwd, ".owx", "logs", `exec-followups-${new Date().toISOString().slice(0, 10)}.jsonl`);
  ...
  it("emits exactly one parseable JSON object for active Stop CLI continuation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-json-"));
    try {
  ...
  it("keeps noisy Stop hook plugin stdout out of native Stop CLI stdout", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-noisy-plugin-"));
    try {
      await writeActiveAutopilotSession(cwd, "sess-cli-stop-noisy-plugin");
      await mkdir(join(cwd, ".owx", "hooks"), { recursive: true });
      await writeFile(
        join(cwd, ".owx", "hooks", "noisy.mjs"),
        `export async function onHookEvent(event) {
...
`,
  ...
  it("logs Stop dispatch failures without foreground stderr noise", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-stop-dispatch-silent-"));
    try {
      const result = spawnSync(process.execPath, [nativeHookScriptPath()], {
        cwd,
  ...
  it("keeps non-Stop dispatch failures fail-closed without foreground stderr noise", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-cli-pretool-dispatch-silent-"));
    try {
      const result = spawnSync(process.execPath, [nativeHookScriptPath()], {
        cwd,
        input: JSON.stringify({
        ...
        env: {
          ...process.env,
          NODE_ENV: "test",
          OWX_NATIVE_HOOK_TEST_THROW_DISPATCH: "1",
        },
      });
  ...
  it("maps Codex events onto OWX logical surfaces", () => {
  ...
  it("does not write PreCompact stdout that Codex rejects as hook JSON", async () => {
    ...
    try {
      ...
      assert.equal(result.owxEventName, "pre-compact");
      assert.equal(result.outputJson, null);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("emits no CLI stdout for PreCompact when no Codex action is needed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-precompact-cli-"));
    try {
      writePage(cwd, {
        filename: "architecture.md",
        frontmatter: {
  ...
  it("does not write PostCompact stdout that Codex rejects as hook JSON", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-postcompact-"));
    try {
      const result = await dispatchCodexNativeHook({
      ...
      assert.equal(result.owxEventName, "post-compact");
      assert.equal(result.outputJson, null);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("emits no CLI stdout for PostCompact when no Codex action is needed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-postcompact-cli-"));
    try {
      const stdout = runNativeHookCli({
        hook_event_name: "PostCompact",
        cwd,
  ...
  it("preserves canonical OWX session scope when native SessionStart arrives with a different id", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-reconcile-"));
    try {
  ...
  it("keeps subagent SessionStart from replacing the canonical leader session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-subagent-session-start-"));
    const originalCodexHome = process.env.CODEX_HOME;
    try {
      process.env.CODEX_HOME = join(cwd, "codex-home");
      await writeJson(join(process.env.CODEX_HOME, ".owx-config.json"), {
        notifications: {
          enabled: true,
          verbosity: "session",
      ...
      await mkdir(join(cwd, ".owx", "hooks"), { recursive: true });
      await writeFile(
        join(cwd, ".owx", "hooks", "record-lifecycle.mjs"),
        [
  ...
  it("allows explicit child-agent lifecycle hook dispatch when includeChildAgents is enabled", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-subagent-session-include-"));
    const originalCodexHome = process.env.CODEX_HOME;
    try {
      process.env.CODEX_HOME = join(cwd, "codex-home");
      await writeJson(join(process.env.CODEX_HOME, ".owx-config.json"), {
  ...
  it("allows child-agent lifecycle hook dispatch at agent verbosity", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-subagent-session-agent-"));
    const originalCodexHome = process.env.CODEX_HOME;
    try {
      process.env.CODEX_HOME = join(cwd, "codex-home");
      await writeJson(join(process.env.CODEX_HOME, ".owx-config.json"), {
  ...
  it("keeps a self-parented native role thread as subagent evidence", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-self-parented-subagent-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not attach a subagent SessionStart to an unrelated canonical leader", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-subagent-session-start-mismatch-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      const canonicalSessionId = "owx-leader-session-a";
  ...
  it("keeps SessionStart quiet when .owx/ is already ignored by repo-level gitignore", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-existing-ignore-"));
    try {
      await writeFile(join(cwd, ".gitignore"), "node_modules/\n.owx/\n");
      execFileSync("git", ["init"], { cwd, stdio: "pipe" });
      ...
      assert.doesNotMatch(JSON.stringify(result.outputJson), /Added \.owx\//);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("respects existing Git ignore resolution before writing local excludes", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-global-ignore-"));
  ...
  it("includes persisted project-memory summary in SessionStart context", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-memory-"));
    try {
  ...
  it("includes repo-local .owx project-memory during SessionStart when OWX_ROOT is boxed", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-boxed-memory-"));
    const boxedRoot = await mkdtemp(join(tmpdir(), "owx-native-hook-boxed-root-"));
    ...
    try {
      ...
      assert.match(additionalContext, /Prefer repo-local \.owx project memory over boxed runtime fallback\./);
  ...
  it("prefers repository project-memory.json during SessionStart while preserving legacy wiki guidance", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-root-memory-legacy-wiki-"));
    try {
      ...
      assert.match(additionalContext, /Load root project-memory\.json before legacy \.owx memory\./);
      ...
      assert.match(additionalContext, /legacy pages at \.owx\/wiki\//);
  ...
  it("starts a fresh native session without inheriting stale task-scoped context", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-session-isolation-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      await writeFile(
        ...
        [
          ...
          "",
          "## WORKING MEMORY",
          "[2026-04-06T00:33:44Z] stale UI rework context snapshot .owx/context/ui-rework-plan-01-20260406T003344Z.md",
        ].join("\n"),
      );
  ...
  it("records keyword activation from UserPromptSubmit payloads", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-"));
    try {
      ...
      const statePath = join(cwd, ".owx", "state", "sessions", "sess-1", "skill-active-state.json");
      ...
      assert.equal(existsSync(join(cwd, ".owx", "state", "sessions", "sess-1", "ralplan-state.json")), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("injects deep-interview config overrides into UserPromptSubmit developer context", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-config-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      await writeFile(
...
`,
  ...
  it("proves UserPromptSubmit context changes before and after adding deep-interview config", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-config-before-after-"));
...
`,
  ...
  it("injects deep-interview config for mixed workflow prompts that defer execution modes", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-config-mixed-"));
...
`,
  ...
  it("keeps deep-interview config override context on continuation prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-config-continuation-"));
...
`,
  ...
  it("keeps explicit deep-interview profile flags reflected on continuation prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-config-profile-continuation-"));
...
`,
  ...
  it("keeps the documented deep-interview Suggested Config reflected in UserPromptSubmit context", async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-doc-config-"));
    const sessionId = "sess-deep-interview-doc-config";
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      await writeFile(join(cwd, ".owx", "config.toml"), `${documentedConfig}\n`);
  ...
  it("injects deep-interview config overrides when state is boxed under OWX_ROOT", async () => {
    const root = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-config-boxed-"));
...
`,
  ...
  it("records boxed keyword activation mode detail and skill state under OWX_ROOT", async () => {
    const root = await mkdtemp(join(tmpdir(), "owx-native-hook-boxed-"));
    const cwd = join(root, "source");
    const owxRoot = join(root, "box");
  ...
  it("warns completion-like prompts when active goal workflows need Codex snapshot reconciliation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-goal-warning-"));
    try {
  ...
  it("blocks Stop when a completion-like final answer skips active goal snapshot reconciliation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-goal-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "performance", "latency", "state.json"), {
        version: 1,
      ...
      assert.match(JSON.stringify(result.outputJson), /owx performance-goal complete --slug latency/);
  ...
  it("does not repeat performance-goal reconciliation after a recorded objective mismatch blocker", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-performance-mismatch-blocked-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "performance", "latency", "state.json"), {
        version: 1,
        workflow: "performance-goal",
  ...
  it("does not block Stop for an already complete performance-goal state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-performance-complete-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "performance", "latency", "state.json"), {
        version: 1,
        workflow: "performance-goal",
  ...
  it("blocks ultragoal Stop for concise generic goal completion claims", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-generic-complete-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "ultragoal", "goals.json"), {
        version: 1,
        activeGoalId: "G001-demo",
  ...
  it("does not block ultragoal Stop for ordinary prose about a goal to complete work", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-ordinary-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "ultragoal", "goals.json"), {
        version: 1,
        activeGoalId: "G001-demo",
  ...
  it("blocks ultragoal Stop with blocked checkpoint and available-goal-context remediation for completed legacy snapshots", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-legacy-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "ultragoal", "goals.json"), {
        version: 1,
        activeGoalId: "G001-demo",
      ...
      assert.equal(result.outputJson?.decision, "block");
      assert.match(output, /owx ultragoal checkpoint --goal-id G001-demo --status complete/);
  ...
  it("does not repeat ultragoal Stop recovery after a safe completed-aggregate microgoal blocker is recorded", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-aggregate-blocked-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "ultragoal", "goals.json"), {
        version: 1,
  ...
  it("does not block Stop for non-passing autoresearch-goal professor-critic verdicts", async () => {
    for (const verdict of ["blocked", "fail", "failed"]) {
      const cwd = await mkdtemp(join(tmpdir(), `owx-native-hook-autoresearch-${verdict}-stop-`));
      const slug = `${verdict}-mission`;
      try {
        await writeJson(join(cwd, ".owx", "goals", "autoresearch", slug, "mission.json"), {
          version: 1,
        ...
        await writeJson(join(cwd, ".owx", "goals", "autoresearch", slug, "completion.json"), {
          verdict,
          passed: false,
        });
  ...
  it("blocks Stop for passing autoresearch-goal professor-critic verdicts that need reconciliation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autoresearch-pass-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "passing-mission", "mission.json"), {
        version: 1,
        workflow: "autoresearch-goal",
      ...
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "passing-mission", "completion.json"), {
        verdict: "fail",
        passed: true,
      });
      ...
      assert.equal(result.outputJson?.decision, "block");
      assert.match(JSON.stringify(result.outputJson), /get_goal snapshot reconciliation/);
      assert.match(JSON.stringify(result.outputJson), /owx autoresearch-goal complete --slug passing-mission/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("blocks Stop for autoresearch-goal verdict=pass even when passed is omitted", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autoresearch-verdict-pass-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "verdict-pass-mission", "mission.json"), {
        version: 1,
        workflow: "autoresearch-goal",
      ...
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "verdict-pass-mission", "completion.json"), {
        verdict: "pass",
      });
  ...
  it("does not repeat Stop block when the last autoresearch-goal completion attempt reported objective mismatch", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autoresearch-mismatch-reported-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "mismatched-mission", "mission.json"), {
        version: 1,
        workflow: "autoresearch-goal",
      ...
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "mismatched-mission", "completion.json"), {
        verdict: "pass",
        passed: true,
      });
  ...
  it("still blocks later autoresearch-goal completion claims after an objective mismatch if no mismatch is reported in the final answer", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autoresearch-mismatch-later-retry-stop-"));
    try {
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "retryable-mission", "mission.json"), {
        version: 1,
        workflow: "autoresearch-goal",
      ...
      await writeJson(join(cwd, ".owx", "goals", "autoresearch", "retryable-mission", "completion.json"), {
        verdict: "pass",
        passed: true,
      });
      ...
      assert.equal(result.outputJson?.decision, "block");
      assert.match(JSON.stringify(result.outputJson), /get_goal snapshot reconciliation/);
      assert.match(JSON.stringify(result.outputJson), /owx autoresearch-goal complete --slug retryable-mission/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("treats workflow keywords in native subagent prompt text as literal delegation text", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-subagent-keyword-literal-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not treat a corrupt leader kind=subagent tracker entry as native subagent prompt scope", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-corrupt-leader-subagent-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("lets the current canonical leader boundary beat stale global subagent tracking with a distinct prompt thread id", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-current-leader-stale-global-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("lets the current session native leader beat stale global subagent tracking without a canonical summary and with a distinct prompt thread id", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-current-native-leader-stale-global-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("lets the current session native leader beat a malformed canonical subagent entry", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-current-native-leader-malformed-canonical-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("still treats mixed child and leader payload identities as native subagent scope", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-mixed-child-leader-identity-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      assert.equal(reversedResult.owxEventName, "keyword-detector");
  ...
  it("records plugin-prefixed keyword activation from UserPromptSubmit payloads", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-plugin-prefixed-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
          ...
          prompt: "$owen-codex:ralplan implement issue #1307",
        },
        { cwd },
      );
      ...
      assert.equal(existsSync(join(cwd, ".owx", "state", "sessions", "sess-plugin-1", "ralplan-state.json")), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("injects autopilot ralplan consensus gate guidance on prompt activation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-gate-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("records ultragoal prompt skill activation with goal-tool handoff guidance", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("deactivates active deep-interview state on explicit ultragoal handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-deep-interview-handoff-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("applies only explicit structured UserPromptSubmit ultragoal steering directives", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-steer-"));
    try {
      await createUltragoalPlan(cwd, {
        brief: "G002-cli-and-prompt-submit-bridge .owx/ultragoal hook steering fixture",
      ...
      const jsonExample = await dispatchCodexNativeHook(
        {
          ...
          prompt: `Here is an inert example:\n\`\`\`json\n${JSON.stringify({
            ...
            evidence: "Example JSON should not mutate .owx/ultragoal.",
  ...
  it("does not apply UserPromptSubmit ultragoal steering from native subagent prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-steer-subagent-"));
    try {
      await createUltragoalPlan(cwd, {
        brief: "G002-cli-and-prompt-submit-bridge .owx/ultragoal subagent steering fixture",
        goals: [{ title: "First", objective: "Complete first milestone with tests." }],
      });
      const stateDir = join(cwd, ".owx", "state");
      ...
      const result = await dispatchCodexNativeHook(
        {
          ...
          prompt: `OWX_ULTRAGOAL_STEER: ${JSON.stringify({
            ...
            rationale: "Subagent prompts should not mutate the parent .owx/ultragoal ledger.",
  ...
  it("dedupes repeated UserPromptSubmit ultragoal steering directives by prompt signature", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultragoal-steer-dedupe-"));
    try {
      await createUltragoalPlan(cwd, {
        brief: "G002-cli-and-prompt-submit-bridge .owx/ultragoal dedupe fixture",
        goals: [{ title: "First", objective: "Complete first milestone with tests." }],
      });
...
\`\`\``;
  ...
  it("normalizes the Korean keyboard typo for ulw during UserPromptSubmit activation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ulw-ko-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("adds ultrawork-specific activation guidance only for true ultrawork workflow activation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ultrawork-routing-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("does not activate Ralph workflow state from a plain conversational mention", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralph-plain-text-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("adds latest-followup priority context for short same-thread follow-up prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-followup-priority-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("clarifies that prompt-side $ralph activation does not invoke the PRD-gated CLI path", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralph-routing-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("clarifies that plugin-prefixed prompt-side $ralph activation does not invoke the PRD-gated CLI path", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-plugin-ralph-routing-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
          ...
          prompt: "$owen-codex:ralph continue verification",
        },
        { cwd },
      );
  ...
  it("keeps bare keep-going continuation on the active autopilot skill instead of denying with generic ralph overlap", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-bare-continuation-"));
    try {
  ...
  it("keeps bare keep-going continuation on the active ralph skill without resetting through generic keep-going routing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralph-bare-continuation-"));
    try {
  ...
  it("ignores generic wrapper fields so metadata cannot trigger workflow routing or Stop blocking", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-wrapper-metadata-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const promptResult = await dispatchCodexNativeHook(
        {
      ...
      assert.equal(stopResult.owxEventName, "stop");
  ...
  it("does not expose submitted prompt text to keyword-detector hook plugins", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-prompt-sanitized-"));
    try {
      await mkdir(join(cwd, ".owx", "hooks"), { recursive: true });
      await writeFile(
        join(cwd, ".owx", "hooks", "capture-keyword-context.mjs"),
        `import { mkdir, writeFile } from "node:fs/promises";
import { dirname, join } from "node:path";

export async function onHookEvent(event) {
  if (event.event !== "keyword-detector") return;
  const outPath = join(process.cwd(), ".owx", "captured-keyword-context.json");
  await mkdir(dirname(outPath), { recursive: true });
...
`,
  ...
  it("does not emit UserPromptSubmit routing context for unknown $tokens", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-unknown-token-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("does not emit UserPromptSubmit routing context for unknown plugin-prefixed $tokens", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-unknown-plugin-token-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
          ...
          prompt: "$owen-codex:maer-thinking 다시 설명해봐",
        },
        { cwd },
      );
  ...
  it("surfaces transition success output for allowlisted prompt-submit handoffs", async () => {
    ...
    try {
      const sessionDir = join(cwd, ".owx", "state", "sessions", "sess-handoff-1");
      await mkdir(sessionDir, { recursive: true });
      await writeJson(join(sessionDir, "deep-interview-state.json"), {
        active: true,
        mode: "deep-interview",
...
`,
  ...
  it("allows deep-interview artifact and state writes while blocking implementation Bash writes", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-deep-interview-artifact-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      const allowedAppendBash = await dispatchCodexNativeHook(
        {
          ...
          tool_input: { command: "echo more context >> .owx/context/demo.md" },
        },
        { cwd },
      );
  ...
  it("allows null-device fd redirects while deep-interview blocks real Bash writes", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-deep-interview-null-redirect-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows implementation tools after an explicit deep-interview handoff deactivates the mode", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-deep-interview-handoff-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("returns a destructive-command caution on PreToolUse for rm -rf dist", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-danger-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for neutral pwd", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-neutral-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("does not warn on PreToolUse for read-only fallback text inspection", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-slop-readonly-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("warns when a read-only command is chained before sloppy fallback writes", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-slop-chained-write-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("does not warn on PreToolUse for grounded compatibility fallback code", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-slop-grounded-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks Stop for untracked non-Bash-style sloppy fallback source edits", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-untracked-");
    try {
  ...
  it("keeps blocking repeated Stop while sloppy fallback diff remains", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-repeat-");
    try {
      await mkdir(join(cwd, "src"), { recursive: true });
      await writeFile(
        join(cwd, "src", "runtime.ts"),
  ...
  it("blocks Stop for unstaged tracked sloppy fallback source edits", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-unstaged-");
    try {
      await mkdir(join(cwd, "src"), { recursive: true });
  ...
  it("blocks Stop from a subdirectory cwd for untracked sloppy source elsewhere", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-subdir-");
    try {
  ...
  it("blocks Stop for staged sloppy fallback source edits", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-staged-");
    try {
      await mkdir(join(cwd, "src"), { recursive: true });
      await writeFile(join(cwd, "src", "runtime.ts"), "export const runtime = 'base';\n");
      execFileSync("git", ["add", "src/runtime.ts"], { cwd, stdio: "ignore" });
  ...
  it("does not block Stop for grounded compatibility fallback source edits", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-grounded-");
    try {
      await mkdir(join(cwd, "src"), { recursive: true });
      await writeFile(
  ...
  it("does not block Stop when existing nearby source context grounds a new fallback line", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-existing-ground-");
    try {
      await mkdir(join(cwd, "src"), { recursive: true });
      await writeFile(
        join(cwd, "src", "compat.ts"),
  ...
  it("does not block Stop for source-adjacent test file fallback wording", async () => {
    const cwd = await initTempGitRepo("owx-native-hook-stop-slop-test-file-");
    try {
      await mkdir(join(cwd, "src"), { recursive: true });
      await writeFile(
  ...
  it("does not block Stop for note-only fallback wording", async () => {
  ...
  it("keeps git commit Lore enforcement ahead of sloppy fallback advisory", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-slop-git-priority-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit with supported response shape when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "git commit is blocked until the inline commit message satisfies the Lore format and includes the required OwX co-author trailer.",
        hookSpecificOutput: {
          hookEventName: "PreToolUse",
        },
        systemMessage: [
          "git commit is blocked until the inline commit message follows the Lore protocol and includes `Co-authored-by: OwX <owx@owen-codex.dev>`.",
  ...
  it("blocks PreToolUse git commit when process env explicitly enables the Lore commit guard", async () => {
    ...
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      process.env.OWX_LORE_COMMIT_GUARD = "1";
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
          cwd,
      ...
      if (original === undefined) delete process.env.OWX_LORE_COMMIT_GUARD;
      else process.env.OWX_LORE_COMMIT_GUARD = original;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it("allows non-Lore git commit messages when the Lore commit guard is disabled by default", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-disabled-"));
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      delete process.env.OWX_LORE_COMMIT_GUARD;
      const result = await dispatchCodexNativeHook(
  ...
  it("allows non-Lore git commit messages when the Lore commit guard is disabled inline", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-inline-disabled-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("does not treat newline-separated Lore guard assignment as inline git commit opt-in", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-newline-assignment-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("restores default-off Lore guard when env -u unsets an enabled process env", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-env-unset-"));
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      process.env.OWX_LORE_COMMIT_GUARD = "1";
      const result = await dispatchCodexNativeHook(
  ...
  it("restores default-off Lore guard when env -i clears an enabled process env", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-env-ignore-"));
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      process.env.OWX_LORE_COMMIT_GUARD = "1";
      const result = await dispatchCodexNativeHook(
  ...
  it("keeps Lore commit enforcement disabled for unknown inline guard values", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-inline-unknown-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("treats Lore commit guard disabled values as trim and case tolerant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-off-"));
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      process.env.OWX_LORE_COMMIT_GUARD = " OFF ";
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
          cwd,
  ...
  it("keeps Lore commit enforcement disabled for unknown guard values", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-lore-unknown-"));
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      process.env.OWX_LORE_COMMIT_GUARD = "maybe";
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
          cwd,
  ...
  it("continues to later PreToolUse checks when Lore commit guard is disabled", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-lore-disabled-destructive-"));
    const original = process.env.OWX_LORE_COMMIT_GUARD;
    try {
      process.env.OWX_LORE_COMMIT_GUARD = "false";
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
          cwd,
  ...
  it("stays silent on PreToolUse for `git help commit`", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-help-commit-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for `git config alias.ci commit`", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-config-alias-commit-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for `git tag commit`", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-tag-commit-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env-prefixed git commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-env-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when git options appear before the real commit subcommand", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-option-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-exe-commit-env-wrapper-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-exe-commit-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env flag wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-exe-commit-env-flag-wrapper-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse env value-taking wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-exe-commit-env-value-wrapper-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse path-qualified Windows git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-exe-commit-windows-path-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse quoted backslash Windows git.exe commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-exe-commit-windows-backslash-path-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse path-qualified git commit when the inline message is not Lore-compliant", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-path-invalid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when the message comes from an external source", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-file-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse git commit when Lore trailers exist but the OwX co-author trailer is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-missing-owx-coauthor-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for Lore-compliant git commit with OwX co-author trailer", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-valid-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for compact inline Lore commit with only OwX co-author trailer", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-compact-coauthor-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("stays silent on PreToolUse for body-omitted inline Lore commit with decision trailers", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-compact-trailers-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("blocks PreToolUse compact inline Lore commit when the blank separator is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-pretool-git-commit-compact-no-separator-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PreToolUse",
  ...
  it("returns PostToolUse remediation guidance for command-not-found output", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-failure-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("stays silent when successful search output contains old Bash failure text", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-successful-search-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
          ...
          tool_input: { command: "rg 'command not found' .owx/logs" },
  ...
  it("stays silent when Bash stdout only contains failure-like source text", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-failure-source-text-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("stays silent for rc-zero build logs that mention missing grep paths", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-build-log-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not treat Bash output containing MCP transport text as MCP transport death", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-mcp-source-text-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("stays silent when successful output includes prior hook context text", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-recursive-context-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("stays silent when successful Bash output quotes MCP transport warnings", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-bash-mcp-quote-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("stays silent when Bash hard-failure text has no parsed exit code", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-bash-unparsed-failure-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not treat non-MCP source output containing detector constants as MCP transport death", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-read-mcp-source-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not treat non-MCP docs stdout mentioning closed MCP transport as transport death", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-docs-mcp-log-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not MCP-block non-MCP command output with unrelated stderr and MCP transport stdout", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-nonmcp-mixed-output-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("still blocks MCP-like raw transport failures", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-mcp-raw-transport-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not classify non-transport MCP failures as transport death", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-mcp-nontransport-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not block ordinary non-zero grep output in PostToolUse", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-grep-nonzero-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not block ordinary non-zero diagnostic output in PostToolUse", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-diagnostic-nonzero-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("treats stderr-only informative non-zero output as reviewable instead of a generic failure", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-informative-stderr-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("treats non-zero gh pr checks style output as informative instead of a generic failure", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-informative-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("treats wrapped gh pr checks output as reviewable", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-gh-wrapped-"));
    try {
  ...
  it("does not treat heredoc gh pr checks text as a reviewable command", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-gh-heredoc-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("does not treat echoed gh pr checks text as a reviewable command", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-gh-echo-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  it("stays silent on neutral successful PostToolUse output", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-posttool-neutral-"));
    try {
      const result = await dispatchCodexNativeHook(
        {
          hook_event_name: "PostToolUse",
  ...
  for (const rootActiveCase of [
    ...
    it(`returns Stop continuation output from root ${rootActiveCase.mode} state when no session is active`, async () => {
      const cwd = await mkdtemp(join(tmpdir(), `owx-native-hook-stop-root-${rootActiveCase.mode}-`));
      try {
        const stateDir = join(cwd, ".owx", "state");
        await mkdir(stateDir, { recursive: true });
        ...
        assert.deepEqual(result.outputJson, {
          decision: "block",
          reason: `OWX ${rootActiveCase.mode} is still active (phase: ${rootActiveCase.phase}); continue the task and gather fresh verification evidence before stopping.`,
          stopReason: `${rootActiveCase.mode}_${rootActiveCase.phase}`,
          systemMessage: `OWX ${rootActiveCase.mode} is still active (phase: ${rootActiveCase.phase}).`,
        });
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
  ...
  it("returns Stop continuation output while Autopilot is active", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OWX autopilot is still active (phase: execution); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "autopilot_execution",
        systemMessage: "OWX autopilot is still active (phase: execution).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("suppresses parent Autopilot Stop continuation in side conversations", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-autopilot-side-conversation-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("requires Autopilot code review after a compact-boundary Stop exemption", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-autopilot-review-compact-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      await writeJson(join(stateDir, "sessions", sessionId, "autopilot-state.json"), {
        ...
        state: {
          ...
          handoff_artifacts: {
            ralplan: ".owx/plans/prd-issue-2366.md",
      ...
      assert.equal(compactBoundary.owxEventName, "stop");
      assert.equal(compactBoundary.outputJson, null);
      assert.equal(resumedStop.owxEventName, "stop");
      assert.deepEqual(resumedStop.outputJson, {
        decision: "block",
        reason:
          "OWX autopilot is still active (phase: code-review); continue the task and gather fresh verification evidence before stopping.",
  ...
  it("suppresses duplicate Autopilot planning Stop replays so stale planning state cannot loop indefinitely", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-autopilot-planning-replay-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      assert.deepEqual(first.outputJson, {
        decision: "block",
        reason:
          "OWX autopilot is still active (phase: planning); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "autopilot_planning",
        systemMessage: "OWX autopilot is still active (phase: planning).",
      });
  ...
  it("allows Stop when terminal Autopilot run-state shadows stale session ralplan state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-autopilot-terminal-run-state-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("still blocks Stop while Autopilot ralplan state is genuinely non-terminal", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-autopilot-active-ralplan-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OWX autopilot is still active (phase: ralplan); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "autopilot_ralplan",
        systemMessage: "OWX autopilot is still active (phase: ralplan).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  for (const staleRootCase of [
    ...
    it(`does not block Stop from stale root ${staleRootCase.mode} state when the explicit session directory is missing`, async () => {
      const cwd = await mkdtemp(join(tmpdir(), `owx-native-hook-stop-missing-session-${staleRootCase.mode}-`));
      try {
        const stateDir = join(cwd, ".owx", "state");
        await mkdir(stateDir, { recursive: true });
  ...
  it("does not block Stop when an explicit blocked_on_user run_outcome is present on a mode state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-autopilot-blocked-outcome-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("returns Stop continuation output while UltraQA is active", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OWX ultraqa is still active (phase: diagnose); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "ultraqa_diagnose",
        systemMessage: "OWX ultraqa is still active (phase: diagnose).",
      });
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("does not block when canonical root ralplan state is inactive but session ralplan state is stale active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-stale-session-ralplan-root-inactive-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("keeps blocking current session ralplan when root inactive ralplan state belongs to another session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-session-ralplan-root-other-session-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("keeps blocking current session ralplan when root inactive ralplan state is unscoped", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-session-ralplan-root-unscoped-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block stale session ralplan when root ralplan is terminal and another root skill is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-stale-ralplan-other-root-skill-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("keeps blocking session ralplan when canonical root state is not inactive", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-session-ralplan-root-active-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block on stale ralplan skill-active when canonical run-state is terminal", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-terminal-ralplan-run-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block on stale ralplan skill-active when pinned mode state belongs to another session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-foreign-ralplan-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block on stale root ralplan skill when the explicit session-scoped canonical skill state is absent", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-stale-root-skill-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      await mkdir(stateDir, { recursive: true });
      await writeJson(join(stateDir, "skill-active-state.json"), {
  ...
  it("does not block Stop from stale root autoresearch state when the explicit session directory is missing", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-missing-session-autoresearch-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      await mkdir(stateDir, { recursive: true });
      await writeJson(join(stateDir, "autoresearch-state.json"), {
  ...
  it("ignores root skill-active fallback from a different thread when evaluating Stop", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-foreign-thread-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      await mkdir(stateDir, { recursive: true });
      await writeJson(join(stateDir, "skill-active-state.json"), {
  ...
  it("blocks Codex App Stop when Ralph is marked complete without completion-audit evidence", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralph-complete-audit-missing-"));
    try {
  ...
  it("returns Stop continuation output while Ralph is active without an explicit session pin", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      await mkdir(stateDir, { recursive: true });
      await writeFile(
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OWX Ralph is still active (phase: executing; state: .owx/state/ralph-state.json); continue the task and gather fresh verification evidence before stopping.",
  ...
  it("blocks Stop from session-scoped Ralph state when session.json points to another session", async () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OWX Ralph is still active (phase: executing; state: .owx/state/sessions/sess-live-ralph/ralph-state.json); continue the task and gather fresh verification evidence before stopping.",
        stopReason: "ralph_executing",
        systemMessage:
          "OWX Ralph is still active (phase: executing; state: .owx/state/sessions/sess-live-ralph/ralph-state.json); continue the task and gather fresh verification evidence before stopping.",
      });
  ...
  it("does not block Stop from stale current-session Ralph state when session.json points to a dead owner", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-stale-current-session-ralph-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not hard-block Stop on stale session-scoped Ralph starting state after visible active modes are cleared", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-cleared-stale-ralph-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows Stop from stale orphaned session-scoped Ralph starting iteration zero state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-stale-orphan-starting-ralph-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks Stop on visible active session-scoped Ralph starting state and reports its path", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-visible-starting-ralph-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      assert.deepEqual(result.outputJson, {
        decision: "block",
        reason:
          "OWX Ralph is still active (phase: starting; state: .owx/state/sessions/sess-visible-ralph/ralph-state.json); continue the task and gather fresh verification evidence before stopping.",
  ...
  it("retires prompt-seeded Ralph starting state when canonical Ralph already completed with audit", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-shadowed-starting-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not retire prompt-seeded Ralph starting state from a completed canonical Ralph owned by another thread", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-shadowed-thread-mismatch-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      const nativeSessionId = "native-hook-seed";
      const canonicalSessionId = "owx-runtime-session";
  ...
  it("does not block Stop when Ralph skill-active initialization points at another session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-stale-skill-active-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      await writeJson(join(stateDir, "sessions", currentSessionId, "ralph-state.json"), {
        ...
        current_phase: "verifying",
        session_id: currentSessionId,
        owner_owx_session_id: currentSessionId,
  ...
  it("blocks same-session Ralph Stop continuation when ownership identifiers match", async () => {
    ...
    try {
      ...
      await writeJson(join(stateDir, "sessions", owxSessionId, "ralph-state.json"), {
        active: true,
        mode: "ralph",
        current_phase: "executing",
        session_id: owxSessionId,
  ...
  it("allows native verifier subagent Stop to complete while leader Ralph remains active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-subagent-verdict-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      assert.equal(childStop.owxEventName, "stop");
      ...
      assert.equal(leaderStop.owxEventName, "stop");
      assert.deepEqual(leaderStop.outputJson, {
        decision: "block",
        reason:
          "OWX Ralph is still active (phase: verifying; state: .owx/state/sessions/sess-ralph-leader-verifier/ralph-state.json); continue the task and gather fresh verification evidence before stopping.",
  ...
  it("prefers canonical run-state terminal lifecycle before stale session Ralph state during Stop", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-canonical-run-state-ralph-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block Stop from root Ralph fallback when an explicit session_id is present and session.json points to another worktree", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-root-fallback-cwd-mismatch-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      await mkdir(stateDir, { recursive: true });
      await writeJson(join(stateDir, "session.json"), {
  ...
  it("keeps blocking Ralph Stop replays until the active task advances", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-replay-"));
    const previousOmxSessionId = process.env.OWX_SESSION_ID;
    try {
      const stateDir = join(cwd, ".owx", "state");
      await mkdir(stateDir, { recursive: true });
      ...
      process.env.OWX_SESSION_ID = "sess-stop-ralph-replay";
      const payload = {
        hook_event_name: "Stop",
        cwd,
  ...
  it("lets dispatcher dedupe identical native stop hook replays after Stop payload normalization", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-hook-dedupe-"));
    const previousOmxSessionId = process.env.OWX_SESSION_ID;
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      process.env.OWX_SESSION_ID = "sess-stop-ralph-hook-dedupe";
      const payload = {
        hook_event_name: "Stop",
        cwd,
        session_id: "sess-stop-ralph-hook-dedupe",
  ...
  it("preserves per-turn native stop hook delivery even when stop_hook_active remains true", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-ralph-hook-refire-"));
    const previousOmxSessionId = process.env.OWX_SESSION_ID;
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      process.env.OWX_SESSION_ID = "sess-stop-ralph-hook-refire";
      const payload = {
        hook_event_name: "Stop",
        cwd,
        session_id: "sess-stop-ralph-hook-refire",
  ...
  it("dedupes native stop hook replay across owner launch SessionStart reconciliation drift", async () => {
    ...
    try {
      ...
      await writeJson(join(stateDir, "sessions", "owx-canonical", "ralph-state.json"), {
        active: true,
        current_phase: "executing",
        session_id: "owx-canonical",
      });
  ...
  it("clears stale root skill-active state when current session ralplan is terminal", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-stale-root-skill-terminal-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      const sessionId = "sess-stop-terminal-ralplan";
      await mkdir(join(stateDir, "sessions", sessionId), { recursive: true });
  ...
  it("preserves legitimate session-scoped ultrawork blocking while reconciling root skill-active state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-active-root-skill-session-mode-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("reconciles stale root skill-active state under OWX_ROOT boxed state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-boxed-source-"));
    const owxRoot = await mkdtemp(join(tmpdir(), "owx-native-hook-stop-boxed-root-"));
    const previousOmxRoot = process.env.OWX_ROOT;
    try {
      process.env.OWX_ROOT = owxRoot;
      ...
      await rm(cwd, { recursive: true, force: true });
      await rm(owxRoot, { recursive: true, force: true });
    }
  });
  ...
  it("blocks implementation writes while ralplan is active without execution handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-pretool-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks implementation writes while Autopilot is supervising ralplan without handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-pretool-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("ignores stale Autopilot ralplan skill mirrors after detail state leaves planning", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-stale-ralplan-mirror-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows explicit blank Autopilot detail phase to use a ralplan skill mirror", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-blank-phase-mirror-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block implementation writes from Autopilot ralplan detail state without canonical skill state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-no-canonical-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows implementation writes when terminal Autopilot run-state shadows stale supervised ralplan state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-terminal-pretool-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks bash implementation writes while Autopilot is supervising ralplan without handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-pretool-bash-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks implementation writes when ralplan and Autopilot ralplan are both active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-autopilot-mixed-planning-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks implementation writes while Autopilot is supervising replan without handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-replan-pretool-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks implementation writes when native Codex id maps to OWX Autopilot ralplan state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-native-map-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks bash implementation writes when native Codex id maps to OWX Autopilot ralplan state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-native-map-bash-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks standalone ralplan writes when native Codex id maps to OWX session state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-native-map-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("blocks deep-interview writes when native Codex id maps to OWX session state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-deep-interview-native-map-block-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows mapped ralplan planning artifact writes without execution handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-native-map-artifact-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows mapped implementation writes when explicit execution handoff is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-native-map-handoff-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows mapped implementation writes when terminal Autopilot run-state shadows stale supervised ralplan state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-autopilot-ralplan-native-map-terminal-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("does not block unrelated native Codex ids when current OWX session mapping does not match", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-native-map-unrelated-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
      ...
      const result = await dispatchCodexNativeHook(
        {
          ...
          tool_name: "Write",
          tool_input: { file_path: ".owx/plans/prd-issue-2603.md" },
        },
        { cwd },
      );
  ...
  it("allows bash planning artifact writes while ralplan is active without execution handoff", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-pretool-bash-artifact-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
  ...
  it("allows implementation writes when an explicit execution handoff is active", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-native-hook-ralplan-pretool-handoff-"));
    try {
      const stateDir = join(cwd, ".owx", "state");
...
describe("codex native hook triage integration", () => {
  ...
  it("does not inject triage advisory for $ralplan keyword prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-keyword-ralplan-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-kw-ralplan-1", "prompt-routing-state.json");
  ...
  it("does not activate workflow state for native subagent prompts even when canonical id is the child session", async () => {
    ...
    try {
      ...
      const result = await dispatchCodexNativeHook(
        {
          ...
          prompt: [
            "Read-only review only. Do not edit files. Do not inspect/mutate OWX state/hooks.",
      ...
      assert.equal(
        ...
        false,
        "subagent tracking must not leak into the source worktree when OWX_ROOT is boxed",
      );
    } finally {
  ...
  it("does not inject triage advisory for autopilot keyword prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-keyword-autopilot-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("makes bare autopilot command activation observable in state and prompt guidance", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-autopilot-bare-observable-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
  ...
  it("injects HEAVY advisory and writes prompt-routing-state for a multi-step goal prompt", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-heavy-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-heavy-1", "prompt-routing-state.json");
  ...
  it("injects LIGHT/explore advisory and writes state for a question-shaped prompt", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-light-explore-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-explore-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), true);
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string };
        suppress_followup?: boolean;
  ...
  it("injects LIGHT/executor advisory and writes state for a narrow edit-shaped prompt", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-light-executor-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-executor-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), true);
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string };
      };
  ...
  it("injects LIGHT/designer advisory and writes state for a visual/style prompt", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-light-designer-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-designer-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), true);
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string };
      };
  ...
  it("injects LIGHT/researcher advisory and writes state for an official-doc lookup prompt", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-light-researcher-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-researcher-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), true);
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
  ...
  it("routes Korean external lookup phrasing to researcher without treating it as workflow activation", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-light-researcher-ko-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-researcher-ko-1", "prompt-routing-state.json");
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string };
      };
      assert.equal(state.last_triage?.lane, "LIGHT");
  ...
  it("routes official-doc question prompts to researcher instead of explore", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-question-researcher-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("routes endpoint-shaped official-doc lookups to researcher instead of local explore", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-endpoint-researcher-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("routes dotted technology official-doc lookups to researcher instead of local explore", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-dotted-tech-researcher-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("routes URL-shaped official-doc lookups with repo paths to researcher instead of local routes", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-url-path-researcher-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("keeps implementation-shaped official-doc prompts on HEAVY instead of researcher", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-researcher-implementation-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("keeps planning-shaped official-doc prompts on HEAVY instead of researcher", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-researcher-planning-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("keeps local source lookup prompts off researcher", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-local-source-explore-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-local-source-1", "prompt-routing-state.json");
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string };
      };
      assert.equal(state.last_triage?.lane, "LIGHT");
  ...
  it("keeps anchored local API usage prompts on executor instead of researcher", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-local-api-executor-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-local-api-1", "prompt-routing-state.json");
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string };
      };
      assert.equal(state.last_triage?.lane, "LIGHT");
  ...
  it("keeps project-scoped local API usage prompts on explore instead of researcher", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-project-api-explore-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-project-api-1", "prompt-routing-state.json");
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string; reason?: string };
      };
      assert.equal(state.last_triage?.lane, "LIGHT");
  ...
  it("keeps repository changelog lookup prompts on explore despite generic docs terms", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-repo-changelog-explore-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-repo-changelog-1", "prompt-routing-state.json");
      const state = JSON.parse(await readFile(stateFile, "utf-8")) as {
        last_triage?: { lane?: string; destination?: string; reason?: string };
      };
      assert.equal(state.last_triage?.lane, "LIGHT");
  ...
  it("routes anchored read-only questions through explore before executor", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-anchored-question-explore-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("produces no triage advisory and no state for trivial greeting prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-pass-hello-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-pass-hello-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("produces no triage advisory and no state for ambiguous short prompts", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-pass-short-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-pass-short-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("suppresses HEAVY triage re-injection on a short follow-up in the same session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-suppress-heavy-"));
  ...
  it("suppresses LIGHT/explore triage re-injection on a short follow-up in the same session", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-suppress-explore-"));
  ...
  it("still applies triage on turn 2 when turn 1 was a PASS with no state written", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-pass-then-light-"));
  ...
  it("produces no triage advisory when prompt contains 'just chat' opt-out", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-optout-chat-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-optout-chat-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("produces no triage advisory when prompt contains 'no workflow' opt-out", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-optout-noworkflow-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
  ...
  it("keyword on turn 2 suppresses triage and writes no triage state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-kw-followup-"));
    ...
    try {
      ...
      const triageState = join(cwd, ".owx", "state", "sessions", sessionId, "prompt-routing-state.json");
  ...
  it("produces no triage advisory and no state when triage is disabled in config", async () => {
    const tmpHome = await mkdtemp(join(tmpdir(), "owx-triage-config-disabled-home-"));
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-config-disabled-cwd-"));
    try {
      // Write a .owx-config.json in the fake CODEX_HOME that disables triage
      await writeJson(join(tmpHome, ".owx-config.json"), {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-disabled-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), false);
    } finally {
  ...
  it("keeps triage default-enabled when config omits promptRouting.triage.enabled", async () => {
    const tmpHome = await mkdtemp(join(tmpdir(), "owx-triage-config-omitted-home-"));
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-config-omitted-cwd-"));
    ...
    try {
      ...
      const stateFile = join(cwd, ".owx", "state", "sessions", "triage-defaulted-1", "prompt-routing-state.json");
      assert.equal(existsSync(stateFile), true);
    } finally {
  ...
  it("does not suppress a short anchored follow-up that is a new request", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-short-new-request-"));
  ...
  it("skips triage state persistence for malformed explicit session ids without writing root state", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "owx-triage-invalid-session-"));
    try {
      await mkdir(join(cwd, ".owx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook(
        {
