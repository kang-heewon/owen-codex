#!/usr/bin/env bash
#
# OWX Tmux Claude Workers Demo Script
#
...
#   OWX_TEAM_WORKER_LAUNCH_MODE     Worker launch mode (default: interactive)
#   OWX_TEAM_WORKER_LAUNCH_ARGS     Arguments passed to Claude CLI
#
# Example:
...
OWX_TEAM_WORKER_LAUNCH_MODE="${OWX_TEAM_WORKER_LAUNCH_MODE:-interactive}"
...
OWX_TEAM_WORKER_CLI="${OWX_TEAM_WORKER_CLI:-auto}"
OWX_TEAM_WORKER_CLI_MAP="${OWX_TEAM_WORKER_CLI_MAP:-$(build_claude_cli_map "$WORKER_COUNT")}"

TEAM_STARTED=0
cleanup() {
  if ((TEAM_STARTED == 1)); then
    echo "[cleanup] shutting down team: $TEAM_NAME"
    owx team shutdown "$TEAM_NAME" >/dev/null 2>&1 || true
    echo "[cleanup] cleaning state for team: $TEAM_NAME"
    owx team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT

echo "== OWX Tmux Claude Workers Demo v${SCRIPT_VERSION} =="
echo "TEAM_TASK=$TEAM_TASK"
echo "TEAM_NAME=$TEAM_NAME"
echo "WORKER_COUNT=$WORKER_COUNT"
echo "OWX_TEAM_WORKER_CLI=$OWX_TEAM_WORKER_CLI"
echo "OWX_TEAM_WORKER_CLI_MAP=$OWX_TEAM_WORKER_CLI_MAP"
echo ""
echo "This demo showcases Claude Code CLI workers in tmux panes"
echo "coordinated through the OWX team orchestration system."
echo ""

echo "[1/10] Starting team with ${WORKER_COUNT} Claude workers..."
owx team "${WORKER_COUNT}:executor" "$TEAM_TASK"
TEAM_STARTED=1
echo ""

echo "[2/10] Checking team status..."
owx team status "$TEAM_NAME"
echo ""
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  CREATE_JSON="$(owx team api create-task --input "$CREATE_INPUT" --json)"
...
owx team api list-tasks --input "$LIST_INPUT" --json | jq -r '.data.tasks[] | "  Task \(.id): \(.subject) [\(.status)]"'
echo ""
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  TASK_ID="$(owx team api list-tasks --input "$(jq -nc --arg team "$TEAM_NAME" '{team_name:$team}')" --json | \
  ...
  if [[ -n "$TASK_ID" ]]; then
    ...
    CLAIM_JSON="$(owx team api claim-task --input "$CLAIM_INPUT" --json)"
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  TASK_INFO="$(owx team api list-tasks --input "$(jq -nc --arg team "$TEAM_NAME" '{team_name:$team}')" --json | \
  ...
  if [[ -n "$TASK_INFO" ]]; then
    ...
    owx team api transition-task-status --input "$TRANSITION_INPUT" --json >/dev/null
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  owx team api send-message --input "$SEND_INPUT" --json >/dev/null
...
for i in $(seq 1 "$WORKER_COUNT"); do
  ...
  MAILBOX_JSON="$(owx team api mailbox-list --input "$MAILBOX_INPUT" --json)"
...
BROADCAST_RESULT="$(owx team api broadcast --input "$BROADCAST_INPUT" --json)"
...
SUMMARY_JSON="$(owx team api get-summary --input "$SUMMARY_INPUT" --json)"
...
owx team shutdown "$TEAM_NAME"
owx team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json >/dev/null
TEAM_STARTED=0
echo ""
...
echo ""
echo "The Claude workers were coordinated through the OWX"
