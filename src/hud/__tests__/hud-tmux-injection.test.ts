...
describe('VULNERABILITY – old string-interpolation approach', () => {
   ...
   *   const cmd  = `node ${owxBin} hud --watch${presetArg}`;
   *   execSync(`tmux split-window -v -l 4 -c "${cwd}" '${cmd}'`);
   */
  function buildOldCommand(cwd: string, owxBin: string, preset?: string): string {
  ...
  it('cwd containing $() is injectable via double-quote context', () => {
    ...
    const shellCmd = buildOldCommand(maliciousCwd, '/usr/bin/owx.js');
  ...
  it("owxBin containing single quote breaks out of the '-quoted command", () => {
    const maliciousOmx = "/tmp/it';touch /tmp/pwned;echo '/owx.js";
    ...
    assert.ok(
      shellCmd.includes("'node /tmp/it';touch /tmp/pwned;echo '/owx.js"),
...
describe('buildTmuxSplitArgs – shell injection hardening', () => {
  it('produces correct argv for normal inputs', () => {
    const args = buildTmuxSplitArgs('/home/user/project', '/usr/local/bin/owx.js');
    assert.deepEqual(args, [
      'split-window', '-v', '-l', String(HUD_TMUX_HEIGHT_LINES),
      // split height should come from shared HUD constants
      '-c', '/home/user/project',
      `exec env OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/local/bin/owx.js' hud --watch`,
    ]);
  });
  ...
  it('cwd is an isolated array element – never shell-interpreted', () => {
    const maliciousCwd = '/tmp/foo"$(touch /tmp/pwned)"bar';
    const args = buildTmuxSplitArgs(maliciousCwd, '/usr/bin/owx.js');
  ...
  it("owxBin with single quote is properly escaped in command string", () => {
    const maliciousOmx = "/tmp/it's/owx.js";
    ...
    assert.equal(cmd, `exec env OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/it'\\''s/owx.js' hud --watch`);
  });

  it('owxBin with $() is neutralised by single-quote wrapping', () => {
    const maliciousOmx = '/tmp/$(id)/owx.js';
    const args = buildTmuxSplitArgs('/home/user', maliciousOmx);
    const cmd = args[6];

    // Inside single quotes, $() is literal.
    assert.equal(cmd, `exec env OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/$(id)/owx.js' hud --watch`);
  });

  it('owxBin with backticks is neutralised by single-quote wrapping', () => {
    const maliciousOmx = '/tmp/`whoami`/owx.js';
    const args = buildTmuxSplitArgs('/home/user', maliciousOmx);
    const cmd = args[6];

    assert.equal(cmd, `exec env OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/\`whoami\`/owx.js' hud --watch`);
  });

  it("owxBin with ';command' breakout attempt is neutralised", () => {
    const maliciousOmx = "/tmp/x';touch /tmp/pwned;echo '/owx.js";
    const args = buildTmuxSplitArgs('/home/user', maliciousOmx);
    const cmd = args[6];
    ...
    //
    // Raw expected value: exec node '/tmp/x'\\'';touch /tmp/pwned;echo '\\''/owx.js' hud --watch
    assert.equal(
      cmd,
      `exec env OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/tmp/x'\\'';touch /tmp/pwned;echo '\\''/owx.js' hud --watch`,
    );
  ...
  it('preset is appended safely', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/owx.js', 'minimal');
    const cmd = args[6];
    assert.ok(cmd.endsWith('--preset=minimal'));
  });

  it('absent preset produces no --preset flag', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/owx.js');
    const cmd = args[6];
  ...
  it('invalid preset is dropped (defense in depth)', () => {
    ...
    const cmd = args[6];
    assert.equal(cmd, `exec env OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/bin/owx.js' hud --watch`);
    assert.ok(!cmd.includes('--preset='));
  });

  it('prepends OWX_SESSION_ID when provided', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/owx.js', 'focused', 'sess-managed');
    const cmd = args[6];
    assert.equal(cmd, `exec env OWX_SESSION_ID='sess-managed' OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/bin/owx.js' hud --watch --preset=focused`);
  });

  it('forwards OWX_ROOT with OWX_SESSION_ID using shell-safe quoting', () => {
    const args = buildTmuxSplitArgs(
      '/home/user',
      '/usr/bin/owx.js',
      'focused',
    ...
    assert.equal(
      cmd,
      `exec env OWX_SESSION_ID='sess managed' OWX_TMUX_HUD_OWNER=1 OWX_ROOT='/tmp/boxed root/it'\\''s/$(literal)' ${runtimePrefix} '/usr/bin/owx.js' hud --watch --preset=focused`,
    );
  });

  it('omits OWX_ROOT when unset while preserving existing OWX_SESSION_ID behavior', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/owx.js', undefined, 'sess-managed');
    const cmd = args[6];
    assert.equal(cmd, `exec env OWX_SESSION_ID='sess-managed' OWX_TMUX_HUD_OWNER=1 ${runtimePrefix} '/usr/bin/owx.js' hud --watch`);
    assert.doesNotMatch(cmd, /OWX_ROOT=/);
  });

  it('tags tmux-launched HUD panes with the emitting leader pane', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/owx.js', undefined, 'sess-managed', undefined, '%leader');
    const cmd = args.at(-1) ?? '';
    assert.deepEqual(args.slice(0, 7), ['split-window', '-v', '-l', String(HUD_TMUX_HEIGHT_LINES), '-t', '%leader', '-c']);
    assert.equal(cmd, `exec env OWX_SESSION_ID='sess-managed' OWX_TMUX_HUD_OWNER=1 OWX_TMUX_HUD_LEADER_PANE='%leader' ${runtimePrefix} '/usr/bin/owx.js' hud --watch`);
  });
});

describe('buildHudWatchCommand', () => {
  it('forwards OWX_ROOT and OWX_TMUX_HUD_OWNER for reconciled HUD panes with shell-safe quoting', () => {
    ...
    assert.equal(
      cmd,
      `exec env OWX_SESSION_ID='sess managed' OWX_TMUX_HUD_OWNER='1' OWX_ROOT='/tmp/boxed root/it'\\''s/$(literal)' ${runtimePrefix} '/usr/bin/owx.js' hud --watch --preset=minimal`,
    );
  });

  it('always emits OWX_TMUX_HUD_OWNER even when OWX_SESSION_ID and OWX_ROOT are unset', () => {
    const cmd = buildHudWatchCommand('/usr/bin/owx.js');
    assert.equal(cmd, `exec env OWX_TMUX_HUD_OWNER='1' ${runtimePrefix} '/usr/bin/owx.js' hud --watch`);
  });
});
