...
async function writePrdJson(
  ...
  await writeFile(join(cwd, '.owx', 'prd.json'), JSON.stringify(payload, null, 2));
}

describe('owx ralph --prd smoke gate', () => {
  it('aborts before Codex launch when .owx/prd.json is missing', async () => {
    const cwd = await initRepo('owx-ralph-prd-smoke-');
    ...
    try {
      ...
      assert.match(`${result.stderr}\n${result.stdout}`, /Missing required PRD\.json at \.owx\/prd\.json/);
  ...
  it('launches Codex exactly once when .owx/prd.json is valid', async () => {
    const cwd = await initRepo('owx-ralph-prd-smoke-');
    const home = join(cwd, 'home');
    const fakeBin = join(cwd, 'bin');
    const launchLog = join(cwd, 'codex-launch.log');
    try {
      ...
      const ralphState = JSON.parse(await readFile(join(cwd, '.owx', 'state', 'ralph-state.json'), 'utf-8')) as Record<string, unknown>;
      ...
      const instructions = await readFile(join(cwd, '.owx', 'ralph', 'session-instructions.md'), 'utf-8');
