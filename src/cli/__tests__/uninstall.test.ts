...
/** Build a realistic OWX config.toml for testing */
function buildOmxConfig(): string {
  return [
    ...
    'developer_instructions = "You have owen-codex installed."',
    '',
    '[features]',
    'multi_agent = true',
    ...
    '[mcp_servers.owx_code_intel]',
    'command = "node"',
    'args = ["/path/to/code-intel-server.js"]',
    'enabled = true',
    'startup_timeout_sec = 10',
    '',
    '# OWX Trace MCP Server',
    '[mcp_servers.owx_trace]',
    'command = "node"',
...
/** Build a config with OWX entries mixed with user entries */
...
describe('owx uninstall', () => {
  it('removes OWX block from config.toml with --dry-run', async () => {
  ...
  it('removes OWX block from config.toml', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
      ...
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.match(res.stdout, /Removed OWX configuration block/);

      const config = await readFile(join(codexDir, 'config.toml'), 'utf-8');
      assert.doesNotMatch(config, /owen-codex \(OWX\) Configuration/);
      assert.doesNotMatch(config, /owx_state/);
      assert.doesNotMatch(config, /owx_memory/);
      assert.doesNotMatch(config, /owx_code_intel/);
      assert.doesNotMatch(config, /owx_trace/);
      assert.doesNotMatch(config, /owx_wiki/);
  ...
  it('does not restore stale OWX dispatcher metadata as notify', async () => {
    ...
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
      const metadataPath = join(codexDir, '.owx', 'notify-dispatch.json');
  ...
  it('preserves user config entries when removing OWX', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
  ...
  it('preserves user hooks while removing OWX-managed wrappers', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
  ...
  it('removes unchanged OWX-seeded model/context keys during uninstall', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
  ...
  it('--keep-config skips config.toml cleanup', async () => {
    ...
    try {
      ...
      const config = await readFile(join(codexDir, 'config.toml'), 'utf-8');
      assert.match(config, /owen-codex \(OWX\) Configuration/);
      assert.match(config, /owx_state/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it('--purge removes .owx/ cache directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      await mkdir(home, { recursive: true });
      // Create .owx/ directory with some files
      const owxDir = join(wd, '.owx');
      await mkdir(join(owxDir, 'state'), { recursive: true });
      await writeFile(join(owxDir, 'setup-scope.json'), JSON.stringify({ scope: 'user' }));
      await writeFile(join(owxDir, 'notepad.md'), '# notes');
      await writeFile(join(owxDir, 'state', 'ralph-state.json'), '{}');

      const res = runOmx(wd, ['uninstall', '--keep-config', '--purge'], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);
      assert.match(res.stdout, /\.owx\/ cache directory/);

      assert.equal(existsSync(owxDir), false, '.owx/ directory should be removed');
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('works with project scope', async () => {
    ...
    try {
      ...
      await writeFile(join(owxDir, 'setup-scope.json'), JSON.stringify({ scope: 'project' }));
      await writeFile(join(codexDir, 'config.toml'), buildOmxConfig());
      // Install a prompt
  ...
  it('shows summary of what was removed', async () => {
    ...
    try {
      ...
      assert.match(res.stdout, /MCP servers: owx_state, owx_memory, owx_code_intel, owx_trace, owx_wiki/);
  ...
  it('warns when overlapping legacy ~/.agents/skills remains after user-scope uninstall', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Warning: 1 overlapping skill names remain between .*\.codex[\\/]+skills and .*\.agents[\\/]+skills; 1 differ in SKILL\.md content\. owx uninstall only removes the active canonical skill root; archive or remove ~\/\.agents\/skills if Codex still shows duplicates/,
      );
      assert.equal(existsSync(canonicalHelp), false, 'canonical OWX skill should be removed');
  ...
  it('warns when a distinct legacy ~/.agents/skills root remains after user-scope uninstall', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Warning: legacy ~\/\.agents\/skills still exists \(1 skills\)\. owx uninstall does not remove that historical root automatically; archive or remove ~\/\.agents\/skills if Codex still shows stale or duplicate skills/,
      );
      assert.equal(existsSync(canonicalHelp), false, 'canonical OWX skill should be removed');
      assert.equal(existsSync(join(home, '.agents', 'skills')), true, 'legacy skill root should remain for manual cleanup');
    } finally {
  ...
  it('does not warn about legacy ~/.agents/skills when none exists', async () => {
    ...
    try {
      ...
      assert.doesNotMatch(res.stdout, /owx uninstall does not remove legacy ~\/\.agents\/skills/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('does not warn when legacy ~/.agents/skills is just a link to the canonical skills root', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-legacy-link-'));
    try {
      const home = join(wd, 'home');
      const codexDir = join(home, '.codex');
  ...
  it('--dry-run --purge does not actually remove .owx/ directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      await mkdir(home, { recursive: true });
      ...
      // .owx/ should still exist
      assert.equal(existsSync(owxDir), true, '.owx/ should NOT be removed in dry-run');
      assert.equal(existsSync(join(owxDir, 'notepad.md')), true);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('second uninstall run reports nothing to remove (idempotent)', async () => {
    ...
    try {
      ...
      assert.match(first.stdout, /Removed OWX configuration block/);
  ...
  it('does not delete user AGENTS.md that merely mentions owen-codex', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-uninstall-'));
    try {
      const home = join(wd, 'home');
      await mkdir(home, { recursive: true });
      const userAgentsMd = '# My Agents\n\nDo not use owen-codex for this project.\n';
  ...
  it('removes managed user-scope AGENTS.md from CODEX_HOME', async () => {
    ...
    try {
      ...
      await mkdir(join(wd, '.owx'), { recursive: true });
      await writeFile(join(wd, '.owx', 'setup-scope.json'), JSON.stringify({ scope: 'user' }));
      await writeFile(
  ...
  it('removes setup-scope.json and hud-config.json without --purge', async () => {
    ...
    try {
      ...
      await mkdir(owxDir, { recursive: true });
      await writeFile(join(owxDir, 'setup-scope.json'), JSON.stringify({ scope: 'user' }));
      await writeFile(join(owxDir, 'hud-config.json'), JSON.stringify({ preset: 'focused' }));
      await writeFile(join(owxDir, 'notepad.md'), '# keep this');

      const res = runOmx(wd, ['uninstall', '--keep-config'], { HOME: home });
      if (shouldSkipForSpawnPermissions(res.error)) return;
      assert.equal(res.status, 0, res.stderr || res.stdout);

      assert.equal(existsSync(join(owxDir, 'setup-scope.json')), false);
      assert.equal(existsSync(join(owxDir, 'hud-config.json')), false);
...
describe('stripOmxFeatureFlags', () => {
  it('removes OWX feature flags and preserves user flags', async () => {
