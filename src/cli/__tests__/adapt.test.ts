...
describe("adaptCommand", () => {
	...
	it("emits compact JSON envelopes when --json is set", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "owx-adapt-cli-"));
		const out: string[] = [];
		try {
	...
	it("rejects --write outside init", async () => {
		await assert.rejects(
			...
			/only supported with owx adapt <target> init/i,
		);
	});

	it("reports OpenClaw local observation details in JSON status output", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "owx-adapt-cli-openclaw-"));
		const out: string[] = [];
		try {
