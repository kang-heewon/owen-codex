...
describe('resolveSparkShellBinaryPath', () => {
  it('prefers OWX_SPARKSHELL_BIN override', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-override-'));
    try {
      const binary = join(cwd, 'bin', 'custom-sparkshell');
      assert.equal(
        resolveSparkShellBinaryPath({
          cwd,
          env: { OWX_SPARKSHELL_BIN: './bin/custom-sparkshell' },
          packageRoot: '/unused',
        }),
        binary,
      );
  ...
  it('falls back from packaged binary to repo-local build artifact', () => {
    ...
    const packaged = join(packageRoot, 'bin', 'native', `${process.platform}-${process.arch}`, process.platform === 'win32' ? 'owx-sparkshell.exe' : 'owx-sparkshell');
  ...
  it('checks Linux musl packaged paths before glibc and legacy paths', () => {
    assert.deepEqual(
      ...
      [
        '/repo/bin/native/linux-x64-musl/owx-sparkshell',
        '/repo/bin/native/linux-x64-glibc/owx-sparkshell',
        '/repo/bin/native/linux-x64/owx-sparkshell',
      ],
    );
  });
  ...
  it('tries Linux musl packaged binaries before glibc fallbacks', () => {
    const packageRoot = '/repo';
    const seen: string[] = [];
    const glibcPath = '/repo/bin/native/linux-x64-glibc/owx-sparkshell';
  ...
  it('hydrates a native binary when packaged and repo-local binaries are absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-hydrated-'));
    try {
      ...
      const stagedBinary = join(stagingDir, process.platform === 'win32' ? 'owx-sparkshell.exe' : 'owx-sparkshell');
      ...
      const archiveName = process.platform === 'win32'
        ? 'owx-sparkshell-x86_64-pc-windows-msvc.zip'
      ...
      const buildArchive = process.platform === 'win32'
        ...
        : spawnSync('tar', ['-czf', archivePath, '-C', stagingDir, 'owx-sparkshell'], { encoding: 'utf-8' });
      ...
      try {
        await writeFile(join(assetRoot, 'native-release-manifest.json'), JSON.stringify({
          ...
          assets: [{
            ...
            arch: 'x64',
            archive: archiveName,
            binary: process.platform === 'win32' ? 'owx-sparkshell.exe' : 'owx-sparkshell',
            binary_path: process.platform === 'win32' ? 'owx-sparkshell.exe' : 'owx-sparkshell',
            sha256: checksum,
  ...
  it('falls back cleanly when hydration manifest is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-hydration-missing-'));
    try {
...
describe('runSparkShellBinary', () => {
  it('passes argv directly to the native sidecar', () => {
    ...
    runSparkShellBinary('/fake/owx-sparkshell', ['git', 'diff --stat', 'a|b'], {
      cwd: '/tmp/example',
      env: { TEST_ENV: '1' },
    ...
    assert.deepEqual(invoked, {
      binaryPath: '/fake/owx-sparkshell',
      args: ['git', 'diff --stat', 'a|b'],
      stdio: ['ignore', 'pipe', 'pipe'],
    });
  });

  it('merges .owx-config.json env overrides behind explicit shell env', async () => {
    const codexHome = await mkdtemp(join(tmpdir(), 'owx-sparkshell-config-env-'));
    await writeFile(join(codexHome, '.owx-config.json'), JSON.stringify({
      env: {
        OWX_DEFAULT_FRONTIER_MODEL: 'frontier-local',
        OWX_DEFAULT_STANDARD_MODEL: 'standard-local',
        OWX_DEFAULT_SPARK_MODEL: 'spark-local',
        OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE: '/config/sparkshell-instructions.md',
      },
    }));

    try {
      let invokedEnv: NodeJS.ProcessEnv | undefined;
      runSparkShellBinary('/fake/owx-sparkshell', ['git', 'status'], {
        cwd: codexHome,
        env: {
          CODEX_HOME: codexHome,
          OWX_DEFAULT_FRONTIER_MODEL: 'frontier-shell',
        },
      ...
      assert.equal(invokedEnv?.OWX_DEFAULT_FRONTIER_MODEL, 'frontier-shell');
      assert.equal(invokedEnv?.OWX_DEFAULT_STANDARD_MODEL, 'standard-local');
      assert.equal(invokedEnv?.OWX_DEFAULT_SPARK_MODEL, 'spark-local');
      assert.equal(invokedEnv?.OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE, '/config/sparkshell-instructions.md');
    } finally {
  ...
  it('defaults to packaged lightweight instructions outside the role prompts directory', () => {
    ...
    assert.match(
      invokedEnv?.OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE || '',
  ...
  it('detects GLIBC symbol version failures from the native loader', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: "owx-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39' not found\n",
        status: 1,
        signal: null,
      }),
      true,
  ...
  it('ignores non-compatibility stderr failures', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: 'owx sparkshell: summary unavailable (tmux failed)\n',
        status: 1,
        signal: null,
      }),
      false,
...
describe('owx sparkshell', () => {
  it('includes sparkshell in top-level help output', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-help-'));
    try {
      const result = runOmx(cwd, ['--help']);
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /owx explore\s+DEPRECATED compatibility command; use normal repo inspection or owx sparkshell/);
      assert.match(result.stdout, /owx sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /owx sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
  ...
  it('prints sparkshell usage when invoked with --help', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-subhelp-'));
    try {
      const result = runOmx(cwd, ['sparkshell', '--help']);
      if (shouldSkipForSpawnPermissions(result.error)) return;

      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage: owx sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /or: owx sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
      assert.match(result.stdout, /OWX_SPARKSHELL_BIN overrides the native binary/);
      assert.match(result.stdout, /OWX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE overrides packaged summary instructions/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('preserves child stdout, stderr, and exit code through the JS bridge', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-bridge-'));
    try {
      const binDir = join(cwd, 'bin');
      const stubPath = join(binDir, process.platform === 'win32' ? 'owx-sparkshell.cmd' : 'owx-sparkshell');
  ...
  it('falls back to raw execution when the packaged native binary is GLIBC-incompatible', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-glibc-fallback-'));
    try {
      const testDir = dirname(fileURLToPath(import.meta.url));
      const repoRoot = join(testDir, '..', '..', '..');
      ...
      const binDir = join(cacheDir, packageJson.version, `${process.platform}-${process.arch}`, 'owx-sparkshell');
      await mkdir(binDir, { recursive: true });
      const stubPath = join(binDir, process.platform === 'win32' ? 'owx-sparkshell.exe' : 'owx-sparkshell');
      await writeFile(
        stubPath,
        process.platform === 'win32'
          ? '@echo off\r\n>&2 echo owx-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.39\' not found\r\nexit /b 1\r\n'
          : "#!/bin/sh\necho \"owx-sparkshell: /lib/x86_64-linux-gnu/libc.so.6: version \\`GLIBC_2.39' not found\" 1>&2\nexit 1\n",
      );
  ...
  it('fails clearly when the configured native binary path does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-sparkshell-missing-'));
    try {
      const missingBinary = join(cwd, 'bin', 'does-not-exist');
      const result = runOmx(cwd, ['sparkshell', 'ls'], {
        OWX_SPARKSHELL_BIN: missingBinary,
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
