...
const LOCK_ROOT = join(process.cwd(), '.owx', 'test-locks');
...
export async function withPackagedExploreHarnessHidden<T>(fn: () => Promise<T>): Promise<T> {
  return withPackagedExploreHarnessLock(async () => {
    ...
    const packagedBinary = join(packageBinDir, process.platform === 'win32' ? 'owx-explore-harness.exe' : 'owx-explore-harness');
    const packagedMeta = join(packageBinDir, 'owx-explore-harness.meta.json');
