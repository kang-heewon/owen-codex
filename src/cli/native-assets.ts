...
export type NativeProduct = 'owx-explore-harness' | 'owx-sparkshell' | 'owx-api';
...
const NATIVE_AUTO_FETCH_ENV = 'OWX_NATIVE_AUTO_FETCH';
const NATIVE_MANIFEST_URL_ENV = 'OWX_NATIVE_MANIFEST_URL';
const NATIVE_RELEASE_BASE_URL_ENV = 'OWX_NATIVE_RELEASE_BASE_URL';
const NATIVE_CACHE_DIR_ENV = 'OWX_NATIVE_CACHE_DIR';
export const EXPLORE_BIN_ENV = 'OWX_EXPLORE_BIN';
export const SPARKSHELL_BIN_ENV = 'OWX_SPARKSHELL_BIN';
export const API_BIN_ENV = 'OWX_API_BIN';
...
export function resolveNativeCacheRoot(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (process.platform === 'win32') {
    return resolve(env.LOCALAPPDATA?.trim() || join(homedir(), 'AppData', 'Local'), 'owen-codex', 'native');
  }
  return resolve(env.XDG_CACHE_HOME?.trim() || join(homedir(), '.cache'), 'owen-codex', 'native');
}
