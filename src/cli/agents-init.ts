...
export const AGENTS_INIT_USAGE = [
  "Usage: owx agents-init [path] [--dry-run] [--force] [--verbose]",
  "       owx deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
const MANAGED_MARKER = "<!-- OWX:AGENTS-INIT:MANAGED -->";
const MANUAL_START = "<!-- OWX:AGENTS-INIT:MANUAL:START -->";
const MANUAL_END = "<!-- OWX:AGENTS-INIT:MANUAL:END -->";
...
export async function agentsInit(
  ...
  console.log("owen-codex AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active owx session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by owx agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active owx session is running.\n",
  );
