...
const HELP = `
Usage:
  owx hooks init       Create .owx/hooks/sample-plugin.mjs scaffold
  owx hooks status     Show plugin directory + discovered plugins
  owx hooks validate   Validate plugin exports/signatures
...
async function initHooks(): Promise<void> {
  ...
  console.log('Plugins are enabled by default. Disable with OWX_HOOK_PLUGINS=0.');
}
...
async function statusHooks(): Promise<void> {
  ...
  console.log(
    `Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with OWX_HOOK_PLUGINS=0)'}`,
  );
...
async function validateHooks(): Promise<void> {
  ...
  const plugins = await discoverHookPlugins(cwd);
  if (plugins.length === 0) {
    console.log('No plugins found. Run: owx hooks init');
    return;
  }
...
async function testHooks(): Promise<void> {
  ...
  const event = buildHookEvent('turn-complete', {
    source: 'native',
    context: {
      reason: 'owx-hooks-test',
    },
    session_id: 'owx-hooks-test',
  ...
  const logPath = join(cwd, '.owx', 'logs', `hooks-${new Date().toISOString().split('T')[0]}.jsonl`);
