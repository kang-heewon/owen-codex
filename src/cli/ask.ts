...
export const ASK_USAGE = [
  'Usage: owx ask <claude|gemini> <question or task>',
  '   or: owx ask <claude|gemini> -p "<prompt>"',
  '   or: owx ask claude --print "<prompt>"',
  '   or: owx ask gemini --prompt "<prompt>"',
  '   or: owx ask <claude|gemini> --agent-prompt <role> "<prompt>"',
  '   or: owx ask <claude|gemini> --agent-prompt=<role> --prompt "<prompt>"',
].join('\n');
...
const ASK_ADVISOR_SCRIPT_ENV = 'OWX_ASK_ADVISOR_SCRIPT';
...
function resolveAskPromptsDir(cwd: string, env: NodeJS.ProcessEnv = process.env): string {
  ...
  try {
    const scopePath = join(cwd, '.owx', 'setup-scope.json');
...
async function resolveAgentPromptContent(
  ...
  if (!existsSync(promptsDir)) {
    throw new Error(`[ask] prompts directory not found: ${promptsDir}. Run "owx setup" to install prompts.`);
  }
