...
const AUTORESEARCH_WORKTREE_EXCLUDES = ['results.tsv', 'run.log', 'node_modules', '.owx/'];
...
function activeRunStateFile(projectRoot: string): string {
  return join(projectRoot, '.owx', 'state', 'autoresearch-state.json');
}
...
export function buildAutoresearchInstructions(
  ...
  const codeGraphInstructions = context.toolContext
    ? renderCodeGraphInstructions(context.toolContext)
    : '';
  return [
    '# OWX Autoresearch Supervisor Instructions',
    '',
    ...
    ...(codeGraphInstructions ? ['', codeGraphInstructions] : []),
    '',
...
export async function loadAutoresearchRunManifest(projectRoot: string, runId: string): Promise<AutoresearchRunManifest> {
  const manifestFile = join(projectRoot, '.owx', 'logs', 'autoresearch', runId, 'manifest.json');
...
export async function prepareAutoresearchRuntime(
  ...
  const runDir = join(projectRoot, '.owx', 'logs', 'autoresearch', runId);
