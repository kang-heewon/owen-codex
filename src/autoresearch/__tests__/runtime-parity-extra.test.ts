...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'owx-autoresearch-parity-extra-'));
...
describe('autoresearch runtime parity extras', () => {
  it('treats allowed runtime files as reset-safe and blocks unrelated dirt', async () => {
    ...
    try {
      ...
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t020000z');
  ...
  it('rejects concurrent fresh runs via the repo-root active-run lock', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePathA = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t030000z');
      ...
      const worktreePathB = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t030500z');
  ...
  it('resumes a running manifest and rejects missing worktrees', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t040000z');
      ...
      const statePath = join(repo, '.owx', 'state', 'autoresearch-state.json');
      const idleState = {
        schema_version: 1,
        active: false,
  ...
  it('resume rejects terminal manifests', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t050000z');
      ...
      await writeFile(join(repo, '.owx', 'state', 'autoresearch-state.json'), `${JSON.stringify({
        schema_version: 1,
        active: false,
        run_id: runtime.runId,
        mission_slug: contract.missionSlug,
  ...
  it('records noop and abort candidate branches explicitly', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t060000z');
  ...
  it('rejects candidate integrity mismatches and missing candidate artifacts with actionable failure state', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t070000z');
      ...
      const secondWorktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t071000z');
      ...
      const thirdWorktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t072000z');
  ...
  it('handles interrupted, evaluator failure, and evaluator parse-error branches', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t080000z');
