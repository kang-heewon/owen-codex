...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'owx-autoresearch-runtime-'));
...
describe('autoresearch runtime', () => {
  it('builds bootstrap instructions with mission, sandbox, and evaluator contract', async () => {
    ...
    try {
      ...
      const instructions = buildAutoresearchInstructions(contract, { runId: 'missions-demo-20260314t000000z', iteration: 1, baselineCommit: 'abc1234', lastKeptCommit: 'abc1234', resultsFile: 'results.tsv', candidateFile: '.owx/logs/autoresearch/missions-demo-20260314t000000z/candidate.json', keepPolicy: 'score_improvement' });
  ...
  it('allows untracked .owx runtime files when checking reset safety', async () => {
    const repo = await initRepo();
    try {
      await mkdir(join(repo, '.owx', 'logs'), { recursive: true });
      await mkdir(join(repo, '.owx', 'state'), { recursive: true });
      await writeFile(join(repo, '.owx', 'logs', 'hooks-2026-03-15.jsonl'), '{}\n', 'utf-8');
      await writeFile(join(repo, '.owx', 'metrics.json'), '{}\n', 'utf-8');
      await writeFile(join(repo, '.owx', 'state', 'hud-state.json'), '{}\n', 'utf-8');
  ...
  it('prepares runtime artifacts and persists autoresearch mode state', async () => {
    ...
    try {
      ...
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t000000z');
...
describe('autoresearch parity decisions', () => {
  it('keeps improved candidates and resets discarded candidates back to the last kept commit', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.owx', 'worktrees', 'autoresearch-missions-demo-20260314t010000z');
