/**
 * TS Runtime Bridge — thin wrapper over owx-runtime binary.
 *
 * All semantic state mutations route through `execCommand()`.
 * All state queries read Rust-authored compatibility JSON files.
 * Set OWX_RUNTIME_BRIDGE=0 to disable bridge (fallback to TS-direct).
 */
...
/**
 * Raised when the owx-runtime binary returns output that fails JSON decoding.
 *
...
export function resolveRuntimeBinaryPath(options: RuntimeBinaryDiscoveryOptions = {}): string {
  ...
  const envOverride = process.env.OWX_RUNTIME_BINARY?.trim();
  if (envOverride) return envOverride;

  const workspaceDebug = options.debugPath ?? resolve(__bridge_dirname, '../../target/debug/owx-runtime');
  if (exists(workspaceDebug)) return workspaceDebug;

  const workspaceRelease = options.releasePath ?? resolve(__bridge_dirname, '../../target/release/owx-runtime');
  if (exists(workspaceRelease)) return workspaceRelease;

  return options.fallbackBinary ?? 'owx-runtime';
}
...
export class RuntimeBridge {
  ...
  constructor(options: { stateDir?: string; binaryPath?: string } = {}) {
    this.enabled = process.env.OWX_RUNTIME_BRIDGE !== '0';
    this.stateDir = options.stateDir;
    this.binaryPath = options.binaryPath ?? resolveRuntimeBinaryPath();
  }

  /** Whether the bridge is enabled (OWX_RUNTIME_BRIDGE != '0'). */
  isEnabled(): boolean {
    return this.enabled;
  }
  ...
  execCommand(cmd: RuntimeCommand, options?: { compact?: boolean }): RuntimeEvent {
    ...
    try {
      ...
      throw new RuntimeBridgeError(
        `owx-runtime exec returned non-JSON output for ${cmd.command}`,
  ...
  readSnapshot(): RuntimeSnapshot {
    ...
    try {
      ...
      throw new RuntimeBridgeError('owx-runtime snapshot returned non-JSON output', {
        command: 'snapshot',
  ...
  private validateSchemaOnce(): void {
    ...
    try {
      ...
      if (missing.length > 0) {
        throw new Error(
          `owx-runtime schema missing commands: ${missing.join(', ')}. ` +
  ...
  private run(args: string[]): string {
    try {
      ...
      throw new Error(`owx-runtime ${args[0]} failed: ${stderr}`);
    }
  }
}
...
export function isBridgeEnabled(): boolean {
  return process.env.OWX_RUNTIME_BRIDGE !== '0';
}
