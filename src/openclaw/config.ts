 ...
 *
 * Reads OpenClaw config from the notifications.openclaw key in ~/.codex/.owx-config.json.
 ...
 * Config file path can be overridden via OWX_OPENCLAW_CONFIG env var (points to a separate file).
 */
	...
	if (cliEnabled && cliAlias) {
		...
		applyHooks(
			...
			parseInstruction(
				...
				"OWX event {{event}} for {{projectPath}}",
			),
			"custom_cli_command",
		);
	}
...
export function inspectOpenClawConfig(
	...
	const activationGateEnabled = env.OWX_OPENCLAW === "1";
	const commandGateEnabled = env.OWX_OPENCLAW_COMMAND === "1";
	const envOverride = env.OWX_OPENCLAW_CONFIG?.trim();
	...
	if (!activationGateEnabled) {
		return {
			...
			warnings: [],
			detail: "OpenClaw is disabled locally because OWX_OPENCLAW=1 is not set.",
			config: null,
		};
	}
	...
	if (!configExists) {
		return {
			...
			warnings: [],
			detail: envOverride
				? `OWX_OPENCLAW_CONFIG points to ${configPath}, but the file does not exist.`
	...
	try {
		if (envOverride) {
			...
			if (!isValidOpenClawConfig(raw)) {
				return {
					...
					warnings: [],
					detail:
						"OWX_OPENCLAW_CONFIG is present but does not contain a valid OpenClaw config.",
					config: null,
				};
			}
			...
			return {
				...
				explicitOverridesAliases: false,
				warnings: [],
				detail: "OpenClaw config loaded from OWX_OPENCLAW_CONFIG.",
				config: raw,
			};
		}
		...
		if (!notifications) {
			return {
				...
				warnings: [],
				detail:
					"The OWX config file exists, but notifications are not configured.",
				config: null,
			};
		}
 ...
 *
 * Returns null when:
 * - OWX_OPENCLAW env var is not "1"
 * - Config file does not exist
 * - Config file is invalid JSON
 * - Config has enabled: false
 *
 * Config is read from:
 * 1. OWX_OPENCLAW_CONFIG env var path (separate file), if set
 * 2. notifications.openclaw key in ~/.codex/.owx-config.json
...
export function getOpenClawConfig(): OpenClawConfig | null {
	// Activation gate: only active when OWX_OPENCLAW=1
	if (process.env.OWX_OPENCLAW !== "1") {
		return null;
	}
	...
	try {
		const envOverride = process.env.OWX_OPENCLAW_CONFIG;

		if (envOverride) {
			// OWX_OPENCLAW_CONFIG points to a separate config file
		...
		// Primary: read from notifications block in .owx-config.json
		const owxConfigPath = defaultOmxConfigPath();
		if (!existsSync(owxConfigPath)) {
			_cachedConfig = undefined;
			return null;
		}

		const fullConfig = JSON.parse(
			readFileSync(owxConfigPath, "utf-8"),
