...
async function withTempTeamStateRoot<T>(
  ...
  const previousRoot = process.env.OWX_TEAM_STATE_ROOT;
  process.env.OWX_TEAM_STATE_ROOT = teamStateRoot;
  try {
    return await fn();
  } finally {
    if (previousRoot === undefined) delete process.env.OWX_TEAM_STATE_ROOT;
    else process.env.OWX_TEAM_STATE_ROOT = previousRoot;
  }
}
...
describe('rust runtime legacy-reader compatibility', () => {
  it('keeps team status on the manifest-authored compatibility view', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-rust-compat-team-'));
    try {
      ...
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'owx-team-legacy-rust-compat-team';
        manifest.workspace_mode = 'worktree';
        manifest.tmux_session = 'owx-team-rust-compat-team';

        await writeFile(configPath, `${JSON.stringify(config, null, 2)}\n`);
        await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

        const result = runOmx(wd, ['team', 'status', 'rust-compat-team', '--json'], { OWX_TEAM_STATE_ROOT: teamStateRoot });
  ...
  it('keeps doctor --team on the manifest-authored tmux session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-rust-compat-doctor-'));
    try {
      const teamStateRoot = join(wd, '.owx', 'state');
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'owx-team-legacy-rust-compat-doctor';
        manifest.tmux_session = 'owx-team-rust-compat-doctor';
        ...
        await writeFile(
          tmuxPath,
          '#!/bin/sh\nif [ "$1" = "list-sessions" ]; then echo "owx-team-rust-compat-doctor"; exit 0; fi\nexit 0\n',
        );
        ...
        const result = runOmx(
          wd,
          ['doctor', '--team'],
          { PATH: `${fakeBin}:${process.env.PATH || ''}`, OWX_TEAM_STATE_ROOT: teamStateRoot },
        );
        if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('keeps HUD team state on the session-scoped compatibility file', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-rust-compat-hud-'));
    try {
  ...
  it('prefers bridge-authored dispatch/mailbox compatibility views over stale legacy files', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'owx-rust-compat-bridge-'));
