...
describe('native release manifest generator', () => {
  it('annotates Linux libc variants and sorts musl assets before glibc fallbacks', async () => {
    const root = await mkdtemp(join(tmpdir(), 'owx-native-release-manifest-'));
    try {
      const artifactsDir = join(root, 'artifacts');
      await mkdir(artifactsDir, { recursive: true });

      const muslArchive = 'owx-sparkshell-x86_64-unknown-linux-musl.tar.gz';
      const glibcArchive = 'owx-sparkshell-x86_64-unknown-linux-gnu.tar.gz';
      ...
      await writeFile(planPath, JSON.stringify({
        ...
        releases: [
          { app_name: 'owx-sparkshell', app_version: '0.10.2' },
        ],
        artifacts: {
          linuxGlibc: {
            kind: 'executable-zip',
