...
describe('native release workflow', () => {
  it('defines a unified tag workflow that publishes both Rust binaries before npm publish', () => {
    ...
    assert.doesNotMatch(workflow, /generate-release-body\.js/);
    assert.match(workflow, /owx-api/);
    assert.match(workflow, /owx-explore-harness/);
    assert.match(workflow, /owx-sparkshell/);
    ...
    assert.match(workflow, /publish-native-assets:[\s\S]*Generate release body[\s\S]*Release assets are attached below\.[\s\S]*>\s*RELEASE_BODY\.generated\.md/);
