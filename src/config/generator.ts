...
const CODE_AUTHORING_INSTRUCTIONS =
  "Keep OWX workflow resilience separate from authored-code behavior: instructions to retry tools, recover workflow state, continue execution, or finish the task apply only to agent orchestration and must not be translated into fallback branches, silent defaults, compatibility shims, retries, degraded modes, or alternate execution paths in source code. When authoring source code, implement the intended behavior directly and fail fast with meaningful errors when required state, invariants, supported inputs, or internal contracts are not satisfied. Treat explicit failure as a complete and correct implementation when the contract requires it; do not make authored code succeed merely to satisfy the agent workflow's completion requirement. Introduce fallback behavior only when it is required by an explicit user requirement, an established repository or public contract, an uncontrollable external or version boundary, or an explicit availability requirement; uncertainty, defensive programming, test convenience, and a desire to make an operation always succeed are not sufficient justification. Prefer declarative, immutable, type-safe code with precise types, exhaustive handling, validated boundaries, and minimal intention-revealing structure. Avoid unnecessary comments; use clear names, types, and structure instead.";
