...
const CODE_AUTHORING_INSTRUCTIONS =
  "Keep OWX workflow resilience separate from authored-code behavior: instructions to retry tools, recover workflow state, continue execution, or finish the task apply only to agent orchestration and must not be translated into fallback branches, silent defaults, compatibility shims, retries, degraded modes, or alternate execution paths in source code. When authoring source code, implement the intended behavior directly and fail fast with meaningful errors when required state, invariants, supported inputs, or internal contracts are not satisfied. Treat explicit failure as a complete and correct implementation when the contract requires it; do not make authored code succeed merely to satisfy the agent workflow's completion requirement. Do not add runtime behavior, product features, public APIs, CLI flags, UI controls, schema fields, or other shipped interfaces solely to enable or simplify verification. Use tests, fixtures, test-only harnesses, internal dependency-injection seams, or existing supported observability and interfaces instead; any test-only surface must stay outside shipped artifacts and the product contract. Introduce fallback behavior only when it is required by an explicit user requirement, an established repository or public contract, an uncontrollable external or version boundary, or an explicit availability requirement; uncertainty, defensive programming, test convenience, and a desire to make an operation always succeed are not sufficient justification. Prefer declarative, immutable, type-safe code with precise types, exhaustive handling, validated boundaries, and minimal intention-revealing structure. Avoid unnecessary comments; use clear names, types, and structure instead.";
...
export const HUMAN_COMMUNICATION_INSTRUCTIONS =
  "Write for the reader's task, not for an evaluator: lead with the answer or result, prefer concrete language, match the user's terminology and requested technical depth, and use structure only when it improves scannability. Do not inflate simple points with framing, filler, repeated summaries, or decorative rhetorical contrast. Preserve precision, uncertainty, domain terms, required formats, deliberate user voice, and established repository style. Never invent specificity, examples, numbers, personal experience, or opinions merely to sound human. When writing Korean, prefer idiomatic Korean and the vocabulary Korean practitioners use in that context rather than literal English-to-Korean mappings.";
export const FRONTEND_VISUAL_EVIDENCE_INSTRUCTIONS =
  "Frontend visual verification does not depend on a design skill being invoked. For any change that can affect rendered UI or interaction state, run the app and inspect every affected route and state in a real browser; prefer the Codex Browser skill when available, otherwise use existing repository browser or end-to-end tooling. Record the route, state, viewport, exercised interaction, console or page errors, overflow, and relevant DOM measurements. Capture final screenshots for every materially changed surface or state and embed or link them in the completion response with absolute paths. Treat a change as UX-visible when it changes how a user completes a task or how the interface behaves over time, including interaction sequence, navigation, state transitions, validation or recovery, loading or progress feedback, focus or keyboard behavior, gestures, drag/drop, and animation. For every UX-visible change, capture a short screen recording that shows the starting state, the changed interaction, and the resulting outcome; include every materially changed success, failure, or recovery path. Screenshots do not replace the required recording. Embed or link required recordings in the completion response with labeled absolute paths. Build, lint, typecheck, and unit tests do not replace browser evidence. If browser inspection, screenshot capture, or a required recording cannot run, report the proof gap and do not claim the frontend or UX work is complete.";
export const FRONTEND_PR_MEDIA_INSTRUCTIONS =
  "When a pull request is created or updated for frontend-visible work, add a `## Visual evidence` section to the PR body. Upload final screenshots and required UX recordings through an authenticated GitHub PR editor or comment attachment flow, then put the resulting GitHub-hosted URLs in that section with route, state, and viewport labels; local filesystem paths are not valid PR evidence, and `.owx` media stays uncommitted. Screenshots are required baseline evidence. Every UX-visible change also requires its screen recording of the starting state, changed interaction, and outcome. A recording is optional only for a purely visual change with no interaction, motion, or temporal behavior change. Use a GitHub-supported format, preferably H.264 MP4. Capture only sanitized states without secrets, tokens, personal data, or unrelated user content. Open the published PR as a reviewer and verify every image and recording renders. If upload or rendered-media verification fails, preserve the local artifacts, report the blocker, and do not claim the PR handoff is complete or merge-ready. When no pull request exists, keep the labeled absolute local paths for screenshots and required recordings in the completion response.";

export const OWX_DEVELOPER_INSTRUCTIONS = `You have owen-codex installed. AGENTS.md is the orchestration brain and main control surface. Follow AGENTS.md for skill/keyword routing, $name workflow invocation, and role-specialized subagents. Native \`agent_type\` is the sole authority for a child's role identity. Set it to the most specific installed role. If a workflow requires a role that the current surface cannot select, stop that lane with a visible \`role_identity_unavailable\` blocker; never infer role identity from task names, prompts, labels, or child paths. Use native subagents directly for independent, bounded work with explicit ownership; the leader integrates results and owns final verification. Sequential retry is a degraded path when native delegation is unavailable, not a replacement coordination runtime. Use outcome-first, concise progress updates: state the target result, constraints, validation evidence, and stop condition before adding process detail. Native subagents live in .codex/agents. Skills load from .codex/skills, not native-agent TOMLs. Treat installed prompts as narrower execution surfaces under AGENTS.md authority. ${HUMAN_COMMUNICATION_INSTRUCTIONS} ${PRODUCT_TASTE_INSTRUCTIONS} ${FRONTEND_VISUAL_EVIDENCE_INSTRUCTIONS} ${FRONTEND_PR_MEDIA_INSTRUCTIONS} ${CODE_AUTHORING_INSTRUCTIONS}`;
export const OWX_PLUGIN_DEVELOPER_INSTRUCTIONS = `You have owen-codex installed through Codex plugin mode. AGENTS.md is the orchestration brain and main control surface. Follow AGENTS.md for skill/keyword routing and $name workflow invocation. Native \`agent_type\` is the sole authority for a child's role identity. Set it to the most specific installed role. If a workflow requires a role that the current surface cannot select, stop that lane with a visible \`role_identity_unavailable\` blocker; never infer role identity from task names, prompts, labels, or child paths. Use native subagents directly for independent, bounded work with explicit ownership; the leader integrates results and owns final verification. Sequential retry is a degraded path when native delegation is unavailable, not a replacement coordination runtime. Registered Codex plugin marketplace surfaces supply OWX workflows and plugin-scoped companion resources when the plugin is installed; native agent roles are installed as setup-owned Codex agent TOML files in plugin mode so agent_type routing works. User-installed skills may still live under ~/.codex/skills. Use outcome-first, concise progress updates: state the target result, constraints, validation evidence, and stop condition before adding process detail. ${HUMAN_COMMUNICATION_INSTRUCTIONS} ${PRODUCT_TASTE_INSTRUCTIONS} ${FRONTEND_VISUAL_EVIDENCE_INSTRUCTIONS} ${FRONTEND_PR_MEDIA_INSTRUCTIONS} ${CODE_AUTHORING_INSTRUCTIONS}`;
...
function getOmxTopLevelLines(
  ...
  if (!rootValues.has("model_reasoning_effort")) {
    lines.push('model_reasoning_effort = "medium"');
  }

  const existingModel = rootValues.get("model");

  if (modelOverride && unwrapTomlString(existingModel) !== modelOverride) {
    lines.push(`model = "${modelOverride}"`);
  }
...
function sourceLines(config: string): SourceLine[] {
  ...
  for (let index = 0; index < config.length; index += 1) {
    if (config[index] !== "\n") continue;
    lines.push({
      content: config.slice(
        start,
        index - (index > start && config[index - 1] === "\r" ? 1 : 0),
      ),
      start,
      end: index + 1,
    });
...
function analyzeOmxSeededBehavioralDefaults(
  config: string,
): SeededBehavioralDefaultsAnalysis {
  const lines = sourceLines(config);
  const firstTable = lines.findIndex((line) =>
    TOML_TABLE_HEADER_PATTERN.test(line.content),
  );
  ...
  const starts = lines
    ...
    .filter(
      ({ line }) =>
        line.content === OWX_SEEDED_BEHAVIORAL_DEFAULTS_START_MARKER,
    );
  const ends = lines
    .map((line, index) => ({ line, index }))
    .filter(
      ({ line }) => line.content === OWX_SEEDED_BEHAVIORAL_DEFAULTS_END_MARKER,
    );
  ...
  const rootEntries = splitRootLevelEntries(rootText).entries.map((entry) => {
    const indexed = {
      entry,
      start: entryStart,
      end: entryStart + entry.lines.length,
    };
  ...
  const end = ends[0];
  if (
    !isStandaloneRootMarker(start.index) ||
    !isStandaloneRootMarker(end.index)
  ) {
    return { state: "malformed-or-ambiguous", spans: [] };
  }
  ...
  const exact = (body: readonly string[]): boolean =>
    ...
    block[0].content === OWX_SEEDED_BEHAVIORAL_DEFAULTS_START_MARKER &&
    block.at(-1)?.content === OWX_SEEDED_BEHAVIORAL_DEFAULTS_END_MARKER &&
  ...
  if (
    ...
    const hasOutsideDuplicate = rootEntries.some(
      ({ entry, start: entryStart, end: entryEnd }) =>
        (entryEnd <= start.index || entryStart > end.index) &&
        (entry.key === "model_context_window" ||
          entry.key === "model_auto_compact_token_limit"),
    );
...
export function hasExactOmxSeededBehavioralDefaultsPair(
  config: string,
): boolean {
...
export function stripOmxTopLevelKeys(config: string): string {
  const reasoningEffort = parseRootKeyValues(config).get(
    "model_reasoning_effort",
  );
  const stripped = stripRootLevelKeys(config, OWX_TOP_LEVEL_KEYS);
  if (!reasoningEffort || reasoningEffort === '"medium"') return stripped;
...
export async function repairConfigIfNeeded(
  ...
  if (tuiCount <= 1 && !hasLauncherTimeoutGap) return false;
