...
describe('getModelForMode', () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'owx-models-'));
  ...
  afterEach(async () => {
    ...
    if (typeof originalDefaultFrontierModel === 'string') {
      process.env.OWX_DEFAULT_FRONTIER_MODEL = originalDefaultFrontierModel;
    } else {
      delete process.env.OWX_DEFAULT_FRONTIER_MODEL;
    }
    if (typeof originalDefaultStandardModel === 'string') {
      process.env.OWX_DEFAULT_STANDARD_MODEL = originalDefaultStandardModel;
    } else {
      delete process.env.OWX_DEFAULT_STANDARD_MODEL;
    }
    if (typeof originalDefaultSparkModel === 'string') {
      process.env.OWX_DEFAULT_SPARK_MODEL = originalDefaultSparkModel;
    } else {
      delete process.env.OWX_DEFAULT_SPARK_MODEL;
    }
  ...
  async function writeConfig(config: Record<string, unknown>): Promise<void> {
    await writeFile(join(tempDir, '.owx-config.json'), JSON.stringify(config));
  }
  ...
  it('uses OWX_DEFAULT_STANDARD_MODEL when configured in shell env', () => {
    process.env.OWX_DEFAULT_STANDARD_MODEL = 'gpt-5.4-mini-tuned';
  ...
  it('uses .owx-config.json env.OWX_DEFAULT_STANDARD_MODEL when shell env is absent', async () => {
    await writeConfig({ env: { OWX_DEFAULT_STANDARD_MODEL: 'standard-local' } });
    assert.equal(getEnvConfiguredStandardDefaultModel(), 'standard-local');
    assert.equal(getStandardDefaultModel(), 'standard-local');
  });

  it('prefers shell OWX_DEFAULT_FRONTIER_MODEL over .owx-config.json env override', async () => {
    process.env.OWX_DEFAULT_FRONTIER_MODEL = 'frontier-shell';
    await writeConfig({ env: { OWX_DEFAULT_FRONTIER_MODEL: 'frontier-local' } });
  ...
  it('prefers OWX_DEFAULT_SPARK_MODEL over legacy OWX_SPARK_MODEL', () => {
    process.env.OWX_DEFAULT_SPARK_MODEL = 'spark-canonical';
    process.env.OWX_SPARK_MODEL = 'spark-legacy';
    assert.equal(getSparkDefaultModel(), 'spark-canonical');
  });

  it('reads normalized env overrides from .owx-config.json', async () => {
    await writeConfig({
      env: {
  ...
  it('reads normalized per-agent reasoning overrides from .owx-config.json', async () => {
    await writeConfig({
      agentReasoning: {
        Architect: ' xhigh ',
        critic: 'high',
