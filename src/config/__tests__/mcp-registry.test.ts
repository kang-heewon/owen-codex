...
describe("unified MCP registry loader", () => {
  it("prefers ~/.owx/mcp-registry.json over ~/.omc/mcp-registry.json", async () => {
    const wd = await mkdtemp(join(tmpdir(), "owx-mcp-registry-"));
    try {
      const owxPath = join(wd, ".owx", "mcp-registry.json");
      ...
      assert.equal(result.sourcePath, owxPath);
  ...
  it("returns canonical home-based registry candidates", () => {
    ...
    assert.deepEqual(candidates, ["/tmp/home/.owx/mcp-registry.json"]);
  });
