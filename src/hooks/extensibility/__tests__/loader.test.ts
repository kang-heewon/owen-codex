...
describe('hooksDir', () => {
  it('returns .owx/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.owx', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .owx/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.owx', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-load-'));
    try {
