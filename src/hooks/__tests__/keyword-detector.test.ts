...
async function withIsolatedHome<T>(prefix: string, run: (homeDir: string) => Promise<T>): Promise<T> {
  const homeDir = await mkdtemp(join(tmpdir(), `owx-keyword-home-${prefix}-`));
...
describe('keyword detector team compatibility', () => {
  ...
  it('recognizes plugin-prefixed explicit skill tokens', () => {
    const matches = detectKeywords('$owen-codex:ralplan implement issue #1307');
    assert.deepEqual(matches.map((m) => m.skill), ['ralplan']);
    assert.equal(matches[0]?.keyword, '$owen-codex:ralplan');
  });

  it('supports mixed-form explicit multi-skill invocation ordering and dedupe', () => {
    const matches = detectKeywords('$owen-codex:ralplan $ralph $owen-codex:ralplan ship this');
    assert.deepEqual(matches.map((m) => m.skill), ['ralplan', 'ralph']);
    assert.deepEqual(matches.map((m) => m.keyword), ['$owen-codex:ralplan', '$ralph']);
  });

  it('keeps recognized tokens on both sides of an unknown plugin-prefixed token in the same contiguous block', () => {
    const matches = detectKeywords('$owen-codex:ralplan $owen-codex:unknown $ralph');
    assert.deepEqual(matches.map((m) => m.skill), ['ralplan', 'ralph']);
    assert.deepEqual(matches.map((m) => m.keyword), ['$owen-codex:ralplan', '$ralph']);
  });

  it('limits mixed-form explicit invocation to the first contiguous block', () => {
    const matches = detectKeywords('$owen-codex:ralplan text $ralph');
    assert.deepEqual(matches.map((m) => m.skill), ['ralplan']);
  });

  it('normalizes plugin-prefixed ulw shorthand token', () => {
    const ulw = detectPrimaryKeyword('$owen-codex:ulw continue');
    assert.ok(ulw);
    assert.equal(ulw.skill, 'ultrawork');
    assert.equal(ulw.keyword, '$owen-codex:ulw');
  });

  it('supports plugin-prefixed hyphenated workflow tokens', () => {
    const deepInterview = detectPrimaryKeyword('$owen-codex:deep-interview gather requirements');
    assert.ok(deepInterview);
    assert.equal(deepInterview.skill, 'deep-interview');
    assert.equal(deepInterview.keyword, '$owen-codex:deep-interview');

    const codeReview = detectPrimaryKeyword('$owen-codex:code-review before merge');
    assert.ok(codeReview);
    assert.equal(codeReview.skill, 'code-review');
    assert.equal(codeReview.keyword, '$owen-codex:code-review');

    const bestPracticeResearch = detectPrimaryKeyword('$owen-codex:best-practice-research find official best practices');
    assert.ok(bestPracticeResearch);
    assert.equal(bestPracticeResearch.skill, 'best-practice-research');
    assert.equal(bestPracticeResearch.keyword, '$owen-codex:best-practice-research');
  });

  it('does not fall back to implicit keyword detection when an unknown plugin-prefixed $token is present', () => {
    const matches = detectKeywords('$owen-codex:maer-thinking 다시 설명해봐 keep going');
    assert.deepEqual(matches, []);
    const primary = detectPrimaryKeyword('$owen-codex:maer-thinking 다시 설명해봐 keep going');
    assert.equal(primary, null);
  });

  it('suppresses implicit detection when an unknown plugin-prefixed token is present with other keyword text', () => {
    const matches = detectKeywords('$owen-codex:unknown analyze this issue');
    assert.deepEqual(matches, []);
    assert.equal(detectPrimaryKeyword('$owen-codex:unknown analyze this issue'), null);
  });
  ...
  it('maps intentful ultragoal prose without triggering artifact path mentions', () => {
    ...
    const pathOnly = detectPrimaryKeyword('inspect .owx/ultragoal/goals.json');
  ...
  it('does not trigger deep-interview from cleanup or state-management mentions', () => {
    ...
    assert.equal(detectPrimaryKeyword('remove the stale deep interview lock from .owx/state'), null);
  });
...
describe('keyword detector skill-active-state lifecycle', () => {
  it('co-locates direct boxed activation mode detail and canonical skill state for OWX_ROOT', async () => {
    ...
    try {
      ...
      assert.equal(
        existsSync(join(sourceCwd, '.owx', 'state', 'sessions', 'sess-boxed-ralplan', SKILL_ACTIVE_STATE_FILE)),
        false,
      );
      assert.equal(
        existsSync(join(sourceCwd, '.owx', 'state', 'sessions', 'sess-boxed-ralplan', 'ralplan-state.json')),
        false,
      );
    } finally {
  ...
  it('co-locates direct boxed activation mode detail and canonical skill state for OWX_STATE_ROOT', async () => {
    const root = await mkdtemp(join(tmpdir(), 'owx-keyword-boxed-state-root-'));
    const sourceCwd = join(root, 'source');
    ...
    try {
      ...
      assert.equal(
        existsSync(join(sourceCwd, '.owx', 'state', 'sessions', 'sess-state-root-ralplan', SKILL_ACTIVE_STATE_FILE)),
        false,
      );
      assert.equal(
        existsSync(join(sourceCwd, '.owx', 'state', 'sessions', 'sess-state-root-ralplan', 'ralplan-state.json')),
        false,
      );
    } finally {
      if (typeof previousOmxRoot === 'string') process.env.OWX_ROOT = previousOmxRoot;
  ...
  it('writes skill-active-state.json with deep-interview phase when autopilot keyword activates', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-'));
    ...
    try {
      ...
      assert.equal(result.initialized_state_path, '.owx/state/sessions/sess-1/autopilot-state.json');
      ...
      assert.deepEqual(modeState.state.handoff_artifacts, {
        context_snapshot_path: '.owx/context/please-run-and-keep-going-20260225T000000Z.md',
        context_snapshot: {
          path: '.owx/context/please-run-and-keep-going-20260225T000000Z.md',
          kind: 'canonical',
      ...
      const snapshot = await readFile(join(cwd, '.owx', 'context', 'please-run-and-keep-going-20260225T000000Z.md'), 'utf-8');
  ...
  it('migrates legacy Autopilot context snapshot paths into handoff artifacts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-legacy-context-'));
    const stateDir = join(cwd, '.owx', 'state');
    ...
    try {
      ...
      await writeFile(join(stateDir, 'sessions', sessionId, 'autopilot-state.json'), JSON.stringify({
        ...
        context_snapshot_path: '.owx/context/legacy-task-20260529T000000Z.md',
        state: { handoff_artifacts: { deep_interview: null } },
      }, null, 2));
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'context', 'legacy-task-20260529T000000Z.md'), '# legacy task');
      ...
      assert.equal(modeState.state?.handoff_artifacts?.context_snapshot_path, '.owx/context/legacy-task-20260529T000000Z.md');
      assert.deepEqual(modeState.state?.handoff_artifacts?.context_snapshot, {
        path: '.owx/context/legacy-task-20260529T000000Z.md',
        kind: 'legacy',
        original_task_status: 'legacy-unverified',
      });
      assert.equal(existsSync(join(cwd, '.owx', 'context', 'continue-20260530T000000Z.md')), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('rejects unsafe legacy Autopilot context snapshot paths without writing outside .owx/context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-unsafe-context-'));
    const stateDir = join(cwd, '.owx', 'state');
    ...
    try {
      ...
      await writeFile(join(stateDir, 'sessions', sessionId, 'autopilot-state.json'), JSON.stringify({
        ...
        current_phase: 'deep-interview',
        started_at: AUTOPILOT_TEST_STARTED_AT,
        context_snapshot_path: '.owx/context/../../escape.md',
        state: { handoff_artifacts: { deep_interview: null } },
      }, null, 2));
      ...
      assert.ok(result);
      assert.equal(existsSync(join(cwd, '.owx', 'escape.md')), false);
      const modeState = await readAutopilotModeState(stateDir, sessionId);
      assert.equal(modeState.context_snapshot_path, undefined);
      await assertAutopilotRecoverySnapshot(
        cwd,
        modeState,
        '.owx/context/autopilot-recovery-20260530T000000Z.md',
  ...
  it('does not snapshot bare continuation text when active Autopilot mode state is corrupt', async () => {
    ...
    for (const fixture of ['missing-current-phase', 'malformed-json', 'array-json'] as const) {
      const cwd = await mkdtemp(join(tmpdir(), `owx-keyword-autopilot-corrupt-continuation-${fixture}-`));
      const stateDir = join(cwd, '.owx', 'state');
      ...
      try {
        ...
        await assertAutopilotRecoverySnapshot(
          ...
          /^\.owx\/context\/autopilot-recovery-20260530T000000Z(?:-\d+)?\.md$/,
  ...
  it('rejects nested symlink Autopilot context snapshot candidates during reuse', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-nested-symlink-context-'));
    const outside = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-nested-symlink-outside-'));
    const stateDir = join(cwd, '.owx', 'state');
    const sessionId = 'sess-autopilot-nested-symlink-context';
    try {
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await symlink(outside, join(cwd, '.owx', 'context', 'link'));
      ...
      await writeFile(join(stateDir, 'sessions', sessionId, 'autopilot-state.json'), JSON.stringify({
        ...
        started_at: AUTOPILOT_TEST_STARTED_AT,
        state: { handoff_artifacts: { context_snapshot_path: '.owx/context/link/exfil.md' } },
      }, null, 2));
  ...
  it('rejects typed canonical Autopilot recovery snapshot candidates during reuse', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-typed-recovery-context-'));
    const stateDir = join(cwd, '.owx', 'state');
    const sessionId = 'sess-autopilot-typed-recovery-context';
    try {
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'context', 'autopilot-recovery-20260529T000000Z.md'), '# stale degraded recovery');
      await writeActiveAutopilotSkillState(stateDir, sessionId);
      await writeFile(join(stateDir, 'sessions', sessionId, 'autopilot-state.json'), JSON.stringify({
        active: true,
        mode: 'autopilot',
        ...
        state: {
          handoff_artifacts: {
            context_snapshot: {
              path: '.owx/context/autopilot-recovery-20260529T000000Z.md',
              kind: 'canonical',
            },
          },
        },
      ...
      assert.equal(existsSync(join(cwd, '.owx', 'context', 'autopilot-recovery-20260529T000000Z.md')), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('rejects oversized Autopilot context snapshot candidates during reuse', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-oversized-context-'));
    const stateDir = join(cwd, '.owx', 'state');
    const sessionId = 'sess-autopilot-oversized-context';
    try {
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'context', 'oversized-legacy-20260529T000000Z.md'), 'x'.repeat((1024 * 1024) + 1));
      await writeActiveAutopilotSkillState(stateDir, sessionId);
      await writeFile(join(stateDir, 'sessions', sessionId, 'autopilot-state.json'), JSON.stringify({
        active: true,
        mode: 'autopilot',
        ...
        state: {
          handoff_artifacts: {
            context_snapshot_path: '.owx/context/oversized-legacy-20260529T000000Z.md',
          },
        },
      }, null, 2));
      ...
      assert.equal(existsSync(join(cwd, '.owx', 'context', 'oversized-legacy-20260529T000000Z.md')), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('does not promote degraded recovery snapshots to canonical context on reactivation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-recovery-reactivation-'));
    const stateDir = join(cwd, '.owx', 'state');
    const sessionId = 'sess-autopilot-recovery-reactivation';
    try {
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'context', 'autopilot-recovery-20260529T000000Z.md'), '# degraded recovery');
      ...
      await writeFile(join(stateDir, 'sessions', sessionId, 'autopilot-state.json'), JSON.stringify({
        ...
        state: {
          handoff_artifacts: {
            context_snapshot_path: '.owx/context/autopilot-recovery-20260529T000000Z.md',
            context_snapshot: {
              path: '.owx/context/autopilot-recovery-20260529T000000Z.md',
              kind: 'recovery',
      ...
      const modeState = await readAutopilotModeState(stateDir, sessionId);
      assert.equal(modeState.state?.handoff_artifacts?.context_snapshot_path, '.owx/context/implement-the-real-task-20260530T000000Z.md');
      assert.deepEqual(modeState.state?.handoff_artifacts?.context_snapshot, {
        path: '.owx/context/implement-the-real-task-20260530T000000Z.md',
        kind: 'canonical',
        original_task_status: 'activation-prompt',
      });
      assert.equal(modeState.state?.context_snapshot_recovery, undefined);
      const snapshot = await readFile(join(cwd, '.owx', 'context', 'implement-the-real-task-20260530T000000Z.md'), 'utf-8');
  ...
  it('does not follow symlinked Autopilot context directories when writing snapshots', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-symlink-context-'));
    const outside = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-symlink-outside-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(join(cwd, '.owx'), { recursive: true });
      await symlink(outside, join(cwd, '.owx', 'context'));
      await mkdir(stateDir, { recursive: true });
  ...
  it('allocates unique Autopilot context snapshot paths for same-second matching slugs', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-context-collision-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      ...
      assert.equal(first.state?.handoff_artifacts?.context_snapshot_path, '.owx/context/same-task-20260530T000000Z.md');
      assert.equal(second.state?.handoff_artifacts?.context_snapshot_path, '.owx/context/same-task-20260530T000000Z-2.md');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('fully resets terminal Autopilot mode state when reactivated', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autopilot-terminal-reset-'));
    const stateDir = join(cwd, '.owx', 'state');
    ...
    try {
      ...
      assert.deepEqual(modeState.state?.handoff_artifacts, {
        context_snapshot_path: '.owx/context/investigate-the-next-issue-20260530T000000Z.md',
        context_snapshot: {
          path: '.owx/context/investigate-the-next-issue-20260530T000000Z.md',
          kind: 'canonical',
          original_task_status: 'activation-prompt',
        },
        deep_interview: null,
  ...
  it('resets stopped Autopilot mode state when reactivated', async () => {
    for (const phase of ['stopped', 'user-stopped']) {
      const cwd = await mkdtemp(join(tmpdir(), `owx-keyword-autopilot-${phase}-reset-`));
      const stateDir = join(cwd, '.owx', 'state');
  ...
  it('activates ultrawork mode from the Korean keyboard typo for ulw', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ulw-ko-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
      ...
      assert.equal(result.initialized_state_path, '.owx/state/sessions/sess-ulw-ko/ultrawork-state.json');
  ...
  it('seeds executing state for autoresearch prompt-submit activation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autoresearch-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
      ...
      assert.equal(result.initialized_state_path, '.owx/state/sessions/sess-autoresearch/autoresearch-state.json');
  ...
  it('preserves the planning skill when ralplan and autoresearch are invoked together', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-autoresearch-planning-precedence-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('seeds first-class state for ralplan prompt-submit activation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ralplan-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
      ...
      assert.equal(result.initialized_state_path, '.owx/state/sessions/sess-ralplan/ralplan-state.json');
  ...
  it('auto-completes deep-interview during allowlisted forward handoff', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-handoff-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      ...
      assert.equal(result?.initialized_state_path, '.owx/state/sessions/sess-handoff/ultragoal-state.json');
  ...
  it('denies ralplan handoff from deep-interview without completion or explicit skip evidence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-ralplan-handoff-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
  ...
  it('allows ralplan handoff from deep-interview with a durable completion gate', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-ralplan-handoff-complete-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
  ...
  it('emits terminal ralplan state before explicit ultragoal execution handoff', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-ralplan-ultragoal-handoff-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
  ...
  it('acquires a deep-interview input lock immediately on activation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
  ...
  it('persists repo-local deep-interview config values into activation and mode state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-config-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(join(cwd, '.owx'), { recursive: true });
      await mkdir(stateDir, { recursive: true });
...
`,
      ...
      assert.equal(result.initialized_state_path, '.owx/state/sessions/sess-deep-interview-config/deep-interview-state.json');
      ...
      assert.equal(modeState.config_source, join(cwd, '.owx', 'config.toml'));
      assert.equal(modeState.deep_interview_config?.sourcePath, join(cwd, '.owx', 'config.toml'));
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('persists deep-interview config when mixed workflow prompts defer execution modes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-config-mixed-'));
    const stateDir = join(cwd, '.owx', 'state');
...
`,
  ...
  it('shows before-after state change when deep-interview config is added at runtime', async () => {
    await withIsolatedHome('deep-interview-config-before-after', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-config-before-after-'));
      const stateDir = join(cwd, '.owx', 'state');
...
`,
        ...
        assert.equal(afterModeState.config_source, join(cwd, '.owx', 'config.toml'));
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
  ...
  it('preserves deep-interview config values during continuation prompts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-config-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
...
`,
  ...
  it('preserves explicit deep-interview profile flags during continuation prompts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-config-profile-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
...
`,
  ...
  it('keeps the documented deep-interview Suggested Config executable through activation state', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-doc-config-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('keeps deep-interview activation alive when repo config TOML is malformed', async () => {
    await withIsolatedHome('deep-interview-malformed-config', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-malformed-config-'));
      const stateDir = join(cwd, '.owx', 'state');
  ...
  it('creates the session-scoped deep-interview state directory before persisting mode state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-session-dir-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('releases the deep-interview input lock on abort via cancel keyword', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-deep-interview-abort-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      await recordSkillActivation({
  ...
  it('does not seed non-stateful skill mode state on keyword activation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-non-stateful-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
  ...
  it('keeps Autopilot visible when a supervised code-review child keyword appears', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autopilot-child-code-review-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('keeps tracked Autopilot child keywords supervised and completes stale child mode state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autopilot-child-ultraqa-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('denies supervised Autopilot child rollback without clearing stale execution state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autopilot-child-rollback-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('surfaces supervised Autopilot deep-interview to ralplan gate failures', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autopilot-child-gate-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('ignores stale root child mode state during session-scoped Autopilot child reconciliation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autopilot-child-session-root-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('records ultragoal as a prompt skill with first-class mode state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ultragoal-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
      ...
      assert.equal(result.initialized_mode, 'ultragoal');
      assert.equal(result.initialized_state_path, '.owx/state/ultragoal-state.json');
  ...
  it('emits a warning when skill-active-state persistence fails', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-persist-fail-'));
    const warnings: unknown[][] = [];
    mock.method(console, 'warn', (...args: unknown[]) => {
      warnings.push(args);
    });
  ...
  it('preserves activated_at for same-skill continuation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
  ...
  it('preserves seeded mode progress for same-skill continuation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-seed-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          updated_at: '2026-02-25T00:10:00.000Z',
          session_id: 'sess-autopilot',
          state: { context_snapshot_path: '.owx/context/existing.md' },
        }),
      );
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'context', 'existing.md'), '# existing context');
      ...
      assert.equal(modeState.state?.handoff_artifacts?.context_snapshot_path, '.owx/context/existing.md');
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('does not persist Ralph workflow state for a plain conversational mention', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ralph-plain-text-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('preserves Ralph iteration counters for same-skill continuation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ralph-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
    const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('keeps Korean ulw typo first in mixed explicit workflow persistence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ulw-ko-mixed-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
  ...
  it('lets an explicit Korean ulw typo override an active workflow continuation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ulw-ko-explicit-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
  ...
  it('routes bare keep-going continuation to the active autopilot skill instead of generic ralph continuation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-autopilot-bare-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          updated_at: '2026-04-19T00:10:00.000Z',
          session_id: 'sess-autopilot-bare',
          state: { context_snapshot_path: '.owx/context/autopilot.md' },
        }, null, 2),
      );
      await mkdir(join(cwd, '.owx', 'context'), { recursive: true });
      await writeFile(join(cwd, '.owx', 'context', 'autopilot.md'), '# autopilot context');
      ...
      assert.equal(modeState.current_phase, 'code-review');
      assert.equal(modeState.state?.context_snapshot_path, undefined);
      assert.equal(modeState.state?.handoff_artifacts?.context_snapshot_path, '.owx/context/autopilot.md');
  ...
  it('resets terminal Ralph blocked_on_user state when reactivated', async () => {
    ...
    for (const testCase of cases) {
      const cwd = await mkdtemp(join(tmpdir(), `owx-keyword-state-ralph-terminal-${testCase.name}-reactivation-`));
      const stateDir = join(cwd, '.owx', 'state');
  ...
  it('routes bare keep-going continuation to the active ralph skill instead of resetting through generic keep-going detection', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-ralph-bare-continuation-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
  ...
  it('does not reuse active workflow continuation when prompt contains an unknown plugin-prefixed explicit token', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-unknown-prefixed-explicit-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      ...
      const result = await recordSkillActivation({
        stateDir,
        text: '$owen-codex:unknown continue',
  ...
  it('does not continue a workflow from another session root canonical entry', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-cross-session-continue-'));
    const stateDir = join(cwd, '.owx', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      await writeFile(
  ...
  it('denies switching away from a standalone workflow without explicit clear', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-skill-switch-deny-'));
    const stateDir = join(cwd, '.owx', 'state');
    const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('resets activated_at when keyword changes within the same skill', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'owx-keyword-state-keyword-switch-'));
    const stateDir = join(cwd, '.owx', 'state');
    const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('does not re-enter ralplan for a short approved ralph follow-up with durable consensus', async () => {
    ...
    try {
      ...
      await writeFile(
        ...
        '# Approved plan\n\nLaunch hint: owx ralph "Execute approved issue 832 plan"\n',
      );
