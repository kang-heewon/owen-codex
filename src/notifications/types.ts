...
export interface DiscordBotNotificationConfig {
  enabled: boolean;
  /** Discord bot token (or env var: OWX_DISCORD_NOTIFIER_BOT_TOKEN) */
  botToken?: string;
  /** Channel ID to send messages to (or env var: OWX_DISCORD_NOTIFIER_CHANNEL) */
  channelId?: string;
  /** Optional mention to prepend to messages (e.g. "<@123456>" for user, "<@&789>" for role) */
  mention?: string;
}
...
/** Top-level notification configuration (stored in .owx-config.json) */
...
export interface FullNotificationPayload {
  ...
  projectName?: string;
  /** Active OWX modes during this session */
  modesUsed?: string[];
