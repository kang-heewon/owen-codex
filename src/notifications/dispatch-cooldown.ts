 ...
 *
 * Config key : notifications.dispatchCooldownSeconds in ~/.codex/.owx-config.json
 * Env var    : OWX_DISPATCH_COOLDOWN_SECONDS  (overrides config)
 * State file : .owx/state/dispatch-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. OWX_DISPATCH_COOLDOWN_SECONDS env var
 *   2. notifications.dispatchCooldownSeconds in ~/.codex/.owx-config.json
 *   3. Default: 60 seconds
 */
export function getDispatchNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.OWX_DISPATCH_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
