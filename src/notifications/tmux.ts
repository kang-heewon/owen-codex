...
const OWX_METADATA_SEGMENT_RE = /^\[OWX(?:[#\]].*)?$/;
...
function isMetadataOnlyTmuxSegment(segment: string): boolean {
  return OWX_METADATA_SEGMENT_RE.test(segment)
...
function isMetadataOnlyTmuxLine(line: string): boolean {
  ...
  const hasExplicitStatusSegment = segments.some((segment) => OWX_METADATA_SEGMENT_RE.test(segment) || HUD_STATUS_SEGMENT_RE.test(segment));
...
function shouldUsePidFallback(): boolean {
  return process.env.OWX_TMUX_PID_FALLBACK === "1";
}
