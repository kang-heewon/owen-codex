/**
 * Notification system for owen-codex
...
/**
 * Load notification config from .owx/notifications.json
 */
export async function loadNotificationConfig(projectRoot?: string): Promise<NotificationConfig | null> {
  const configPath = join(projectRoot || process.cwd(), '.owx', 'notifications.json');
...
export function _buildDesktopArgs(
  ...
  if (platform === 'darwin') {
    ...
    const ps =
      ...
      `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('owen-codex').Show($xml)`;
...
async function sendDiscordNotification(payload: NotificationPayload, webhookUrl: string): Promise<void> {
  ...
  const body = JSON.stringify({
    embeds: [{
      title: `[OWX] ${payload.title}`,
      description: payload.message,
      color: colorMap[payload.type],
      footer: { text: `owen-codex | ${payload.mode || 'general'}` },
