 ...
 *
 * Reads hookTemplates from .owx-config.json for user-customizable message templates.
 * Config is stored under the notifications.hookTemplates key in codexHome()/.owx-config.json.
 * Env var OWX_HOOK_CONFIG overrides to a separate file path.
 */
 ...
 *
 * Primary source: notifications.hookTemplates key in codexHome()/.owx-config.json
 * Env var override: OWX_HOOK_CONFIG points to a separate file containing the
...
export function getHookConfig(): HookNotificationConfig | null {
  ...
  const envOverridePath = process.env.OWX_HOOK_CONFIG;
  ...
  // Primary: read from notifications.hookTemplates in .owx-config.json
  const OWX_CONFIG_PATH = join(codexHome(), ".owx-config.json");
  if (!existsSync(OWX_CONFIG_PATH)) {
    cachedConfig = null;
    return null;
  }

  try {
    const raw = JSON.parse(readFileSync(OWX_CONFIG_PATH, "utf-8"));
 ...
 * - hook event `enabled: false` overrides .owx-config.json event `enabled: true`
 * - Platform credentials are NOT affected (they stay in .owx-config.json)
 */
