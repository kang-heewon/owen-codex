export const OWX_NOTIFY_TEMP_ENV = 'OWX_NOTIFY_TEMP';
export const OWX_NOTIFY_TEMP_CONTRACT_ENV = 'OWX_NOTIFY_TEMP_CONTRACT';
...
export function parseNotifyTempContractFromArgs(
  ...
  const envActivated = env[OWX_NOTIFY_TEMP_ENV] === '1';
...
export function isNotifyTempEnvActive(env: NodeJS.ProcessEnv = process.env): boolean {
  return env[OWX_NOTIFY_TEMP_ENV] === '1';
}

export function readNotifyTempContractFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): NotifyTempContract | null {
  const raw = env[OWX_NOTIFY_TEMP_CONTRACT_ENV];
  if (!raw) return null;
  try {
