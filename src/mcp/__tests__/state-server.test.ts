...
describe('state-server directory initialization', () => {
  ...
  it('creates boxed runtime state under OWX_ROOT for mutating tools', async () => {
    process.env.OWX_STATE_SERVER_DISABLE_AUTO_START = '1';
    const previousOmxRoot = process.env.OWX_ROOT;
    const { handleStateToolCall } = await import('../state-server.js');

    const root = await mkdtemp(join(tmpdir(), 'owx-state-server-boxed-'));
  ...
  it('co-locates boxed tracked mode and canonical skill state under OWX_ROOT', async () => {
    process.env.OWX_STATE_SERVER_DISABLE_AUTO_START = '1';
    const previousOmxRoot = process.env.OWX_ROOT;
    ...
    try {
      ...
      assert.equal(response.isError, undefined);
      const boxedSessionDir = join(box, '.owx', 'state', 'sessions', sessionId);
  ...
  it('accepts canonical lifecycle_outcome and backfills compatibility run_outcome', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-lifecycle-'));
    try {
      const writeResponse = await handleStateToolCall({
        params: {
          name: 'state_write',
  ...
  it('derives canonical lifecycle_outcome from legacy run_outcome when needed', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-run-outcome-'));
    try {
  ...
  it('state_write accepts canonical lifecycle_outcome while preserving compatibility run_outcome', async () => {
    ...
    try {
      ...
      assert.equal(response.isError, undefined);
      const state = JSON.parse(await readFile(join(wd, '.owx', 'state', 'autopilot-state.json'), 'utf-8')) as {
        active?: boolean;
  ...
  it('state_write lets canonical lifecycle_outcome take precedence over legacy run_outcome', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-lifecycle-precedence-'));
    try {
      const response = await handleStateToolCall({
        params: {
          name: 'state_write',
  ...
  it('syncs canonical skill-active state for tracked mode writes and clears', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-canonical-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
  ...
  it('writes a session-scoped inactive tombstone when clearing a mode under an active session', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-clear-root-fallback-'));
    try {
      const stateDir = join(wd, '.owx', 'state');
  ...
  it('allows approved overlaps and preserves the remaining canonical state on clear', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-overlap-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
      ...
      const canonicalPath = join(wd, '.owx', 'state', 'sessions', 'sess-overlap', 'skill-active-state.json');
      const canonical = JSON.parse(await readFile(canonicalPath, 'utf-8')) as {
  ...
  it('allows ultrawork when canonical session state is stricter than mode files', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-canonical-prevalidate-'));
    try {
      ...
      await writeFile(
        join(wd, '.owx', 'state', 'skill-active-state.json'),
        JSON.stringify({
          version: 1,
          active: true,
          skill: 'team',
      ...
      await writeFile(
        join(wd, '.owx', 'state', 'sessions', 'sess-canonical-deny', 'skill-active-state.json'),
        JSON.stringify({
          version: 1,
          active: true,
          skill: 'team',
      ...
      assert.equal(
        existsSync(join(wd, '.owx', 'state', 'sessions', 'sess-canonical-deny', 'ultrawork-state.json')),
        true,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
  ...
  it('removes tracked workflows from canonical skill-active state on all_sessions clear', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-canonical-clear-all-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
      ...
      const canonicalPath = join(wd, '.owx', 'state', 'skill-active-state.json');
      const canonical = JSON.parse(await readFile(canonicalPath, 'utf-8')) as {
        active: boolean;
        active_skills?: unknown[];
      };
  ...
  it('does not propagate root clears into isolated session canonical copies', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-root-clear-propagate-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
      ...
      const sessionCanonical = JSON.parse(
        await readFile(
          join(wd, '.owx', 'state', 'sessions', 'sess-root-clear', 'skill-active-state.json'),
          'utf-8',
        ),
      ) as { active_skills?: Array<{ skill: string }> };
  ...
  it('keeps root-scoped team state out of session-scoped ralph canonical state', async () => {
    ...
    try {
      ...
      const rootCanonical = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'skill-active-state.json'), 'utf-8'),
      ...
      const sessionCanonical = JSON.parse(
        await readFile(
          join(wd, '.owx', 'state', 'sessions', 'sess-team-ralph', 'skill-active-state.json'),
          'utf-8',
        ),
      ) as { active_skills?: Array<{ skill: string; phase?: string; session_id?: string }> };
      assert.deepEqual(
  ...
  it('rejects standalone overlaps without mutating canonical state', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-standalone-overlap-'));
    try {
      ...
      const canonical = JSON.parse(
        await readFile(
          join(wd, '.owx', 'state', 'sessions', 'sess-standalone', 'skill-active-state.json'),
          'utf-8',
        ),
      ) as { active_skills?: Array<{ skill: string; phase?: string; session_id?: string }> };
      assert.deepEqual(
  ...
  it('auto-completes deep-interview when starting ralplan and returns transition messaging', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-handoff-interview-'));
    try {
      await mkdir(join(wd, '.owx', 'state', 'sessions', 'sess-handoff'), { recursive: true });
      await writeFile(
        join(wd, '.owx', 'state', 'sessions', 'sess-handoff', 'deep-interview-state.json'),
        JSON.stringify({
          active: true,
          mode: 'deep-interview',
      ...
      const completed = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-handoff', 'deep-interview-state.json'), 'utf-8'),
      ) as {
        active?: boolean;
  ...
  it('rejects execution-to-planning rollback with clear-first guidance', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-rollback-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
  ...
  it('does not auto-complete existing workflow state when tracked write validation fails', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-validate-before-transition-'));
    try {
      await mkdir(join(wd, '.owx', 'state', 'sessions', 'sess-invalid'), { recursive: true });
      await writeFile(
        join(wd, '.owx', 'state', 'sessions', 'sess-invalid', 'ralplan-state.json'),
      ...
      const ralplanState = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-invalid', 'ralplan-state.json'), 'utf-8'),
      ) as Record<string, unknown>;
      assert.equal(ralplanState.active, true);
      assert.equal(ralplanState.current_phase, 'planning');
      assert.equal(existsSync(join(wd, '.owx', 'state', 'sessions', 'sess-invalid', 'ralph-state.json')), false);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('allows ultrawork overlap with any tracked mode', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-ultrawork-any-'));
    try {
  ...
  it('keeps session-scoped workflow states isolated across writes and clears', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-session-isolation-'));
    try {
      ...
      const sessionAState = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-a', 'deep-interview-state.json'), 'utf-8'),
      ) as Record<string, unknown>;
      ...
      const sessionACanonical = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-a', 'skill-active-state.json'), 'utf-8'),
  ...
  it('does not auto-complete session workflows from root-scoped workflow writes', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-root-session-isolation-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
      ...
      const sessionState = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-interview', 'deep-interview-state.json'), 'utf-8'),
      ) as Record<string, unknown>;
      ...
      const sessionCanonical = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-interview', 'skill-active-state.json'), 'utf-8'),
      ) as { active_skills?: Array<{ skill: string; session_id?: string }> };
      assert.deepEqual(
      ...
      const rootState = JSON.parse(await readFile(join(wd, '.owx', 'state', 'ralplan-state.json'), 'utf-8')) as Record<string, unknown>;
  ...
  it('keeps session canonical state when clearing the root scope without all_sessions', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'owx-state-server-root-clear-isolation-'));
    try {
      await handleStateToolCall({
        params: {
          name: 'state_write',
      ...
      assert.equal(existsSync(join(wd, '.owx', 'state', 'deep-interview-state.json')), false);
      const sessionState = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-keep', 'deep-interview-state.json'), 'utf-8'),
      ) as Record<string, unknown>;
      assert.equal(sessionState.active, true);

      const sessionCanonical = JSON.parse(
        await readFile(join(wd, '.owx', 'state', 'sessions', 'sess-keep', 'skill-active-state.json'), 'utf-8'),
      ) as { active_skills?: Array<{ skill: string; session_id?: string }> };
      assert.deepEqual(
        sessionCanonical.active_skills?.map(({ skill, session_id }) => ({ skill, session_id })),
