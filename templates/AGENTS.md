...
This AGENTS.md is the top-level operating contract for the workspace. Its guidance is subordinate to system, developer, and explicit user instructions. Skill and role guidance cannot narrow authority already granted by the user.
...
- Use Codex native subagents for independent, bounded subtasks when delegation materially improves quality, speed, or correctness.
...
- ASK only when a destructive, irreversible, credential-gated, external-production, or materially scope-changing action lacks user authorization, or missing information or authority blocks progress.
- On AUTO-CONTINUE branches, do not use permission-handoff phrasing; state the next action or evidence-backed result.
- Keep going unless blocked; finish the current safe branch before asking for confirmation or handoff.
- Use absolute language only for true invariants: safety, security, side-effect boundaries, required output fields, workflow state transitions, and product contracts.
- Do not ask or instruct humans to perform ordinary non-destructive, reversible actions. Treat safe OWX runtime manipulation, state transitions, and ordinary command execution as agent responsibilities.
...
- Treat a change as UX-visible when it changes how a user completes a task or how the interface behaves over time, including interaction sequence, navigation, state transitions, validation or recovery, loading or progress feedback, focus or keyboard behavior, gestures, drag/drop, and animation.
...
- For every UX-visible change, capture a short screen recording that shows the starting state, the changed interaction, and the resulting outcome. Include every materially changed success, failure, or recovery path, keep the clip focused, and store it alongside the screenshots. Screenshots do not replace the required recording.
- In the completion response, embed or link the final screenshots and required recordings with absolute paths. Label screenshots with route, state, and viewport; label recordings with route, viewport, starting state, action, and outcome. Build, lint, typecheck, and unit-test results do not replace browser evidence.
- When a pull request is created or updated for frontend-visible work, add a `## Visual evidence` section to the PR body. Upload the final screenshots and required UX recordings through an authenticated GitHub PR editor or comment attachment flow, then put the resulting GitHub-hosted URLs in that section with route, state, and viewport labels. Local filesystem paths are not valid PR evidence, and `.owx` media stays uncommitted.
- Screenshots are the baseline PR evidence. Every UX-visible change also requires its screen recording of the starting state, changed interaction, and outcome; a recording is optional only for a purely visual change with no interaction, motion, or temporal behavior change. Use a GitHub-supported format and prefer H.264 MP4 for browser compatibility. Capture only sanitized states without secrets, tokens, personal data, or unrelated user content.
- Open the published PR as a reviewer and verify that every image and recording renders. If upload or rendered-media verification fails, preserve the local artifacts, report the exact blocker, and do not claim the PR handoff is complete or merge-ready.
- When no pull request exists, keep the labeled absolute local paths for the screenshots and required recordings in the completion response.
- If browser inspection, screenshot capture, or a required recording cannot run, report the exact blocker and mark the visual or UX verification incomplete. Do not claim the frontend or UX work is complete. Reference scoring and pixel-diff loops remain the responsibility of `$visual-ralph` when a visual reference exists; ordinary frontend changes do not require that skill.
</frontend_visual_evidence>
