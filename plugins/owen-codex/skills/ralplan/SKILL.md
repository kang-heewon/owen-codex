...
`$ralplan` is a planning mode. While ralplan is active and no explicit execution handoff is active, implementation-focused write tools are out of scope. Ralplan may inspect the repository and may write only planning artifacts such as `.owx/context/`, `.owx/plans/`, `.owx/specs/`, and required `.owx/state/` records.
...
4. If ambiguity remains high, gather brownfield facts first. `owx explore` is deprecated; use normal repository inspection tools/subagents for simple read-only repository lookups and `owx sparkshell` only for explicit shell-native read-only evidence. Then run `$deep-interview --quick <task>` before continuing.
