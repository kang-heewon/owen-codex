...
5. **Stale state**: old `.owx/state` files, mismatched sessions, missing timestamps, and contradictory phase metadata.
...
- **Sanitize OWX runtime env for isolated probes.** When the scenario creates a temporary repo/state tree or intentionally checks local isolation, run the probe with `OWX_ROOT` and `OWX_STATE_ROOT` unset (for example `env -u OWX_ROOT -u OWX_STATE_ROOT ...`) so ambient boxed runtime state cannot redirect reads/writes away from the scenario fixture.
...
Use the CLI-first state surface (`owx state ... --json`) for UltraQA lifecycle state. If explicit MCP compatibility tools are already available, equivalent `owx_state` calls are optional compatibility, not the default.

- **On start**:
  `owx state write --input '{"mode":"ultraqa","active":true,"current_phase":"planning","iteration":1,"started_at":"<now>","scenario_matrix":[]}' --json`
- **On each cycle**:
  `owx state write --input '{"mode":"ultraqa","current_phase":"qa","iteration":<cycle>,"scenario_matrix":"<updated matrix path or summary>"}' --json`
- **On adversarial e2e transition**:
  `owx state write --input '{"mode":"ultraqa","current_phase":"adversarial-e2e"}' --json`
- **On diagnose/fix transitions**:
  `owx state write --input '{"mode":"ultraqa","current_phase":"diagnose"}' --json`
  `owx state write --input '{"mode":"ultraqa","current_phase":"fix"}' --json`
- **On cleanup transition**:
  `owx state write --input '{"mode":"ultraqa","current_phase":"cleanup"}' --json`
- **On completion**:
  `owx state write --input '{"mode":"ultraqa","active":false,"current_phase":"complete","completed_at":"<now>"}' --json`
- **For resume detection**:
  `owx state read --input '{"mode":"ultraqa"}' --json`
...
`owx state clear --input '{"mode":"ultraqa"}' --json`
