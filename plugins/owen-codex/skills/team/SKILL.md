...
`$team` is the tmux-based parallel execution mode for OWX. It starts real worker Codex and/or Claude CLI sessions in split panes and coordinates them through `.owx/state/team/...` files plus CLI team interop (`owx team api ...`) and state files.

This skill is operationally sensitive. Treat it as an operator workflow, not a generic prompt pattern. In Codex App or plain outside-tmux sessions, do not present `$team` / `owx team` as directly available; launch OWX CLI from shell first, or stay on the nearest app-safe surface until the user explicitly wants the tmux runtime.
...
- Use **`owx team`** when you need durable tmux workers, shared task state, mailbox/dispatch coordination, worktrees, explicit lifecycle control, or long-running parallel execution that must survive beyond one local reasoning burst.
...
1. Invoke OWX runtime directly with `owx team ...`
...
If `owx team` is unavailable, stop with a hard error.

## Invocation Contract

```bash
owx team [N:agent-type] "<task description>"
```

Examples:

```bash
owx team 3:executor "analyze feature X and report flaws"
owx team "debug flaky integration tests"
owx team "ship end-to-end fix with verification"
```

### Team-first launch contract

`owx team ...` is now the canonical launch path for coordinated execution.
...
- **Canonical launch:** use plain `owx team ...` / `$team ...` for coordinated workers.
- **Verification ownership:** keep one lane focused on tests, regression coverage, and evidence before shutdown.
- **Escalation:** start a separate `owx ralph ...` / `$ralph ...` only when a later manual follow-up still needs a persistent single-owner fix/verification loop.
- **Deprecation:** `owx team ralph ...` has been removed. Use plain `owx team ...` for team execution or run `owx ralph ...` separately when you explicitly want a later Ralph loop.
...
ATEM fit: treat this as agile teamwork support for transition/action/interpersonal moments around boundaries, not as a heavyweight process model. Do not copy provider-specific plugin implementations; keep the protocol in OWX/Codex prompts, inboxes, state, and tests.

### Team + Ultragoal bridge

Use `$ultragoal` for durable leader-owned goal/ledger tracking and `$team` for parallel execution lanes. When Team is launched with an active `.owx/ultragoal/goals.json`, worker inboxes/status may include leader-owned Ultragoal context: `.owx/ultragoal/goals.json`, `.owx/ultragoal/ledger.jsonl`, the active goal id, Codex goal mode, and the `fresh_leader_get_goal_required` checkpoint policy.

Workers provide task status and verification evidence only. They do not own Ultragoal goal state, create worker ledgers, mutate `.owx/ultragoal`, auto-launch Team from Ultragoal, or perform hidden Codex goal mutation. The leader uses terminal Team evidence plus a fresh `get_goal` snapshot to run `owx ultragoal checkpoint --goal-id <id> --status complete --evidence "<team evidence mentioning .owx/ultragoal and <id>>" --codex-goal-json <fresh-get_goal-json-or-path>`.
...
OWX_TEAM_WORKER_CLI=claude owx team 2:executor "update docs and report"

# Mixed team (worker 1 = Codex, worker 2 = Claude)
OWX_TEAM_WORKER_CLI_MAP=codex,claude owx team 2:executor "split doc/code tasks"

# Auto mode: Claude is selected when worker launch args/model contains 'claude'
OWX_TEAM_WORKER_CLI=auto OWX_TEAM_WORKER_LAUNCH_ARGS="--model claude-..." owx team 2:executor "run mixed validation"
```
...
3. `owx` command resolves to the intended install/build
4. If running repo-local `node bin/owx.js ...`, run `npm run build` after `src` changes
...
If duplicates exist, remove extras before `owx team` to prevent HUD ending up in worker stack.

## Pre-context Intake Gate

Before launching `owx team`, require a grounded context snapshot:

1. Derive a task slug from the request.
2. Reuse the latest relevant snapshot in `.owx/context/{slug}-*.md` when available.
3. If none exists, create `.owx/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
   - task statement
   - desired outcome
...
For simple read-only brownfield lookups during intake, follow active session guidance: when `USE_OWX_EXPLORE_CMD` is enabled, prefer `owx explore` with narrow, concrete prompts; otherwise use the richer normal explore path and fall back normally if `owx explore` is unavailable.
...
- include an explicit launch hint (`owx team N "<task>"` / `$team N "<task>"`) for the coordinated team run; mention `$ultragoal` as the default durable follow-up/ledger path; mention a later separate Ralph follow-up only when explicitly requested or genuinely needed as a fallback
...
`owx team` currently performs:

1. Parse args (`N`, `agent-type`, task)
2. Sanitize team name from task text
3. Initialize team state:
   - `.owx/state/team/<team>/config.json`
   - `.owx/state/team/<team>/manifest.v2.json`
   - `.owx/state/team/<team>/tasks/task-<id>.json`
4. Compose team-scoped worker instructions file at:
   - `.owx/state/team/<team>/worker-agents.md`
   - Uses project `AGENTS.md` content (if present) + worker overlay, without mutating project `AGENTS.md`
5. Resolve canonical shared state root from leader cwd (`<leader-cwd>/.owx/state`)
6. Split current tmux window into worker panes
7. Launch workers with:
   - `OWX_TEAM_WORKER=<team>/worker-<n>`
   - `OWX_TEAM_STATE_ROOT=<leader-cwd>/.owx/state`
   - `OWX_TEAM_LEADER_CWD=<leader-cwd>`
   - worker CLI selected by `OWX_TEAM_WORKER_CLI` / `OWX_TEAM_WORKER_CLI_MAP` (`codex` or `claude`)
...
- Workers may run in separate git worktrees (`owx team --worktree[=<name>]`) while sharing one team state root
...
- Submit routing uses this CLI resolution order per worker trigger:
  ...
  2) `OWX_TEAM_WORKER_CLI_MAP` entry for that worker index,
  3) fallback `OWX_TEAM_WORKER_CLI` / auto detection.
...
1. Explicit worker model in `OWX_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model` flag
3. Low-complexity default from `OWX_DEFAULT_SPARK_MODEL` (legacy alias: `OWX_SPARK_MODEL`) when 1+2 are absent and team `agentType` is low-complexity

Default-model rule:
- Do **not** assume a frontier or spark model from recency or model-family heuristics.
- Use `OWX_DEFAULT_FRONTIER_MODEL` for frontier-default guidance.
- Use `OWX_DEFAULT_SPARK_MODEL` for spark/low-complexity worker-default guidance.
...
- When the leader assigns teammate roles/tasks, OWX allocates **per-worker reasoning effort dynamically** from the resolved worker role and `agentReasoning` overrides (`low`, `medium`, `high`, `xhigh`).
- Explicit launch args still win: if `OWX_TEAM_WORKER_LAUNCH_ARGS` already includes `-c model_reasoning_effort=...`, that explicit value overrides dynamic allocation for every worker.
...
2. Monitor task and worker progress with runtime/state tools first (`owx team status <team>`, `owx team resume <team>`, mailbox/state files)
3. Wait for terminal task state before shutdown:
   - `pending=0`
   - `in_progress=0`
   - `failed=0` (or explicitly acknowledged failure path)
4. Only then run `owx team shutdown <team>`
...
```bash
sleep 30 && owx team status <team-name>
```

Repeat that check while the team stays active, or use `owx team await <team-name> --timeout-ms 30000 --json` when event-driven waiting is a better fit.

If the leader gets a stale, lifecycle, or all-idle nudge, immediately run `owx team status <team-name>` before taking any manual intervention. Deprecated worker stall/progress nudges should not be treated as an active runtime contract.

### Deprecated worker stall/progress knobs

`OWX_TEAM_PROGRESS_STALL_MS` and `OWX_TEAM_WORKER_TURN_STALL_MS` are legacy compatibility/test-only names for the retired worker stall/progress nudge path. Do not recommend them as operator tuning knobs for active team runs; resolved native worker Stop, all-idle, mailbox, and stale-leader evidence are the supported leader wakeup signals.
...
1. Use `owx team ...` runtime lifecycle commands for orchestration.
2. Use `owx team api ... --json` for mailbox/task mutations.
3. Verify delivery via mailbox/state evidence (`mailbox/*.json`, task status, `owx team status`).
...
- `shutdown`: graceful shutdown request, then cleanup (deletes `.owx/state/team/<team>`)

## Data Plane and Control Plane

### Control Plane

- tmux panes/processes (`OWX_TEAM_WORKER` per worker)
- leader notifications via `tmux display-message`

### Data Plane

- `.owx/state/team/<team>/...` files
- Team mailbox files:
- `.owx/state/team/<team>/mailbox/leader-fixed.json`
- `.owx/state/team/<team>/mailbox/worker-<n>.json`
- `.owx/state/team/<team>/dispatch/requests.json` (durable dispatch queue; hook-preferred, fallback-aware)
...
- `.owx/state/team/<team>/manifest.v2.json`
- `.owx/state/team/<team>/tasks/task-<id>.json`
- `.owx/state/team/<team>/workers/worker-<n>/identity.json`
- `.owx/state/team/<team>/workers/worker-<n>/inbox.md`
- `.owx/state/team/<team>/workers/worker-<n>/heartbeat.json`
- `.owx/state/team/<team>/workers/worker-<n>/status.json`
- `.owx/state/team-leader-nudge.json`


## Team Mutation Interop (CLI-first)

Use `owx team api` for machine-readable mutation/reads instead of legacy `team_*` MCP tools.

```bash
owx team api <operation> --input '{"team_name":"my-team",...}' --json
```

Examples:

```bash
owx team api send-message --input '{"team_name":"my-team","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK"}' --json
owx team api claim-task --input '{"team_name":"my-team","task_id":"1","worker":"worker-1"}' --json
owx team api transition-task-status --input '{"team_name":"my-team","task_id":"1","from":"in_progress","to":"completed","claim_token":"<token>"}' --json
```
...
- Send ACK to `leader-fixed` mailbox via `owx team api send-message --json`
- Claim/transition/release task lifecycle via `owx team api <operation> --json`
...
Use only after checking `owx team status <team>` and mailbox/state evidence:

1. Capture pane tail to confirm current worker state:
   - `tmux capture-pane -t %<worker-pane> -p -S -120`
   - If a larger-tail read or bounded summary would help, prefer explicit opt-in inspection via `owx sparkshell --tmux-pane %<worker-pane> --tail-lines 400` before improvising extra tmux commands.
...
2. `.owx/state/team/<team>/mailbox/leader-fixed.json` exists
3. Worker skill loaded and `owx team api send-message --json` called
4. Task-id mismatch not blocking worker flow

### Worker logs `owx team api ... ENOENT` (or legacy `team_send_message ENOENT` / `team_update_task ENOENT`)

Meaning:
- Team state path no longer exists while worker is still running.
- Typical cause: leader/manual flow ran `owx team shutdown <team>` (or removed `.owx/state/team/<team>`) before worker finished.

Checks:

1. `owx team status <team>` and confirm whether tasks were still `in_progress` when shutdown occurred
2. Verify whether `.owx/state/team/<team>/` exists
3. Inspect worker pane tail for post-shutdown writes
4. Confirm no external cleanup (`rm -rf .owx/state/team/<team>`) happened during execution
...
rm -rf .owx/state/team/<team-name>

# 4) Retry
owx team 1:executor "fresh retry"
```

Guidelines:

- Do not kill leader pane
- Do not kill HUD pane (`owx hud --watch`) unless intentionally restarting HUD
...
Use `owx sparkshell --tmux-pane ...` as an explicit opt-in operator aid for pane inspection and summaries; keep raw `tmux capture-pane` evidence available for manual intervention and proof.

## Programmatic Team Orchestration

Use the `owx team ...` CLI as the supported team-launch surface. For automation, drive the same CLI flow from scripts or supervising agents rather than relying on a separate MCP runner.

### Supported current surfaces

- **`owx team ...` CLI** — Primary method for interactive or automated team orchestration. Use this when you want direct tmux-pane visibility or a scriptable launch path.
- **Team state files** — Inspect `.owx/state/team/<team>/` when you need status, task, or mailbox evidence after launch.

### Cleanup distinction

Two cleanup paths exist and must not be confused:

- `team_cleanup` (**state-server**): Deletes team state **files** on disk (`.owx/state/team/<team>/`). Use after a team run is fully complete.
- tmux/session cleanup: Use the documented `owx team` shutdown / cleanup flow when you need to stop worker panes or clean up an interrupted run.

### Automation example

```
1. owx team 1:executor "fix bugs"
2. owx team status <team-name>
3. owx team shutdown <team-name>
