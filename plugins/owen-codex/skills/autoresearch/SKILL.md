...
2. **Persist mode state** in `.owx/state/.../autoresearch-state.json` including:
   - `validation_mode`
...
```json
{
  ...
  "output_artifact_path": ".owx/specs/autoresearch-demo/report.md"
}
```

## Recommended flow
1. Run `$deep-interview --autoresearch` to clarify mission + evaluator.
2. Materialize `.owx/specs/autoresearch-{slug}/mission.md`, `sandbox.md`, and `result.json`.
