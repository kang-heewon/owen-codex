---
name: doctor
description: Diagnose and fix owen-codex installation issues
---
...
OWX installs skills to `${CODEX_HOME:-~/.codex}/skills/` — this is the path current Codex CLI natively loads as its skill root.

`~/.agents/skills/` is a **historical legacy path** from an older Codex CLI release, before Codex settled on `~/.codex` as its home directory. Current Codex CLI and OWX no longer write there.

**In a mixed OWX + plain Codex environment:**
...
- **Interop rule**: OWX writes only to the canonical path; archive or remove `~/.agents/skills/` once you have confirmed `${CODEX_HOME:-~/.codex}/skills/` is your active root

## Task: Run Installation Diagnostics

You are the OWX Doctor - diagnose and fix installation issues.

### Step 1: Check Plugin Version

Official Codex plugin caches are marketplace- and version-scoped, for example `${CODEX_HOME:-~/.codex}/plugins/cache/$MARKETPLACE_NAME/owen-codex/$VERSION/`. Local installs may use `local` as the version identifier.

```bash
# Get installed plugin cache versions across marketplaces.
# Cache shape: $PLUGIN_CACHE_ROOT/$MARKETPLACE_NAME/owen-codex/$PLUGIN_VERSION/
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
CACHE_ENTRIES=$(find "$PLUGIN_CACHE_ROOT" -path "*/owen-codex/*" -mindepth 3 -maxdepth 3 -type d 2>/dev/null)
...
# Get latest from npm
LATEST=$(npm view owen-codex version 2>/dev/null)
echo "Latest npm: $LATEST"
```

**Diagnosis**:
- If no cache entry exists: INFO - plugin marketplace artifact not cached; this may be normal when OWX was installed only through npm/setup
...
- Remember: plugin install/discovery is not a replacement for `npm install -g owen-codex` plus `owx setup`; the packaged plugin carries plugin-scoped companion metadata for official Codex lifecycle hooks and optional MCP compatibility servers/apps, with first-party MCP disabled by default, while native/runtime hooks and the rest of OWX runtime wiring stay setup-owned
...
# Check for OWX marker
grep -q "owen-codex Multi-Agent System" ~/.codex/AGENTS.md 2>/dev/null && echo "Has OWX config" || echo "Missing OWX config"
```

**Diagnosis**:
- If missing: CRITICAL - AGENTS.md not configured
- If missing OWX marker: WARN - outdated AGENTS.md
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
find "$PLUGIN_CACHE_ROOT" -path "*/owen-codex/*" -mindepth 3 -maxdepth 3 -type d 2>/dev/null \
  | while IFS= read -r VERSION_DIR; do
      MARKETPLACE_NAME=$(basename "$(dirname "$(dirname "$VERSION_DIR")")")
      PLUGIN_VERSION=$(basename "$VERSION_DIR")
...
**Diagnosis**:
- If `~/.codex/agents/` exists with owen-codex-related files: WARN - legacy generated agents or hand-installed role files. The Codex plugin can package reusable workflows plus plugin-scoped companion metadata for official Codex lifecycle hooks and optional MCP/apps; legacy setup mode installs prompts/native agents and .codex/hooks.json, while plugin setup mode archives stale legacy prompt/native-agent files and keeps config/hooks current.
- If `~/.codex/commands/` exists with owen-codex-related files: WARN - legacy command files from older installs. Current OWX uses skills/workflows plus setup-managed native surfaces.
- If `${CODEX_HOME:-~/.codex}/skills/` exists with OWX skills: OK - canonical current user skill root
...
- Any owen-codex-related `.md` files in skills/
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
find "$PLUGIN_CACHE_ROOT" -path "*/owen-codex" -type d -prune -exec rm -rf {} +
...
PLUGIN_CACHE_ROOT="${CODEX_HOME:-$HOME/.codex}/plugins/cache"
PLUGIN_CACHE_DIR="$PLUGIN_CACHE_ROOT/$MARKETPLACE_NAME/owen-codex"
...
**Note**: Only remove if these contain owen-codex-related files. If user has custom agents/commands/skills, warn them and ask before removing.
