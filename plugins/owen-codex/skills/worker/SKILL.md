---
name: worker
description: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based OWX teams
---

# Worker Skill

This skill is for a Codex session that was started as an OWX Team worker (a tmux pane spawned by `$team`).

## Identity

You MUST be running with `OWX_TEAM_WORKER` set. It looks like:
...
Use CLI interop:
- `owx team api send-message --input <json> --json` with `{team_name, from_worker, to_worker:"leader-fixed", body}`
...
6. Claim the task (do NOT start work without a claim) using claim-safe lifecycle CLI interop (`owx team api claim-task --json`).
7. Do the work.
8. Complete/fail the task via lifecycle transition CLI interop (`owx team api transition-task-status --json`) from `in_progress` to `completed` or `failed`.
   - Do NOT directly write lifecycle fields (`status`, `owner`, `result`, `error`) in task files.
9. Use `owx team api release-task-claim --json` only for rollback/requeue to `pending` (not for completion).
...
Use CLI interop:
- `owx team api mailbox-list --json` to read
- `owx team api mailbox-mark-delivered --json` to acknowledge delivery
...
- Prefer inbox/mailbox/task state and `owx team api ... --json` operations.
