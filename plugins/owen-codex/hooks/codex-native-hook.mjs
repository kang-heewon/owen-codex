...
const OWX_PLUGIN_HOOK_LAUNCHER_CONTRACT_MARKER = 'owx-plugin-hook-launcher:v1';
...
function stopFallbackOutput(stopReason, detail) {
  const reason = 'OWX plugin Stop hook launcher failed before valid native Stop JSON could be produced. Continue once, preserve runtime state, inspect hook launcher diagnostics, and retry.';
  return {
    decision: 'block',
    reason,
    stopReason,
...
function failLauncher(error, isStop, stopReason = 'plugin_stop_hook_launcher_failure') {
  ...
  console.error(`[owen-codex] ${detail}`);
  if (isStop) {
...
function readPinnedLauncher() {
  const launcherPath = join(hookDir, 'owx-command.json');
  try {
...
function readConfiguredLauncher() {
  if (process.env.OWX_NATIVE_HOOK_COMMAND) {
    return { command: process.env.OWX_NATIVE_HOOK_COMMAND, argsPrefix: [] };
  }
  return readPinnedLauncher() ?? { command: 'owx', argsPrefix: [] };
}
...
function listAuthoritativeStateBaseDirs(cwd) {
  if (process.env.OWX_TEAM_STATE_ROOT?.trim()) return [process.env.OWX_TEAM_STATE_ROOT.trim()];
...
function readCurrentSessionId(stateBaseDirs, cwd) {
  ...
  const envSessionId = process.env.OWX_SESSION_ID || process.env.CODEX_SESSION_ID;
...
async function main() {
  ...
  if (oversized) {
    ...
    if (isStop) {
      if (hasActiveAutopilotStateForOversizedStop(input)) {
        console.error(`[owen-codex] ${message}`);
  ...
  child.on('close', (code, signal) => {
    ...
    if (childSpawnError) {
      console.error(`[owen-codex] failed to launch ${command} codex-native-hook: ${childSpawnError.message}`);
      process.exitCode = 1;
      return;
    }
    if (signal) {
      console.error(`[owen-codex] codex-native-hook terminated by ${signal}`);
      process.exitCode = 1;
      return;
    }
...
main().catch((error) => {
  console.error(`[owen-codex] plugin hook launcher failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
